## 139fm.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：139fm 🔞
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## 139fm.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >139fm 🔞，无需登录！
^https?:\/\/139fm.(fun|bond|cfd|click|cyou)\/podcast\/\d+ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/139fm.js

#[mitm]
# hostname = 139fm.bond,139fm.fun,139fm.cfd,139fm.click,139fm.cyou

## 18pcs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：18品茶社 查看野🐔资料🔓
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## 18pcs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >18品茶社，无需登录！
^https?:\/\/18pcs.(fun|bond|cfd)\/record\/\d+ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/18pcs.js

#[mitm]
# hostname = 18pcs.bond,18pcs.fun,18pcs.cfd

## 18top.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：18TOPTOON 金币&会员解锁🔞
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年0817
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## 18top.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >18TOPTOON 金币&会员解锁 无需登录！
^https?:\/\/(boylove|toptoon).(online|click|cyou|monster)\/(buychapter|comic\/\d+) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/18top.js

#[mitm]
# hostname = toptoons.click,toptoons.online,toptoon.monster,boylove.cyou,boylove.*,toptoon.*

## 666.syh.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：666书友会 会员+付费课
# 软件版本：3.4.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## 666.syh.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 666书友会 会员+付费课
https?:\/\/app.666syh.com\/cloud\/api-(mall\/book\/(online\/getOnlineLearnDel|book\/getVideoDel)|member\/user\/memberDetail).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/666syh.js

#[mitm]  
# hostname = app.666syh.com

## 666syh.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：666书友会 会员+付费课
# 软件版本：3.4.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## 666syh.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 666书友会 会员+付费课
https?:\/\/app.666syh.com\/cloud\/api-(mall\/book\/(online\/getOnlineLearnDel|book\/getVideoDel)|member\/user\/memberDetail).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/666syh.js

#[mitm] 
# hostname = app.666syh.com

## 7dong.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：7动-凯格尔成人运动
# 软件版本：4.8.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## 7dong.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >7动-凯格尔成人运动
^https?:\/\/pc7dong.com\/seveng.+api\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/7dong.js


#[mitm]
# hostname = pc7dong.com

## 7semao.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：18+七色猫 视频、小说、漫画、去广告、凤楼、电台、
# 软件版本：h5&轻量版
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## 7semao.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >18+七色猫 视频、小说、漫画、去广告、凤楼、电台、直播

^https?:\/\/.+.(cn|com)\/api\/(Login\/login|login\/mobile_login|index\/loufeng_info|home\/video_info|user\/info) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/7semao.js
^https?:\/\/.+.(cn|com)\/api\/(index|home|ent)\/(cate_list|zuire|zuixin|index|loufeng|new_zuanti|cate_list|novel|comic|audio_no) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/7semaoad.js
^https?://.+\.(cn|com)/api/home/my_ad url reject-dict
#[mitm]
# hostname = qijk.fantor.cn,qijk.slrrj.com,ldy.syxsxl.com,qijk.accessco.cn,jsqiapi.hspherecn.com,jiekou.antciti.cn,qiip.fushizhengli.cn,ldy.*.cn,ldy.yyycb.com,ldy.yyycb.com,*yyycb*,117.161.96.7,api.tongyutongye.cn

## 7semaoad.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：18+七色猫 视频、小说、漫画、去广告、凤楼、电台、
# 软件版本：h5&轻量版
# 下载地址：
# 脚本作者：
# 更新时间：2024+
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## 7semaoad.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >18+七色猫 视频、小说、漫画、去广告、凤楼、电台、直播

^https?:\/\/jiekou.antciti.cn\/api\/(login\/mobile_login|index\/loufeng_info|home\/video_info|user\/info) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/7semao.js
^https?:\/\/jiekou.antciti.cn\/api\/(index|home)\/(cate_list|zuire|zuixin|index|loufeng|new_zuanti|cate_list) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/7semaoad.js
^https?://jiekou\.antciti\.cn/api/home/my_ad url reject-dict
#[mitm]
# hostname = jiekou.antciti.cn

## 9169.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：🔞9169视频 观影权限
# 软件版本：++
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## 9169.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >🔞9169视频 
^https?:\/\/.+\/bibidd\/(Mediaonenine\/panduan_huiyuan|VipChigua\/check_vip|Advert/get_index_Advert|Mediaonenine\/video_detail) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/9169.js


#[mitm]
# hostname = api.9169kkxstzsjkdd222.app

## 91fenglou.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## 91fenglou.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >91风流

^https?:\/\/91fengliu.club\/(index\/user\/userinfo|index\/data\/show).html url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/91fenglou.js

#[mitm]
# hostname = 91fengliu.club

## Acfun.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：Acfun最高画质
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## Acfun.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >Acfun最高画质
^https?:\/\/.+\/rest\/app\/douga/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/Acfun.js

#[mitm] 
# hostname = api-ipv6.acfunchina.com,api-ipv6.app.acfun.cn

## BLBLHD.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：哔哩哔哩 大会员 画质解锁🔓
# 软件版本：8.22.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## BLBLHD.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 哔哩哔哩 大会员 画质解锁🔓
^https?:\/\/app.bilibili.com\/x\/v2\/(account\/(myinfo|mine)|(feed\/index))\? url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/BLBLHD.js
^https?:\/\/app.bilibili.com\/bilibili.app.playerunite.v1.Player\/PlayViewUnite url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/BLBLHD.js

#[mitm]
# hostname = *.bilibili.*,*.biliapi.*,111.31.33.*,111.6.167.*,app.bilibili.com, api.bilibili.com, api.live.bilibili.com, grpc.biliapi.net

## DraGo.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## DraGo.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## Duodu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：多人待办清单任务管理习惯养成ai好友监督提醒
# 软件版本：1.4.18
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## Duodu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 多人待办清单任务管理习惯养成ai好友监督提醒
^https:\/\/tasky-api\.codefuture\.top\/v1 url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/Duodu.js

#[mitm]
# hostname = tasky-api.codefuture.top

## Ease.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：ease助眠-改善睡眠，缓解焦虑，冥想减压，白噪音（永久会员）
# 软件版本：4.8.3
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## Ease.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >ease助眠-改善睡眠，缓解焦虑，冥想减压，白噪音（永久会员）
^https?:\/\/api0.easeprime.com\/api\/memberInfo\/myInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/Ease.js

#[mitm] 
# hostname = api0.easeprime.com

## Fkj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：会员课+付费课+大师课+直播课
# 软件版本：3.1.2
# 下载地址：
# 脚本作者：**
# 更新时间：2024-2.3$
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## Fkj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 芬空间,会员课+付费课+大师课+直播课
^https?:\/\/api\.zhangdefenspace\.com\/api\/(course\/getCoursePeriodList|course\/getCourseDetails|user\/getIndexUser|v2\/live\/getLiveDetails).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/Fkj.js

#[mitm] 
# hostname = api.zhangdefenspace.com

## Fonts.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：微信字体（永久会员）
# 软件版本：1.5.22
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## Fonts.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >微信字体（永久会员）
^https?:\/\/api.revenuecat.com\/v1\/receipts url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/Fonts.js

#[mitm] 
# hostname = api.revenuecat.com

## Foodie.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：foodie - 美食相机（永久会员）
# 软件版本：6.2.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## Foodie.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >foodie - 美食相机（永久会员）
^https?:\/\/purchase-foodiecn-api.yiruikecorp.com\/v1\/purchase\/subscription\/subscriber\/status url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/Foodie.js

#[mitm] 
# hostname = purchase-qingtu-api.b612kaji.com,api-qingtu.kajicam.com,purchase-tianyan-api.tianyancam.com,purchase-foodiecn-api.yiruikecorp.com

## LuckPik.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：LuckPik 恢复购买 
# 软件版本：1.0.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## LuckPik.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >LuckPik  (ai智能抠图-抠图软件一键扣图换背景-图片变清晰)
^https?:\/\/front-gw.luckpik.com\/productAuthorizeService\/user\/auth\/query\/allAuthSimple url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/LuckPik.js


#[mitm]
# hostname = front-gw.luckpik.com

## MGYY2.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：咪咕音乐
# 软件版本：7.48.0
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## MGYY2.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 去广告
^https://app\.c\.nf.migu\.cn/strategy/listen-url/v2.5 url 302 https://app.c.nf.migu.cn/strategy/listen-url/v2.4
^https://app\.c\.nf\.migu\.cn/member/api/marketing/text url reject
^https://app\.c.nf\.migu\.cn/payment/watch-ad url reject-200
# > 咪咕音乐vip、音质
^https?:\/\/(u|c|app).(musicapp|(c|u).nf).migu.cn.+(user\/api|column\/startup|resource\/skin) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/MGYY2.js
^https?:\/\/app.(c|pd).nf.migu.cn\/.*\/(listen-url|music\/batchQueryMusicPolicy|download-url).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/MGYY2CK.js
#[mitm] 
# hostname = *.migu.cn

## MGYY2CK.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：咪咕音乐
# 软件版本：7.48.0
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## MGYY2CK.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 咪咕音乐vip、音质
^https?:\/\/(u|c|app).(musicapp|(c|u).nf).migu.cn.+(user\/api|column\/startup|resource\/skin) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/MGYY2.js
^https?:\/\/app.(c|pd).nf.migu.cn\/.*\/(listen-url|music\/batchQueryMusicPolicy|download-url).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/MGYY2CK.js
# > 去广告
^https://app\.c\.nf\.migu\.cn/strategy/listen-url/v2.5 url 302 https://app.c.nf.migu.cn/strategy/listen-url/v2.4
^https://app\.c\.nf\.migu\.cn/member/api/marketing/text url reject
^https://app\.c.nf\.migu\.cn/payment/watch-ad url reject-200
#[mitm] 
# hostname = *.migu.cn

## Mixgram.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：collage-maker-图片拼接-拼图软件-照片组合（永久会员）
# 软件版本：18.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## Mixgram.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >collage-maker-图片拼接-拼图软件-照片组合（永久会员）
^https?:\/\/us-central1-project.+cloudfunctions.net\/validateSubscription url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/Mixgram.js

#[mitm] 
# hostname = us-central1-project*

## QDVIP.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：起点读书 解锁会员免费听书板块
# 软件版本：5.9.364
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## QDVIP.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >起点读书 解锁会员免费听书板块
^https?:\/\/.+.qidian.com\/argus\/api\/v[0-9]\/(adv\/getadvlistbatch|user\/getaccountpage|subscription\/getvipprice|client\/getsplashscreen) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/QDVIP.js
^https?:\/\/.+.qidian.com\/argus\/api\/v[0-9]\/(audio\/getAsrInfo\?|bookcontent\/getvipcontent|audio\/getTtsChapterAudio\?|audio\/getaudiochapter\?) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/QDVIP.js

#[mitm] 
# hostname = *.qidian.com,*.if.qidian.com

## QQYD.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：QQ阅读 会员读书
# 软件版本：8.3.01
# 下载地址：
# 脚本作者：
# 更新时间：2025年0820
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## QQYD.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >QQ阅读 会员读书
^https?:\/\/(detailadr|commontgw).reader.qq.com\/(book\/queryDetailPage|.+nativepage\/personal|.+vip\/viptxt) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/QQYD.js
^https?:\/\/newminerva-tgw.reader.qq.com\/ChapBatAuthWithPD url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/QQYD.js

#[mitm] 
# hostname = *.reader.qq.com,newminerva-tgw.reader.qq.com,detailadr.reader.qq.com,commontgw.reader.qq.com

## QQYDTK.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## QQYDTK.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## QuiCut.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：quicut-video editor & maker（永久会员）
# 软件版本：1.0.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## QuiCut.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >quicut-video editor & maker（永久会员）
^https?:\/\/outcut.szsszykj.com\/ap\/OverseasPay\/GetVip url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/QuiCut.js

#[mitm] 
# hostname = outcut.szsszykj.com

## Retouch.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：touchretouch-消除笔及p图修图软件-神奇橡皮擦（恢复购买）
# 软件版本：5.2.4
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## Retouch.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >touchretouch-消除笔及p图修图软件-神奇橡皮擦（恢复购买）
^https?:\/\/api.revenuecat.com\/v1\/receipts url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/Retouch.js

#[mitm] 
# hostname = api.revenuecat.com

## Shzb.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：神户直播(随意输入11位手机号密码=vip)
# 软件版本：1.0.0
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年3月27日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## Shzb.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 神户直播(重新登陆账号->随意输入11位手机号密码=vip)
^https?:\/\/(gameapi.mma.dstor.cc|(pop|push).flare.com.51kuajing.net)\/index.php\/App\/(Account\/Login|User\/Info|Index\/Update).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/Shzb.js
^https://www\.shenhu\.click url 302 https://t.me/GieGie777


#[mitm] 
# hostname = gameapi.mma.dstor.cc,*.flare.com.51kuajing.net

## aiaiwang.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：爱爱网🔞 会员、去广告
# 软件版本：H5/网页版
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## aiaiwang.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 爱爱网🔞 会员、去广告
^https?:\/\/aawapi-v3\.trh999\.com\/msg_demo\/v\/(video\/(mayAlsoLike|index|getVideoInfoNew)|vip\/(VIPVideos|suggestVideo)|index(\/(getAdvertiseList|getIndexCarouselList))?) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/aiaiwang.js

#[mitm]
# hostname = aawapi-v3.trh999.com

## aikanshu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：爱看书-看你爱看的小说
# 软件版本：7.1.3
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## aikanshu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >爱看书-
^http:\/\/.+\/(ios\/regOrLogin|getTotalCoin) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/aikanshu.js



#[mitm]
# hostname = 

## anji.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：暗记-ankiapp-记忆神器-flashcards
# 软件版本：11.2.1
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## anji.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 暗记-ankiapp-记忆神器-flashcards
^https:\/\/api\.ankiapp\.com\/users\/account url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/anji.js

#[mitm]
# hostname = api.ankiapp.com

## atsyj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：收音机-fm广播电台-爱听收音机 vip
# 软件版本：2.3
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## atsyj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >收音机-fm广播电台-爱听收音机
^https?:\/\/tuoba.dida110.com\/(LRGetVipItem|LRGetMeCount).ashx url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/atsyj.js


#[mitm]
# hostname = tuoba.dida110.com

## b612.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：b612咔叽（永久会员）
# 软件版本：13.2.5
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## b612.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >b612咔叽（永久会员）
^https?:\/\/user-kaji-api.b612kaji.com\/v1\/(purchase\/subscription\/subscriber\/status|purchase\/item\/user\/info) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/b612.js

#[mitm] 
# hostname = purchase-qingtu-api.b612kaji.com,api-qingtu.kajicam.com,user-kaji-api.b612kaji.com

## baimaduanju.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：白马剧场
# 软件版本：1.0.2
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## baimaduanju.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >
^https?:\/\/theater-api.sylangyue.xyz\/api\/(playlet\/episode|user\/info) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/baimaduanju.js



#[mitm]
# hostname = theater-api.sylangyue.xyz

## bbzjs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：宝宝早教社 vip  
# 软件版本：2.0.4
# 下载地址：
# 脚本作者：
# 更新时间：2024.4.14
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## bbzjs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 宝宝早教社 vip 
^https?:\/\/appss.rhinoxlab.com\/\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/bbzjs.js
# > 开屏广告
^https://ad\.rhinox\.cn/client/sdk/getData url reject
#[mitm] 
# hostname = ad.rhinox.cn,standard.rhinoxlab.com,appss.rhinoxlab.com

## bdc.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：hellowords背单词-英语日语韩语法语德语西班牙语学习
# 软件版本：2.2.22
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## bdc.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > hellowords背单词-英语日语韩语法语德语西班牙语学习
^https:\/\/api-cn\.hellotalk8\.com\/hello_words\/app url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/bdc.js

#[mitm]
# hostname = api-cn.hellotalk8.com

## bdyy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：波点音乐 会员调试 + 去广告 + 下载歌曲(珍藏下载支持付费专辑导出文件) + 付费专辑解锁
# 软件版本：5.2.5
# 下载地址：
# 脚本作者：
# 更新时间：2025年10月14日
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## bdyy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 波点音乐 会员调试 + 去广告 + 下载功能 + 付费专辑解锁
^https?:\/\/(?:(?:bd-api|h5app|bodianyin|ab-bodian)\.kuwo\.cn\/(?:api\/(?:ucenter\/users\/(?:pub|login)|play\/(?:music\/v2\/(?:audioUrl|checkRight)|advert\/info)|service\/(?:music\/(?:info|download\/(?:info|config))|home\/module|global\/config\/(?:scene|vipEnter)|banner\/positions|advert\/config)|search\/topic\/word\/list|pay\/(?:vip\/(?:invitation\/(?:assist\/popup|swell)|lowPriceText)|sp\/actVip|audition\/url)|advert\/free\/config|popup\/start\/info|abtest\/ui\/info|rec\/feed)|abtest\/ui\/info)|us\.l\.qq\.com\/exapp|xs\.gdt\.qq\.com\/style_factory\/template_list) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/bdyy.js
^https?:\/\/tmeadcomm.y.qq.com\/ url reject-200
#[mitm]
# hostname = tmeadcomm.y.qq.com,bd-api.kuwo.cn,*.kuwo.*,ad.tencentmusic.com,*.l.qq.com,39.156.123.46:443,bd-api.kuwo.com,39.156.123.46,39.156.12*.*:443,h5app.kuwo.cn,bodianimgcdn.kuwo.cn,h5s.kuwo.cn,39.156.121.20:443,111.30.171.170:443,39.156.121.21:443,h5s.kuwo.cn,39.156.121.20:443,101.42.133.54:443,ab-bodian.kuwo.cn,39.156.121.65:443,39.156.121.*:443,bd-api.kuwo.cn:443,ab-bodian.kuwo.cn:443,39.156.121.60:80,xs.gdt.qq.com

## bdyybata1.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：波点音乐 会员调试 + 去广告 + 下载歌曲(珍藏下载支持付费专辑导出文件) + 付费专辑解锁
# 软件版本：5.2.5
# 下载地址：
# 脚本作者：
# 更新时间：2025年10月05日
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## bdyybata1.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 波点音乐 会员调试 + 去广告 + 下载功能 + 付费专辑解锁
^https?:\/\/(?:(?:bd-api|h5app|bodianyin|ab-bodian)\.kuwo\.cn\/(?:api\/(?:ucenter\/users\/(?:pub|login)|play\/(?:music\/v2\/(?:audioUrl|checkRight)|advert\/info)|service\/(?:music\/(?:info|download\/(?:info|config))|home\/module|global\/config\/(?:scene|vipEnter)|banner\/positions|advert\/config)|search\/topic\/word\/list|pay\/(?:vip\/(?:invitation\/(?:assist\/popup|swell)|lowPriceText)|sp\/actVip|audition\/url)|advert\/free\/config|popup\/start\/info|abtest\/ui\/info|rec\/feed)|abtest\/ui\/info)|us\.l\.qq\.com\/exapp|xs\.gdt\.qq\.com\/style_factory\/template_list) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/bdyybata1.js

#[mitm]
# hostname = bd-api.kuwo.cn,*.kuwo.*,ad.tencentmusic.com,*.l.qq.com,39.156.123.46:443,bd-api.kuwo.com,39.156.123.46,39.156.12*.*:443,h5app.kuwo.cn,bodianimgcdn.kuwo.cn,h5s.kuwo.cn,39.156.121.20:443,111.30.171.170:443,39.156.121.21:443,h5s.kuwo.cn,39.156.121.20:443,101.42.133.54:443,ab-bodian.kuwo.cn,39.156.121.65:443,39.156.121.*:443,bd-api.kuwo.cn:443,ab-bodian.kuwo.cn:443,39.156.121.60:80,xs.gdt.qq.com

## bgydtoken.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## bgydtoken.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## bijia.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：京东比价
# 软件版本：15.3.30
# 下载地址：
# 脚本作者：#https://raw.githubusercontent.com/mw418/Loon/main/script/jd_price.js#
# 更新时间：2026年
# 电报频道：https://t.me/GieGie777
# 问题反馈：无需第三方软件获取Cookie、历史价格会显示在京东商品“详情”页。
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## bijia.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >京东比价
^https?:\/\/in\.m\.jd\.com\/product\/graphext\/\d+\.html url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/bijia.js

#[mitm]
# hostname = in.m.jd.com

## bizhi.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：壁纸 - 精选高清墙纸图片和背景主题
# 软件版本：4.5.5
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年3月22日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## bizhi.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 壁纸 - 精选高清墙纸图片和背景主题Vip&Svip
^https?:\/\/leancloud.emotionwp.com\/.*\/classes\/wpf_account url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/bizhi.js

#[mitm] 
# hostname = leancloud.emotionwp.com

## blbldownload.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## blbldownload.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## blblplayurl.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## blblplayurl.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## bqwz.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：标枪王者(微信小程序)
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## bqwz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/javelin.mandrillvr.com\/api\/data\/get_game_data url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/bqwz.js

#[mitm]
# hostname = javelin.mandrillvr.com

## bsq.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：变声器 - 游戏开黑聊天交友语音包（永久会员）
# 软件版本：1.1.2
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## bsq.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >变声器 - 游戏开黑聊天交友语音包（永久会员）
^https?:\/\/appss.baomingding.com\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/bsq.js

#[mitm] 
# hostname = appss.baomingding.com

## bsqbsb.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：变声器-游戏语音开黑聊天变音器软件（永久会员）
# 软件版本：1.1.7
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## bsqbsb.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >变声器-游戏语音开黑聊天变音器软件（永久会员）
^https?:\/\/voicechanger.szsszykj.com\/interface\/GetIosVip.php url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/bsqbsb.js

#[mitm] 
# hostname = voicechanger.szsszykj.com

## buguyuedu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：布咕阅读     解锁付费，会员章节书籍需要在广告模式用咕咕卷购买
# 软件版本：2.6.6
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：需要在广告模式用咕咕卷购买才行
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## buguyuedu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 布咕阅读
^https:\/\/app.api.bgwxc.com\/app\/user\/getUserGold url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/buguyuedu.js
^https?:\/\/app.api.bgwxc.com\/app\/novel\/(contentV2|sub) url script-request-body https://raw.githubusercontent.com/WeiGiegie/666/main/bgydtoken.js
#[mitm] 
# hostname = app.api.bgwxc.com

## bxkt.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：伴学课堂
# 软件版本：1.4.4
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## bxkt.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# >口语星球
^https?:\/\/api\.banxueketang\.com\/api\/classpal\/app\/v1 url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/bxkt.js

#[mitm] 
# hostname = api.banxueketang.com

## byhz.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：鲍鱼盒子18+
# 软件版本：
# 下载地址：无法变身或者无网络拷贝右边数字符号重新打开👉~@85910590@~👈
# 脚本作者：
# 更新时间：20260520
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## byhz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >鲍鱼盒子18+
^https?:\/\/.+\/api\/(video\/report_item?|live\/room\/detail?|video\/related?|video\/detail|socialposts_info|my\/profile) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/byhz.js
^https?://siajksslwiso\.anningsh\.com/report url reject-200

#[mitm] 
# hostname = w1c2.ooat88.com,tu.dfgfdg.win,103.44.236.*,*.artrg.cn,vkfqb.ojyrz.com,byapi.*.com,211.99.98.*,103.39.222.*,yubaoyu.oss-cn-shenzhen.aliyuncs.com,*api.h8h4h.com,42.157.129.25,siajksslwiso.anningsh.com,211.99.98.63,111.180.204.11,88.gbvdfe.com,vkfqb.ojyrz.com,211.99.98.*,vdop.fqoas.com,ica.ooat88.com,103.44.236.131:8001,211.154.21.150:8020,211.154.21.150,103.215.78.*

## cake.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：cake-学习英语与韩语
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## cake.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https:\/\/api\.mycake\.me\/app\/start url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/cake.js

#[mitm]
# hostname = api.mycake.me

## catvpn_extract.js 从第1行到 [rewrite_local] 前一行：
# /*
#  * 脚本功能：CatVPN节点提取
#  * 软件版本：1.4
#  * 脚本作者：https://t.me/GieGie777
#  * 更新时间：2026年01月10日
#  * 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
#  *******************************

## catvpn_extract.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > CatVPN节点提取
^https?:\/\/firebaseremoteconfig\.googleapis\.com\/v1\/projects url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/catvpn_extract.js

#[mitm]
# hostname = firebaseremoteconfig.googleapis.com

## cdsp.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：彩豆视频水印宝 – 视频加水印制作视频剪辑（永久会员）
# 软件版本：1.1.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## cdsp.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >彩豆视频水印宝 – 视频加水印制作视频剪辑（永久会员）
^https?:\/\/appss.baomingding.com\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/cdsp.js

#[mitm] 
# hostname = appss.baomingding.com

## ceshi.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## ceshi.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## chaozipai.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：潮自拍
# 软件版本：4.1.45
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## chaozipai.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >潮自拍
^https?:\/\/.+meitu.com\/(v2\/user\/vip_info_by_group|filter\/timeline|v2\/user\/info_by_entrance).json url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/chaozipai.js


#[mitm]
# hostname = api-sub.meitu.com,api.selfiecity.meitu.com

## clj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：测量计数扫描宝-尺子测距仪（永久会员）
# 软件版本：1.0.7
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## clj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >测量计数扫描宝-尺子测距仪（永久会员）
^https?:\/\/cljssmb.5exg.cn\/papp/getuserinfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/clj.js

#[mitm] 
# hostname = cljssmb.5exg.cn

## cyljy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：成语来解压 (微信小程序) 
# 软件版本：无限金币，可购买游戏道具
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## cyljy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/yr-game-api.feigo.fun\/api\/user\/get-game-user-value url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/cyljy.js

#[mitm]
# hostname = yr-game-api.feigo.fun

## cytq.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：彩云天气
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2024年8.29
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************
# 
# ====================================

## cytq.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# 普通版广告
^https:\/\/ad\.cyapi\.cn\/v2\/req\?app_name=weather url reject-dict
# 赏叶赏花
^https:\/\/wrapper\.cyapi\.cn\/v1\/activity\?app_name=weather url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/cytq.js
# 解锁vip
^https:\/\/biz\.cyapi\.cn\/v2\/user url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/cytq.js
# 卫星云图 48小时预报
^https:\/\/wrapper\.cyapi\.cn\/v1\/(satellite|nafp\/origin_images) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/cytq.js

#[mitm]
# hostname = *.cyapi.cn

## dbdc.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：多背单词-英语学习-考研英语-英语单词-记单词必备软件
# 软件版本：3.5.8
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## dbdc.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 多背单词-英语学习-考研英语-英语单词-记单词必备软件
^http:\/\/www\.duobeidanci\.com\/MobileDevice url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/dbdc.js

#[mitm]
# hostname = www.duobeidanci.com

## dcy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：单词鸭-消消乐背单词-导入单词自建词库-场景主题英语词根词缀
# 软件版本：3.8.2
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## dcy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https:\/\/zhcn\.api\.wordln\.com\/tomato-word url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/dcy.js

#[mitm]
# hostname = zhcn.api.wordln.com

## ddqsy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：抖抖去水印 Vip
# 软件版本：1.4
# 下载地址：
# 脚本作者：
# 更新时间：2024年3月22日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ddqsy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 抖抖去水印 Vip
^https?:\/\/kkyun.com\/api\/(accounts\/user|apps/start_task).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ddqsy.js

#[mitm] 
# hostname = kkyun.com

## dfm.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：懂法猫-裁判文书网司法案例检索法律法规大全合同审查法律师工具
# 软件版本：5.7
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## dfm.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 懂法猫-裁判文书网司法案例检索法律法规大全合同审查法律师工具
^https:\/\/app\.dongfamao\.com:443\/prod-api\/app\/user\/userInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/dfm.js

#[mitm]
# hostname = app.dongfamao.com,app.dongfamao.com:443

## dingdingshuiyin.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：盯盯水印相机-工作查岗打卡拍照实时记录上传
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## dingdingshuiyin.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >盯盯水印相机 VIP
^https?:\/\/camera.xinmengma.com\/api\/user url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/dingdingshuiyin.js

#[mitm]
# hostname = camera.xinmengma.com

## djcs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：dj串烧集-大型dj串烧歌曲
# 软件版本：2.3.8
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## djcs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > dj串烧集-大型dj串烧歌曲
^http:\/\/csj\.yy-5\.com\/api\/User\/Info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/djcs.js

#[mitm]
# hostname = csj.yy-5.com

## djyyk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：dj音乐库-dj电音音乐播放器  VIP
# 软件版本：2.6.7
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## djyyk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >dj音乐库-dj电音音乐播放器 VIP
^http:\/\/app-i.dj-5.com\/api\/User\/Info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/djyyk.js

#[mitm]
# hostname = app-i.dj-5.com

## dkxjc.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：抖看小剧场-海量精彩短剧抢先看
# 软件版本：2.1.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## dkxjc.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >抖看小剧场-海量精彩短剧抢先看
^https?:\/\/(videohub.xdplt|mv-ps.xdplt).com\/api\/v1\/(short\/play|payment\/apple\/pay\/verify\/info|user\/info) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/dkxjc.js


#[mitm]
# hostname = mv-ps.xdplt.com,videohub.xdplt.com

## dkxs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：抖看小说-看海量小说电子书的阅读器
# 软件版本：1.8.3
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## dkxs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >抖看小说-看海量小说电子书的阅读器 解锁VIP
^https?:\/\/(videohub.xdplt|mv-ps.xdplt).com\/api\/v1\/(short\/play|payment\/apple\/pay\/verify\/info|user\/info) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/dkxs.js


#[mitm]
# hostname = mv-ps.xdplt.com,videohub.xdplt.com

## dmx.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：呆萌熊番茄钟-学霸必备提高学习效率神器 vip  
# 软件版本：2.2.1
# 下载地址：
# 脚本作者：
# 更新时间：2024.4.17
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## dmx.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 呆萌熊番茄钟-学霸必备提高学习效率神器 vip 
^https?:\/\/appss.rhinox.cn\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/dmx.js
# > 开屏广告
^https://ad\.rhinox\.cn/client/sdk/getData url reject
#[mitm] 
# hostname = ad.rhinox.cn,standard.rhinoxlab.com.appss.rhinoxlab.com,appss.rhinox.cn

## dmxj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：动漫相机-照片秒变漫画脸
# 软件版本：1.0.4
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## dmxj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >动漫相机-照片秒变漫画脸
^https?:\/\/standard.rhinoxlab.com\/standard\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/dmxj.js
#[mitm] 
# hostname = standard.rhinoxlab.com

## douhuayuedu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：豆花阅读-耽美言情百合小说大全 解锁金币章节购买
# 软件版本：10.11.1
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## douhuayuedu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >豆花阅读-耽美言情百合小说大全 解锁金币章节购买
^https?:\/\/api.douhuawenxue.com\/(index.php\/home\/splash_screen|new_user\/new_user\/personal_center) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/douhuayuedu.js
^https?:\/\/api.douhuawenxue.com\/index.php\/post\/(view|buy_chapter) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/douhuayuedu.js


#[mitm]
# hostname = api.douhuawenxue.com

## dsb.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：读手表-insight
# 软件版本：2.1.9
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## dsb.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 读手表-insight
^https:\/\/watch\.taotiangou\.cn\/prod-api\/watch\/insight\/userInfo\/getUserMemberInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/dsb.js

#[mitm]
# hostname = watch.taotiangou.cn

## dspjj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：短视频剪辑-vlog视频剪辑裁剪软件 vip 
# 软件版本：1.0.5
# 下载地址：
# 脚本作者：
# 更新时间：2024.4.6
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## dspjj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 短视频剪辑-vlog视频剪辑裁剪软件 vip 
^https?:\/\/appss.rhinox.cn\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/dspjj.js
# > 开屏广告
^https://ad\.rhinox\.cn/client/sdk/getData url reject
#[mitm] 
# hostname = ad.rhinox.cn,appss.rhinoxlab.com,appss.rhinox.cn

## dtpk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：地铁跑酷 (微信小程序) 修改金币、钥匙等货币
# 软件版本：
# 脚本作者：
# 更新时间：2026-03-14
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## dtpk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/dtpkwx-prod-web\.gxpan\.cn\/api\/user\/user_other_save_read url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/dtpk.js

#[mitm]
# hostname = dtpkwx-prod-web.gxpan.cn

## dzqj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：电子请柬制作 - 婚礼宝宝满月邀请函视频制作
# 脚本功能：婚礼精选-结婚电子请柬和请帖制作
# 脚本功能：海报制作大师 - 设计logo、邀请函请柬等图片编辑软件
# 软件版本：++
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## dzqj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >电子请柬制作 - 婚礼宝宝满月邀请函视频制作vip 3.1.8
# >婚礼精选-结婚电子请柬和请帖制作 vip 8.0.5
# >海报制作大师 - 设计logo、邀请函请柬等图片编辑软件3.3.4
^https?:\/\/(cn.invit.vip|www.lovewith.me|poster.leminet.cn)\/(users\/info|golove\/v05\/profile|v01\/profile) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/dzqj.js

#[mitm] 
# hostname = cn.invit.vip,www.lovewith.me,poster.leminet.cn

## els.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：俄罗斯克拉 去会员+广告🔞
# 软件版本：1.1.10
# 下载地址：
# 脚本作者：
# 更新时间：2026年9月10日
# 电报频道：https://t.me/GieGie777
# 问题反馈：https://t.me/GieGie777
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## els.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 俄罗斯克拉 去会员+广告🔞
^https?:\/\/sixth.xxcjpt.com\/java\/(user\/my|show) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/els.js

#[mitm]
# hostname = sixth.xxcjpt.com

## elskl.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：俄罗斯克拉 去会员+广告🔞
# 软件版本：1.1.10
# 下载地址：
# 脚本作者：
# 更新时间：2026年9月10日
# 电报频道：https://t.me/GieGie777
# 问题反馈：https://t.me/GieGie777
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## elskl.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 俄罗斯克拉 去会员+广告🔞
^https?:\/\/sixth.xxcjpt.com\/java\/(user\/my|show) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/elskl.js

#[mitm]
# hostname = sixth.xxcjpt.com

## fanqdj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：番茄短剧-视频大全海量爆款短剧持续更新
# 软件版本：1.2.0.1
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## fanqdj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 番茄短剧-视频大全海量爆款短剧持续更新
^http:\/\/8.149.129.124:3002\/api\/account\/v1 url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/fanqdj.js


#[mitm]
# hostname = 

## fanshu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：番薯小说阅读器—百万正版小说听书看书神器（解锁会员）
# 软件版本：2.5.76
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## fanshu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >番薯小说阅读器—百万正版小说听书看书神器（解锁会员）
^https?:\/\/g20.manmeng168.com\/v1\/client\/user\/completeUserInfo\? url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/fanshu.js

#[mitm] 
# hostname = g20.manmeng168.com

## fdds.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：帆书-原樊登读书 解锁樊登讲书+课程+电子书
# 软件版本：6.7.1
# 下载地址：
# 脚本作者：
# 更新时间：2024+
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## fdds.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 帆书-原樊登读书 解锁樊登讲书+课程
^https?:\/\/.+dushu365.com\/(sns-orchestration-system\/homePage\/api\/v100\/myPage|smart-orch\/program|smart-orch\/course\/v100\/info|innovation-orchestration\/api\/ebook\/v100\/ebookInfo) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/fdds.js
^https?:\/\/.+dushu365.com\/resource-orchestration-system\/book\/v101\/content url script-request-body https://raw.githubusercontent.com/WeiGiegie/666/main/fdtk.js


#[mitm]
# hostname = *.dushu365.com,gw2.dushu365.com,gw1.dushu365.com

## fdtk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：帆书-原樊登读书 解锁樊登讲书+课程
# 软件版本：6.7.1
# 下载地址：
# 脚本作者：
# 更新时间：2025++
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## fdtk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 帆书-原樊登读书 解锁樊登讲书+课程
^https?:\/\/.+dushu365.com\/(sns-orchestration-system\/homePage\/api\/v100\/myPage|smart-orch\/program) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/fdds.js
^https?:\/\/.+dushu365.com\/resource-orchestration-system\/book\/v101\/content url script-request-body https://raw.githubusercontent.com/WeiGiegie/666/main/fdtk.js


#[mitm]
# hostname = *.dushu365.com,gw2.dushu365.com,gw1.dushu365.com

## felo.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：felo实时翻译-同传与语音字幕 解锁免费时长++
# 软件版本：3.2.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## felo.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >felo实时翻译-同传与语音字幕
^https?:\/\/(translator|accounts).felo.me\/api\/user.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/felo.js
^https?:\/\/translator.felo.me\/api\/translation\/thirdToken\?token_channel url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/felotk.js
#[mitm] 
# hostname = accounts.felo.me,translator.felo.me,52.78.154.178

## felossfy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：实时翻译-同传与语音字幕 解锁免费时长++
# 软件版本：3.2.6
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## felossfy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >实时翻译-同传与语音字幕
^https?:\/\/(translator|accounts).felo.me\/api\/user.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/felossfy.js
^https?:\/\/translator.felo.me\/api\/translation\/thirdToken\?token_channel url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/felossfy.js
#[mitm] 
# hostname = accounts.felo.me,translator.felo.me,52.78.154.178

## felotk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：felo实时翻译-同传与语音字幕
# 软件版本：3.2.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## felotk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >felo实时翻译-同传与语音字幕
^https?:\/\/(translator|accounts).felo.me\/api\/user.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/felo.js
^https?:\/\/translator.felo.me\/api\/translation\/thirdToken\?token_channel url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/felotk.js
#[mitm] 
# hostname = accounts.felo.me,translator.felo.me

## fengxingshipin.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：风行视频 解锁影视会员
# 软件版本：4.3.6.9
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## fengxingshipin.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 风行视频 解锁影视会员
^http:\/\/pvip.funshion.com\/v1\/vip\/userinfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/fengxingshipin.js
^http:\/\/.+funshion.com\/.+\/(media\/play|vip\/playauth) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/fengxingshipintk.js

#[mitm] 
# hostname = pvip.funshion.com,pm.funshion.com,*.funshion.com

## fengxingshipintk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：风行视频 解锁影视会员
# 软件版本：4.3.6.9
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## fengxingshipintk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 风行视频 解锁影视会员
^http:\/\/pvip.funshion.com\/v1\/vip\/userinfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/fengxingshipin.js
^http:\/\/.+funshion.com\/.+\/(media\/play|vip\/playauth) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/fengxingshipintk.js

#[mitm] 
# hostname = pvip.funshion.com,pm.funshion.com,*.funshion.com

## ffxj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：方弗相机（恢复购买）
# 软件版本：1.7.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ffxj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >方弗相机（恢复购买）
^https?:\/\/api.revenuecat.com\/v1\/(receipts|subscribers\/\$RCAnonymousID) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ffxj.js

#[mitm] 
# hostname = api.revenuecat.com

## filmix.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：filmix-ai影视库重命名整理刮削
# 软件版本：2.6
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## filmix.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > filmix-ai影视库重命名整理刮削
^https:\/\/appv3\.filmix\.com\.cn\/api\/v2\/user\/user\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/filmix.js

#[mitm]
# hostname = appv3.filmix.com.cn

## fkbdc.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：疯狂背单词-英语学习常备软件
# 软件版本：2.5
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## fkbdc.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/api.fengkuangapp.com\/account\/grant\/detail\/info\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/fkbdc.js

#[mitm]
# hostname = api.fengkuangapp.com

## flo.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：女性日记flo-大姨妈助手、排卵期安全期日历
# 软件版本：9.59.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## flo.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >女性日记flo-大姨妈助手、排卵期安全期日历
^https?:\/\/api.owhealth.com\/subscriptions\/v1\/apple.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/flo.js

#[mitm] 
# hostname = api.owhealth.com

## fqdj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：番茄短剧
# 软件版本：3.0.4
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## fqdj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >番茄短剧
^https?:\/\/fqapi.tiantiangf.cn\/center-(biz|admin)\/app-api\/v1\/(users\/me|skitsLists\/frontCheck) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/fqdj.js
^https?://fqapi\.tiantiangf\.cn/center-system/app-api/v1/banner/list url reject-dict

#[mitm]
# hostname = fqapi.tiantiangf.cn

## fqzm.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：番茄助眠  (随意账号密码登录即是24h会员到期重新登)
# 软件版本：TF 1.0
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## fqzm.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 番茄助眠
^https:\/\/www\.asmr\.red\/api\/v1\/login-new url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/fqzm.js


#[mitm]
# hostname = www.asmr.red

## fqzmtoken.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## fqzmtoken.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## fqzmtoken1.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## fqzmtoken1.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## fyrj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：翻译软件-实时语音翻译中英互译（永久会员）
# 软件版本：2.1.2
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## fyrj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >翻译软件-实时语音翻译中英互译（永久会员）
^https?:\/\/appss.rhinoxlab.com\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/fyrj.js

#[mitm] 
# hostname = appss.rhinoxlab.com

## fysmw.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：风云扫描王-文件转pdf、证件扫描、文档管理和ocr文字识别 （永久会员）
# 软件版本：1.4.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## fysmw.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 风云扫描王-文件转pdf、证件扫描、文档管理和ocr文字识别 （永久会员）
^https?:\/\/xieyi.cfcpe.com.cn\/api\/client\/info.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/fysmw.js

#[mitm] 
# hostname = xieyi.cfcpe.com.cn

## game4play.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：解锁会员
# 软件版本：++
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## game4play.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# >玛卡相机-ai照片视频写真一键秒出潮流大片
# >玛卡写真馆-一键生成热门写真、换发型、海量潮流变装
# >蜜桃小剧场-海量热门短剧无限看
# >简易翻译-实时拍照语音文字翻译神器
^https?:\/\/.+xdplt.com\/api\/v1\/(user\/info|payment\/apple\/pay\/verify) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/game4play.js

#[mitm] 
# hostname = *xdplt.com

## gdscds.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：高定素材大师（永久会员）
# 软件版本：1.0.3
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## gdscds.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >高定素材大师（永久会员）需要登录！
^https?:\/\/apps-api.lingege.cn\/user\/getUserInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/gdscds.js

#[mitm] 
# hostname = apps-api.lingege.cn

## getbiji.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：get笔记-你只管说-ai-帮你记下来
# 软件版本：2.3.4
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## getbiji.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > get笔记-你只管说-ai-帮你记下来
^https:\/\/get-notes\.luojilab\.com\/voicenotes\/app\/v1\/user url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/getbiji.js

#[mitm]
# hostname = get-notes.luojilab.com

## ghyw.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：稿好语文-硬核学习备考
# 软件版本：2.10.3
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ghyw.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 稿好语文-硬核学习备考
^https:\/\/app\.gaohaozuowen\.com\/index\.php\?s=\/api\/index\/getUserinfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ghyw.js

#[mitm]
# hostname = app.gaohaozuowen.com

## gps.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：gps工具箱
# 软件版本：2.7.38
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## gps.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > gps工具箱
^^https:\/\/service\.gpstool\.com\/app\/index\/getUserInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/gps.js

#[mitm]
# hostname = service.gpstool.com

## gszhgj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：格式转换工具（永久会员）
# 软件版本：1.1.7
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## gszhgj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >格式转换工具（永久会员）
^https?:\/\/appss.rhinoxlab.com\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/gszhgj.js

#[mitm] 
# hostname = appss.rhinoxlab.com

## hbxs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：话本小说（去广告+无限阅读权）
# 软件版本：6.52.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## hbxs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >话本小说（去广告+无限阅读权）
^https?:\/\/(api|user).ihuaben.com\/(user-ext|api\/userinfo\?tokenId).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/hbxs.js

#[mitm] 
# hostname = user.ihuaben.com,api.ihuaben.com

## hbxt.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：汇播学堂 会员+付费课
# 软件版本：2.2.4
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## hbxt.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 汇播学堂 会员+付费课 （不显示会员！直接看付费栏目）
^https?:\/\/(huibo.hcbnjy.com\/api\/huibo-gzh\/(course\/getCourseItem\?id|my\/getUserInfoResult)|app.666syh.com\/cloud\/api-mall\/huibo\/learnDetail).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/hbxt.js

#[mitm] 
# hostname = app.666syh.com,huibo.hcbnjy.com

## hdkk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：红豆看看-热门抢先看海量资源
# 软件版本：1.5
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## hdkk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 红豆看看-热门抢先看海量资源
^https?:\/\/api.jingyuanw.top\/api\/user\/fetchUser url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/hdkk.js

#[mitm]
# hostname = api.jingyuanw.top

## hdmh.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：画眈漫画VIP
# 软件版本：1.1.2
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## hdmh.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >画眈漫画
^https?:\/\/www.shutiaomh.com\/api\/\/getSysUserInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/hdmh.js
^https?:\/\/www.shutiaomh.com\/api\/\/(novel|cartoon)Chapter\/info\/size\/ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/hdmh.js
# >去广告
^https?://mobads.baidu.com url reject-dict
^https?://bgg.baidu.com url reject-dict
^https?://sf3-fe-tos.pglstatp-toutiao.com url reject-dict
^https?://cmyd-10-88.getui.com url reject-dict
^https?://open.e.kuaishou.com url reject-dict
^https?://api-access.pangolin-sdk-toutiao.com url reject-dict
^https?://toblog.ctobsnssdk.com url reject-dict
^https?://log-api.pangolin-sdk-toutiao.com url reject-dict
^https?://api-access.pangolin-sdk-toutiao1.com url reject-dict
^https?://log-api.pangolin-sdk-toutiao-b.com url reject-dict
^https?://api-access.pangolin-sdk-toutiao1.com url reject-dict
^https?://log-api.pangolin-sdk-toutiao.com url reject-dict
^https?://api-access.pangolin-sdk-toutiao1.com url reject-dict
^https?://log-api.pangolin-sdk-toutiao-b.com url reject-dict
^https?://api-access.pangolin-sdk-toutiao-b.com url reject-dict
^https?://b-gy.getui.net url reject-dict
^https?://gy.gepush.com url reject-dict
^https?://mssdk.volces.com url reject-dict
^https?://sdk.open.talk.getui.net url reject-dict
^https?://gromore.pangolin-sdk-toutiao.com url reject-dict
^https?://sdk.open.talk.gepush.com url reject-dict
^https?://adservice.sigmob.cn url reject-dict
^https?://sdk.open.talk.getui.com url reject-dict
^https?://yun.tuitiger.com url reject-dict
^https?://mobads-pre-config.cdn.bcebos.com url reject-dict
^https?://mobads-logs.baidu.com url reject-dict



#[mitm]
# hostname = www.shutiaomh.com,mobads.baidu.com,bgg.baidu.com,sf3-fe-tos.pglstatp-toutiao.com,cmyd-10-88.getui.com,open.e.kuaishou.com,api-access.pangolin-sdk-toutiao.com,toblog.ctobsnssdk.com,log-api.pangolin-sdk-toutiao.com,api-access.pangolin-sdk-toutiao1.com,log-api.pangolin-sdk-toutiao-b.com,api-access.pangolin-sdk-toutiao-b.com,b-gy.getui.net,gy.gepush.com,mssdk.volces.com,sdk.open.talk.getui.net,gromore.pangolin-sdk-toutiao.com,sdk.open.talk.gepush.com,adservice.sigmob.cn,sdk.open.talk.getui.com,yun.tuitiger.com,mobads-pre-config.cdn.bcebos.com,mobads-logs.baidu.com

## hdts.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：忙豆听书 解锁付费听书购买下载⏬
# 软件版本：
# 下载地址：
# 脚本作者：**
# 更新时间：2024-4.24
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## hdts.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 忙豆听书 解锁付费听书购买
^https?:\/\/md.liaoliaoy.com\/listen\/api\/getUserInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/hdts.js
^https?:\/\/md.liaoliaoy.com\/listen\/api\/(buyBookInit|buyCard|batchBuyChapter|bookChapter|getNextChapter).*$ url script-request-body https://raw.githubusercontent.com/WeiGiegie/666/main/hdtsck.js
# > 去广告
^https?://case-cdn\.oceanplayable\.com url reject
^https?://ios\.bugly\.qq\.com url reject
^https?://ulogs\.um.*\.com url reject
^https?://token\.safebrowsing\.apple url reject
^https?://toblog\.ctobsnssdk\.com url reject

#[mitm] 
# hostname = md.liaoliaoy.com,www.yueliangfm.com,ios.bugly.qq.com,ulogs.umeng.com,ulogs.umengcloud.com,*toutiao.com,toblog.ctobsnssdk.com,token.safebrowsing.apple,toblog.ctobsnssdk.com

## hdtsck.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：忙豆听书 解锁付费听书购买下载⏬
# 软件版本：
# 下载地址：
# 脚本作者：**
# 更新时间：2024-4.24
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## hdtsck.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 忙豆听书 解锁付费听书购买
^https?:\/\/md.liaoliaoy.com\/listen\/api\/getUserInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/hdts.js
^https?:\/\/md.liaoliaoy.com\/listen\/api\/(buyBookInit|buyCard|batchBuyChapter|bookChapter|getNextChapter).*$ url script-request-body https://raw.githubusercontent.com/WeiGiegie/666/main/hdtsck.js
# > 去广告
^https?://case-cdn\.oceanplayable\.com url reject
^https?://ios\.bugly\.qq\.com url reject
^https?://ulogs\.um.*\.com url reject
^https?://token\.safebrowsing\.apple url reject
^https?://toblog\.ctobsnssdk\.com url reject

#[mitm] 
# hostname = md.liaoliaoy.com,www.yueliangfm.com,ios.bugly.qq.com,ulogs.umeng.com,ulogs.umengcloud.com,*toutiao.com,toblog.ctobsnssdk.com,token.safebrowsing.apple,toblog.ctobsnssdk.com

## hffkt.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：好芳法课堂
# 软件版本：2.5.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## hffkt.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >好芳法课堂
^https?:\/\/wfjava.wfkids.net url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/hffkt.js


#[mitm]
# hostname = talkshow.wfkids.net,wfjava.wfkids.net

## hhmh.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：嘿嘿漫画
# 软件版本：2.0.6
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## hhmh.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 嘿嘿漫画 付费章节、会员章节购买
^https?:\/\/api.manxinao.com\/api\/User\/getBalance url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/hhmh.js
^https?:\/\/api.manxinao.com\/api\/Commodity\/(buychapter|chadetail) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/hhmh.js

#[mitm]
# hostname = api.manxinao.com

## himalaya.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：喜马拉雅国际版VIP音频解析 + 课程授权解锁 + 页面广告清理
# 软件版本：通用
# 脚本作者：
# 更新时间：2025年09月28日
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## himalaya.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 喜马拉雅国际版VIP音频解析 + 课程授权解锁 + 页面广告清理
^https?:\/\/.+himalaya\.com\/(himalaya-portal\/(track\/urlEncode|v1\/course)|courses\/\d+) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/himalaya.js

#[mitm]
# hostname = api.himalaya.com, *.himalaya.com, apisg.himalaya.com, www.himalaya.com

## himalayaapp.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：喜马拉雅国际版(himalaya) 会员调试
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## himalayaapp.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 喜马拉雅国际版(himalaya) 会员调试

#节目目录解锁
^https?:\/\/.+himalaya.com\/imobile\/album\/v2\/track\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/himalayaapps.js
^https?:\/\/.+himalaya.com\/imobile-track\/track\/urlEncode url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/himalayaapps.js
^https?:\/\/.+himalaya.com\/imobile-track\/track\/detail url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/himalayaapps.js
^https?:\/\/.+himalaya.com\/hipoints-web\/member\/userInfo\/get url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/himalayaapps.js
^https?:\/\/.+himalaya.com\/intl-promotion-web\/play\/redeem\/simple\/config url reject

#[mitm]
# hostname = apisg.himalaya.com,api.himalaya.com

## himalayaapps.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## himalayaapps.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## himalayack.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## himalayack.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## himalayaui.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：himalaya vip
# 软件版本：h5
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## himalayaui.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > himalaya vip 
^https?:\/\/api.himalaya.com\/himalaya-portal\/profile\/get\/current\/user\/channels url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/himalayack.js
^https?:\/\/api.himalaya.com\/himalaya-portal\/v2\/vip\/premium\/ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/himalayack.js
^https?:\/\/apisg.himalaya.com\/himalaya-portal\/(track\/urlEncode\?trackId|v1\/course\?courseId) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/himalayack.js 
^https?:\/\/www\.himalaya\.com\/courses\/\d+ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/himalayaui.js

#[mitm]
# hostname = www.himalaya.com,apisg.himalaya.com,183.232.157.73:13037,cdn.himalaya.com:443,*.himalaya.*,api.himalaya.com

## himalayaviptab.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## himalayaviptab.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## hlmx.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：哈梨冥想-heartly lab冥想减压安眠白噪音
# 软件版本：1.3.9
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## hlmx.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >哈梨冥想-heartly lab冥想减压安眠白噪音
^https?:\/\/svc.heartlylab.com\/api\/v1\/(app\/user\/apple_login|user\/vip\/info) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/hlmx.js


#[mitm]
# hostname = svc.heartlylab.com

## hongxingshipin.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：红杏视频🔞 解锁VIP
# 软件版本：h5
# 下载地址：
# 脚本作者：
# 更新时间：2026年0117
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## hongxingshipin.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >红杏视频🔞 解锁VIP
^https?:\/\/.+.com\/(addons\/dplayer|vod\/play\/id)\/ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/hongxingshipin.js
^https://.+com/ad/wapad.js url reject

#[mitm]
# hostname = hxxx51.com,www.hxxxaa.com,hxxx52.com,app.hxxxaa.com

## hqfyg.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：环球翻译官-拍照实时语音文字翻译器（永久会员）
# 软件版本：1.0.8
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## hqfyg.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >环球翻译官-拍照实时语音文字翻译器（永久会员）
^https?:\/\/hqfyg.5exg.cn\/translation\/getuserinfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/hqfyg.js

#[mitm] 
# hostname = hqfyg.5exg.cn

## huanxiu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：幻休（永久会员）
# 软件版本：2.8.86
# 下载地址：https://apps.apple.com/cn/app/%E5%B9%BB%E4%BC%91%E7%9D%A1%E7%9C%A0-%E4%B8%93%E6%B3%A8%E7%9D%A1%E7%9C%A0%E7%9B%91%E6%B5%8B-%E6%B7%B1%E5%BA%A6%E7%9D%A1%E7%9C%A0%E5%88%86%E6%9E%90-%E7%99%BD%E5%99%AA%E9%9F%B3%E5%8A%A9%E7%9C%A0/id1595991642
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## huanxiu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >幻休（永久会员）
^https?:\/\/api.shaolinzen.com\/user/v1\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/huanxiu.js

#[mitm] 
# hostname = api.shaolinzen.com

## hundun.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：解锁vip课程,付费课程无限观看！
# 软件版本：++
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月7日 18:39
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！作者TG频道 : https://t.me/GieGie777
# *******************************

## hundun.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 混沌 解锁vip课程,付费课程
^https?:\/\/((course)|(user)|(capi)).hundun.cn\/.*(course_detail|user_info|member_home_page).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/hundun.js

#[mitm] 
# hostname = *hundun.cn

## iTranscreen.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：itranscreen-屏幕翻译、游戏、漫画和视频实时翻译
# 软件版本：3.2.4
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## iTranscreen.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >itranscreen-屏幕翻译
^https?:\/\/.+.(itranscreen|tencentcs).+\/(settings|api\/v1\/user\/quota\?user_id).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/iTranscreen.js


#[mitm]
# hostname = *.tencentcs.com,settings.itranscreen.com,api.itranscreen.com:8080

## jbt.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：剪播台
# 软件版本：1.3.0
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月25日 20:39 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jbt.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 剪播台vip
^https?:\/\/api-chn.jianbotaiapi.com\/api\/rest\/commerce\/integrate\/vip\/perform.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jbt.js

#[mitm] 
# hostname = api-chn.jianbotaiapi.com

## jdxj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：精点相机
# 软件版本：1.6.6
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jdxj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 精点相机
^https:\/\/counting\.movingshop\.cn\/counting-api\/user\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jdxj.js

#[mitm]
# hostname = counting.movingshop.cn

## jhxs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：江湖小说
# 软件版本：2.4.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年3月16日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jhxs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 江湖小说vip
^https?:\/\/freeapp-api.zhangdu.com\/api\/(user\/info|book\/enter-book-after).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jhxs.js

#[mitm] 
# hostname = freeapp-api.zhangdu.com,121.40.246.38,121.41.30.120

## jiaomujun.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：酵母菌
# 软件版本：1.6.8
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jiaomujun.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >
^http:\/\/116\.62\.172\.2(?::80)?\/(user\/(getUserInfoDetail|owndetailForPost)|website\/v1\/config)\.do url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jiaomujun.js

#[mitm]
# hostname = 

## jinli.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：锦鲤聚合 解锁会员视频+
# 软件版本：++
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jinli.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >18+ 锦鲤聚合 会员+
^http:\/\/h5.jinliapp.top\/api.php\/(user\/index|video\/detail\?id|platform|login\/login).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jinli.js
^http://h5\.jinliapp\.top/api\.php/adv/video url reject-200
#[mitm] 
# hostname = h5.jinliapp.top

## jjhl.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：极简汇率 - 让货币流通更简单（永久会员）
# 软件版本：6.0.3
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jjhl.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >极简汇率 - 让货币流通更简单（永久会员）
^https?:\/\/explorer.tratao.com\/api\/client\/xtool\/vip url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jjhl.js

#[mitm] 
# hostname = explorer.tratao.com

## jjjbs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：挤挤挤巴士（微信小程序游戏）
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jjjbs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >挤挤挤巴士（微信小程序游戏）无限体力，道具
^https?:\/\/simplestorage.landintheair.com\/newstorage\/.+\/user-info-data url script-request-body https://raw.githubusercontent.com/WeiGiegie/666/main/jjjbs.js

#[mitm] 
# hostname = simplestorage.landintheair.com

## jjwxr.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：狙击外星人 (微信小程序) 
# 软件版本：解锁金币，道具数量，每次打开注册新账户强登理论无视封号
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jjwxr.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/alien.kampfiregames.cn\/game\/entry\/platform-login url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jjwxr.js

#[mitm]
# hostname = alien.kampfiregames.cn

## jlpt.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：识日jlpt-最新jlpt历年真题全覆盖
# 软件版本：1.1.0
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jlpt.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 识日jlpt-最新jlpt历年真题全覆盖
^http:\/\/jlpt\.jingqueyun\.com\/user\/getUserInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jlpt.js

#[mitm]
# hostname = jlpt.jingqueyun.com

## jmshe.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：姬漫社,解锁=视频,漫画,写真,动漫。 无需登录！
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jmshe.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 姬漫社,解锁=视频,漫画,写真,动漫。 无需登录！
https?:\/\/oxtoon.(cyou|work|monster|net)\/((detail|comic)\/\d|buychapter).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/jmshe.js

#[mitm]
# hostname = oxtoon.cyou,oxtoon.work,oxtoon.monster,oxtoon.net

## jpfyg.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：金牌翻译官 
# 软件版本：1.3.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jpfyg.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >金牌翻译官 恢复购买
^https?:\/\/mv-ps.xdplt.com\/api\/v1\/(user|payment\/apple\/pay\/verify)\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jpfyg.js


#[mitm]
# hostname = mv-ps.xdplt.com

## jpfygloon.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：金牌翻译官 
# 软件版本：1.3.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jpfygloon.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >金牌翻译官 恢复购买
^https?:\/\/mv-ps.xdplt.com\/api\/v1\/(user|payment\/apple\/pay\/verify)\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jpfygloon.js


#[mitm]
# hostname = mv-ps.xdplt.com

## jrsypzxj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：今日水印拍照相机 Pro
# 软件版本：1.7
# 下载地址：
# 脚本作者：**
# 更新时间：2024-2.13
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## jrsypzxj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 今日水印拍照相机 Pro
^https?:\/\/water-fst.yifanxinmei.com\/api\/guests\/login.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jrsypzxj.js

#[mitm] 
# hostname = water-fst.yifanxinmei.com

## jtsp.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：加藤视频vip解锁,付费视频,下载,会员线路,去广告+
# 软件版本：3.5.3
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## jtsp.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 加藤视频vip解锁,付费视频解锁,下载,会员线路,去广告++
^https?:\/\/.+\/shorter\/(video\/longvideoinfo|resource\/adInfoPageList\?adSpaceId|user\/getUser|resource\/getWindowNotice|tabIconConfigView|user\/getVipPvg|video\/home|video\/search).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jtsp.js
# > 解锁漫画
^https?://.+\/shorter\/anime\/tbookchapter0\/info\?bookId url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/jtsp.js

# > 去广告
^https?://.+/shorter/resource/getAdInfoBySpaceId\?ad url reject
^https://.+/shorter/resource/bySpaceId\?adSpaceId url reject
^https://.+/shorter/game/moreList url reject

#[mitm] 
# hostname = zqlhcn.kl9117.xyz,hkg2zx.stejnu.xyz,zvqldx.xv8gvn.xyz,kuhikr.1t3vwf.xyz,s4788x.taqndh.xyz,xkewutdekf873sr.chairsr.com,103.85.254.233,omfnmh.x2b3w6.xyz,zqlhcn.kl9117.xyz,hkg2zx.stejnu.xyz,zvqldx.xv8gvn.xyz,kuhikr.1t3vwf.xyz,s4788x.taqndh.xyz,xkewutdekf873sr.chairsr.com,103.85.254.233,omfnmh.x2b3w6.xyz,sgwitxgwit231sr.xunwxc.com,zqlhcn.kl9117.xyz,ssgwew231s.uzqew4qi.info,ykofye.qklmoq.xyz,pmmqls.gregdoro.com,103.85.254.233,omfnmh.x2b3w6.xyz,zqlhcn.kl9117.xyz,lieeys42jdi2kd.cx4c5mv7.info,fjboni.jn6588.xyz,rylzit.wqws5z.xyz,riwnkx.sw05uv.xyz,pmmqls.gregdoro.com,103.85.254.233,omfnmh.x2b3w6.xyz,zqlhcn.kl9117.xyz,lieeys42jdi2kd.cx4c5mv7.info,fjboni.jn6588.xyz,rylzit.wqws5z.xyz,riwnkx.sw05uv.xyz,pmmqls.gregdoro.com,103.85.254.233,omfnmh.x2b3w6.xyz,zqlhcn.kl9117.xyz,rylzit.wqws5z.xyz,riwnkx.sw05uv.xyz,bsxqtd.xk8q7w.xyz,grvgdq.81fbg2.xyz,pmmqls.gregdoro.com,103.85.254.233,omfnmh.x2b3w6.xyz,zqlhcn.kl9117.xyz,rylzit.wqws5z.xyz,riwnkx.sw05uv.xyz,bsxqtd.xk8q7w.xyz,grvgdq.81fbg2.xyz,pmmqls.gregdoro.com,103.85.254.233,omfnmh.x2b3w6.xyz,rdxijq.vo9ugl.xyz,bpitbu.ouxf9g.live,tjactb.xk2nsy.live,us6tp7.dkr9cv.live,qsukip.9jts57.live,wkqyx6.qdokqj.live,fqohbx.bkljtx.live,zihjyv.jajrc1.live,y7uyqg.hl8dbl.live,qrdure.youngnoble.cn,kpoe2e.x8w7t3.live,*jttv*,kpoe2e.x8w7t3.live,zihjyv.jajrc1.live,fceyeg.johjxe.com,*.vo9ugl.xyz,zqlhcn.kl9117.xyz,bsxqtd.xk8q7w.xyz,grvgdq.81fbg2.xyz,vcryyw.uc86fy.xyz,xospby.mldo9k.xyz,pmmqls.gregdoro.com,103.85.254.233,roa6fi.lyr6if.live,gp1wpm.xcg3zr.xyz,nfmq0v.lknlqz.xyz,nhppjkb.z5x6pzr.xyz, fwroeor.revxcvx.xyz,uetuys234ls.pbog5txn.app,fwroeor.revxcvx.xyz,hrp27mr.1ugq1uf.xyz,nhppjkb.z5x6pzr.xyz,zd7uwox.8ewxvhz.xyz,xkewutdekf873sr.chairsr.com,5tlsw6r.7n26ytu.xyz,uetuys234ls.pbog5txn.app,5jsd6q7.jnfkdtm.xyz,yw4pk9h.7fw2snb.xyz,m3pkeg2.v4l0p7y.xyz,ip6vu33.im0684h.xyz,xkewutdekf873sr.chairsr.com,bor4lch.8l6ldmz.xyz,uetuys234ls.pbog5txn.app,5jsd6q7.jnfkdtm.xyz,yw4pk9h.7fw2snb.xyz,m3pkeg2.v4l0p7y.xyz,ip6vu33.im0684h.xyz,xkewutdekf873sr.chairsr.com,bor4lch.8l6ldmz.xyz,uetuys234ls.pbog5txn.app,58z8gxu.pzmn7ku.xyz,bor4lch.8l6ldmz.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,58z8gxu.pzmn7ku.xyz,bor4lch.8l6ldmz.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,e6xun6l.wy6z0uq.xyz,ldyc3rs.0mlptkl.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,7x16kek.nwjnf0m.xyz,2h4q7lu.hjsn53p.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,0ncf2i3.wl8td88.xyz,4dhuyek.dusgrec.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,9sdec0e.mls08ph.xyz,5c8gqqf.to1ogvz.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,nt7474f.u8kgc14.xyz,85tug87.nkwyzcp.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,85tug87.nkwyzcp.xyz,n05ggob.batlnhh.xyz,eg8oppq.19fc5lg.xyz,xkewutdekf873sr.chairsr.com

## jttk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：加藤视频vip解锁,付费视频,下载,会员线路,去广告+
# 软件版本：3.1.3
# 下载地址：
# 脚本作者：**
# 更新时间：2024-5.27
# 电报频道：https://t.me/GieGie777
# 问题反馈：ps：小部分视频有bug无法播放
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## jttk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 加藤视频vip解锁,付费视频解锁,下载,会员线路,去广告++
^https?:\/\/.+\/shorter\/(video\/longvideoinfo|resource\/adInfoPageList\?adSpaceId|user\/getUser|resource\/getWindowNotice|tabIconConfigView|user\/getVipPvg|video\/home\/list|video\/search).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jtsp.js
#^https?:\/\/.+\/shorter\/anime\/tbookchapter0\/info\?bookId url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/jttk.js
^https?:\/\/.+\/shorter\/resource\/getAdInfoBySpaceId\?ad url reject-dict

#[mitm] 
# hostname = sgwitxgwit231sr.xunwxc.com,rdxijq.vo9ugl.xyz,bpitbu.ouxf9g.live,tjactb.xk2nsy.live,us6tp7.dkr9cv.live,qsukip.9jts57.live,wkqyx6.qdokqj.live,fqohbx.bkljtx.live,zihjyv.jajrc1.live,y7uyqg.hl8dbl.live,qrdure.youngnoble.cn,kpoe2e.x8w7t3.live,*jttv*,kpoe2e.x8w7t3.live,zihjyv.jajrc1.live,fceyeg.johjxe.com,*.vo9ugl.xyz,zqlhcn.kl9117.xyz,bsxqtd.xk8q7w.xyz,grvgdq.81fbg2.xyz,vcryyw.uc86fy.xyz,xospby.mldo9k.xyz,pmmqls.gregdoro.com,103.85.254.233,roa6fi.lyr6if.live

## judu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：句读
# 软件版本：4.3.0
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年3月1日 20:39 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## judu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 句读 vip
^^https?:\/\/judouapp.com\/api\/v2\/mine\/profile url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/judu.js

#[mitm] 
# hostname = judouapp.com

## junwang.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：恢复购买会员
# 软件版本：++++
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## junwang.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >新鲜壁纸-最美炫酷动图&游戏二次元主题（恢复购买）
# >水印大师-搬运素材&ai抠图&物体擦除（恢复购买）
# >资源搬运大师-短视频&图片（恢复购买）
# >水印熊-视频提取&一键水印&下载助手（恢复购买）
# >萌客ai绘画-贴纸&动漫卡通（恢复购买）
^https?:\/\/api.revenuecat.com\/v1\/receipts url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/junwang.js

#[mitm] 
# hostname = api.revenuecat.com

## jyh.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：鸡乐盒
# 软件版本：1.1.14
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jyh.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 鸡乐盒
^http:\/\/www\.mhzcookbook\.xyz\/app url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jyh.js

#[mitm]
# hostname = www.mhzcookbook.xyz

## jzj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：节奏酱
# 软件版本：4.50.2
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月24日 20:21  
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jzj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 节奏酱vip
^https?:\/\/medi.dxzzywxk.top\/api\/rest\/commerce\/integrate\/vip\/perform.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jzj.js

#[mitm] 
# hostname = medi.dxzzywxk.top

## jzlx.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：
# 软件版本：
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## jzlx.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > vip
^https?:\/\/quiz.istarl.com\/api\/index.php\?app_frontend_v2&init&opr=one&code url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jzlx.js

#[mitm] 
# hostname = quiz.istarl.com

## kada.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：kada阅读-儿童绘本故事启蒙大全
# 软件版本：8.28.0
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## kada.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# >kada阅读-儿童绘本故事启蒙大全
^https:\/\/service\.hhdd\.com\/book2 url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/kada.js

#[mitm] 
# hostname = service.hhdd.com

## kaokaoya.js 从第1行到 [rewrite_local] 前一行：
# 
# /*
#  *
#  *
# 脚本功能：考考鸭
# 软件版本：1.04
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## kaokaoya.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 考考鸭
^https?:\/\/kky.logtk.com\/userLogin url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/kaokaoya.js

#[mitm]
# hostname = kky.logtk.com

## keep.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：keep 课程预览 直播课。会员付费课跟练 会员训练计划
# 软件版本：
# 下载地址：8.7.20
# 脚本作者：
# 更新时间：2026+
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## keep.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >keep 课程预览 直播课。会员付费课跟练 会员训练计划
^https?:\/\/(api|kit).gotokeep\.com\/(nuocha|gerudo|athena|nuocha\/plans|suit\/v5\/smart|kprime\/v4\/suit\/sales)\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/keep.js



#[mitm]
# hostname = api.gotokeep.com,kit.gotokeep.com,*.gotokeep.*,120.53.74.*,162.14.5.*,42.187.199.*,101.42.124.*

## kgr.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：凯格尔运动pro-pc肌-盆底肌修复锻炼
# 软件版本：1.5
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## kgr.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >
^https?:\/\/ioskgeyd.lmkaiger.sanwubeixin.cn\/ct\/user\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/kgr.js

#[mitm]
# hostname = ioskgeyd.lmkaiger.sanwubeixin.cn

## kjwsp.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：看鉴微视频-纪录片和短视频社区 
# 软件版本：9.4.9
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## kjwsp.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >看鉴微视频-纪录片和短视频社区
^https?:\/\/api.wokanjian.com.cn\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/kjwsp.js

#[mitm]
# hostname = api.wokanjian.com.cn

## kkxt.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：可可修图（永久会员）
# 软件版本：1.3.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## kkxt.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >可可修图（永久会员）
^https?:\/\/cocamapi.imendon.com\/v2\/(purchase\/(vip\/verification|receipt\/verify)|user\/profile) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/kkxt.js

#[mitm] 
# hostname = cocamapi.imendon.com

## klbsq.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：快聊变声器（永久会员）
# 软件版本：++
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## klbsq.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >快聊变声器（永久会员）
^https?:\/\/voiceapi.jimetec.com\/user\/myInfo\?sysOs url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/klbsq.js

#[mitm] 
# hostname = voiceapi.jimetec.com

## kttx.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：卡通头像-漫画头像制作（永久会员）
# 软件版本：1.0.4
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## kttx.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >卡通头像-漫画头像制作（永久会员）
^https?:\/\/appss.rhinoxlab.com\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/kttx.js

#[mitm] 
# hostname = appss.rhinoxlab.com

## kuaikanmanhua.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：快看漫画 解锁VIP,去广告+提前看
# 软件版本：7.90.0
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## kuaikanmanhua.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >快看漫画 解锁VIP,去广告+提前看
^https?:\/\/.*kkmh.com\/v1\/vip\/me url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/kuaikanmanhua.js
^https?:\/\/.*kkmh.com\/v2\/comic\/detail\/get url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/kuaikanmanhua.js
# >去广告
^https?://.*kkmh.com\/v3\/ad\/show url reject-dict

#[mitm]
# hostname = *.kkmh.com,*kkmh.com

## kuaiy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：快影
# 软件版本：++
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月28日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## kuaiy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 快影 解锁Vip、付费、Ai玩法
^https?:\/\/(id\.kwaiying|api\.kmovie\.gifshow)\.com.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/kuaiy.js
^https://api\.kmovie\.gifshow\.com/rest/n/kmovie/app/merchant/ad/bidding url reject
^https://mi\.gdt\.qq\.com/gdt_mview\.fcg url reject
#[mitm]
# hostname = api.kmovie.gifshow.com,id.kwaiying.com,mi.gdt.qq.com

## kuqueshuiyin.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：酷雀水印管家-图片加水印-一键去消除马赛克-试卷擦除图片还原
# 软件版本：1.8.1
# 脚本功能：酷雀水印相机-真实时间地点经纬度拍照打卡-工作水印打卡相机
# 软件版本：1.1.17
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## kuqueshuiyin.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >酷雀水印管家+酷雀水印相机 会员
^https?:\/\/front-gw.kuque.com\/productAuthorizeService\/user\/auth\/query\/allAuthSimple url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/kuqueshuiyin.js


#[mitm]
# hostname = front-gw.kuque.com

## kuwo.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：酷我音乐 会员调试  
# 软件版本：11.3.8
# 下载地址：
# 脚本作者：引用转发脚本请带出处频道地址，谢谢。
# 更新时间：2025年1105
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## kuwo.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 酷我音乐 会员调试  
# > 酷我音乐 播放器解锁
^.*?(ad(.*)?\.tencentmusic\.com|kuwo\.cn\/((EcomResource|(Mobile)?Ad)Serv(er|ice)|(vip|(open)?api)?\/(setting\/sidebar\/menus|v\d\/(operate\/((adVip\/)?text|pop\/info)|online\/homepage\/homepageBanner|user\/freeMode|album\/adBar|app\/(newMenuList\/menuListInfo|pasterAdvert\/config)))|kuwopay\/vip-tab\/page\/floatbox|commercia\/eproxy\/boot\/recommend\/fee\/config)) url reject-200
^https?:\/\/(?:musicpay|nmobi|vip1|audiobookpay|tingshu|anymatch|mobilebasedata|rich|mobilead|wapi)\.kuwo\.cn\/(?:music\.pay\?newver=\d+|mobi.s\?f=kwxs|vip\/(?:enc\/user\/vip\?op=ui&uid=|v2\/theme\?op=gd)|a\.p|v2\/api\/pay\/user\/info|api\/music\/info|AdService\/kaiping\/adinfo|EcomResourceServer\/(?:adEarnGuajian\/adinfo|getMotor\.do)|openapi\/v1\/(?:operate\/homePage|user\/freeMode\/h5\/text|operate\/message\/info|user\/dolby\/text)|commercia\/(?:vipTab\/myTab\/base|userAssets\/downloadCoupon\/reduce|recall\/pop\/jxhGearInfo|vip\/(?:dressCenter\/getResourceList|player\/getStyleListByModel))|api\/v2\/pay\/app\/getConfigInfo(?:\?configType)?|vip\/v2\/sysinfo|commercia\/recall\/pop\/jxhMultiGearInfo) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/kuwo.js
^https?:\/\/vip1\.kuwo\.cn\/commercia\/vip\/player\/getStyleListByModel url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/kuwo.js

#[mitm]
# hostname = *.kuwo.cn, abt-kuwo.tencentmusic.com,ad.tencentmusic.com:443

## kuwobata1.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：酷我音乐 会员调试  
# 软件版本：11.3.8
# 下载地址：
# 脚本作者：引用转发脚本请带出处频道地址，谢谢。
# 更新时间：2025年1012
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## kuwobata1.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 酷我音乐 会员调试  
^https?:\/\/(?:musicpay|nmobi|vip1|audiobookpay|tingshu|anymatch|mobilebasedata|rich|mobilead|wapi)\.kuwo\.cn\/(?:music\.pay\?newver=\d+|mobi.s\?f=kwxs|vip\/(?:enc\/user\/vip\?op=ui&uid=|v2\/theme\?op=gd)|a\.p|v2\/api\/pay\/user\/info|api\/music\/info|AdService\/kaiping\/adinfo|EcomResourceServer\/(?:adEarnGuajian\/adinfo|getMotor\.do)|openapi\/v1\/(?:operate\/homePage|user\/freeMode\/h5\/text|operate\/message\/info|user\/dolby\/text)|commercia\/(?:vipTab\/myTab\/base|userAssets\/downloadCoupon\/reduce|recall\/pop\/jxhGearInfo|vip\/(?:dressCenter\/getResourceList|player\/getStyleListByModel))|api\/v2\/pay\/app\/getConfigInfo(?:\?configType)?|vip\/v2\/sysinfo|commercia\/recall\/pop\/jxhMultiGearInfo) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/kuwobata1.js
# > 酷我音乐 播放器解锁
^https?:\/\/vip1\.kuwo\.cn\/commercia\/vip\/player\/getStyleListByModel url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/kuwobata1.js
# >底部弹窗
^https?:\/\/h5s.kuwo.cn\/pay\/vip_recall_popup\/ url reject
#[mitm]
# hostname = h5s.kuwo.cn,anymatch.kuwo.cn,musicpay.kuwo.cn,mobilebasedata.kuwo.cn,vip1.kuwo.cn,rich.kuwo.cn,mobilead.kuwo.cn,*.kuwo.cn

## kyxq.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：口语星球
# 软件版本：4.5.8
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## kyxq.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# >口语星球
^https?:\/\/mapi\.kouyuxingqiu\.com\/api\/v2 url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/kyxq.js

#[mitm] 
# hostname = mapi.kouyuxingqiu.com

## lailian.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## lailian.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## leshi.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：乐视视频 vip
# 软件版本：10.9.28
# 下载地址：
# 脚本作者：https://t.me/GieGie777
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## leshi.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 乐视视频
^http:\/\/.+app.m.le.com\/android\/dynamic.php\?mod\=passport url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/leshi.js
^http:\/\/.+app.m.le.com\/play\?AD_IPDX url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/leshi.js

#[mitm]
# hostname = 

## leshi1.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：乐视视频 vip
# 软件版本：10.9.28
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## leshi1.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 乐视视频
^http:\/\/dynamic.user.app.m.le.com\/android\/dynamic.php\?mod\=passport url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/leshi1.js
^http:\/\/api.mob.app.m.le.com\/play\?AD_IPDX url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/leshi1.js

#[mitm]
# hostname = 

## linecamera.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：line camera - 照片编辑器
# 软件版本：15.7.4
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## linecamera.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >line camera - 照片编辑器
^https?:\/\/linecamera-api.snow.me\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/linecamera.js
#[mitm] 
# hostname = linecamera-api.snow.me

## lispod.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：lispod分级播客-无痛英语听力自由
# 软件版本：1.4.3
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## lispod.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# >lispod分级播客-无痛英语听力自由
^https:\/\/api\.lispod\.cn\/api\/User\/CurrentUser url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/lispod.js

#[mitm] 
# hostname = api.lispod.cn

## listenleap.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：listenleap-帮你听懂英语播客-通过播客学英语
# 软件版本：1.5.1
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## listenleap.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 帮你听懂英语播客-通过播客学英语
^https?://.+listenleap\.com/front-api/user/getUserInfoByUserToken url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/listenleap.js

#[mitm]
# hostname = www.listenleap.com,api.listenleap.com

## lldad.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## lldad.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## lrts.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：懒人听书 解锁vip
# 软件版本：8.6.02
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2025年0310
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## lrts.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 懒人听书vip
^https?:\/\/(shapi|gzapi).(mting.info|lanrentingshu.com)\/(yyting\/userclient\/ClientGetUserInfo|lrts\/cgi\/login\/accountLogin) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/lrts.js

#[mitm] 
# hostname = shapi.mting.info,shapi.lanrentingshu.com,gzapi.lanrentingshu.com,shapi.mting.info:443

## lryyyd.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：懒人英语阅读-英语文章-英文原版小说-外刊阅读
# 软件版本：1.3.8
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## lryyyd.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 懒人英语阅读-英语文章-英文原版小说-外刊阅读的学英语软件 
^https?:\/\/duoting.tatatimes.com\/tataeraapi\/api.s\?h=QueryVipUser url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/lryyyd.js

#[mitm] 
# hostname = duoting.tatatimes.com

## lsnl.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：历史年轮
# 软件版本：1.3.0
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## lsnl.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 历史年轮
^https:\/\/www\.(yearsmap|wordvector)\.com url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/lsnl.js

#[mitm]
# hostname = www.yearsmap.com:443,www.yearsmap.com,www.wordvector.com

## luolidao.js 从第1行到 [rewrite_local] 前一行：
# 
# /*
#  *
#  *
# 脚本功能：🔞萝莉岛+|||🔞吸游技+
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年+
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## luolidao.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >🔞萝莉岛+|||🔞吸游技+
^https?:\/\/.+/api\/setapp.php url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/lldad.js
^https?:\/\/.+\/api\/(userinfo|setapp|newstartad|newreg|Get_vod_list|dongtais).php url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/luolidao.js
#[mitm]
# hostname = *.cloudfront.net,dag29jmgma1g.site,*site,d1no17eycutrsi.cloudfront.net,lldajbfyobffkbg288-api.com,lasdasdgapi.cc,las23gsapi.cc

## luping.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：录屏-屏幕录制神器（永久会员）
# 软件版本：1.1.3
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## luping.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >录屏-屏幕录制神器（永久会员）
^https?:\/\/sc.szsszykj.com\/api\/GetIosVip.php url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/luping.js

#[mitm] 
# hostname = sc.szsszykj.com

## lxcs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：脸型测试-测脸型配发型小助手（永久会员）
# 软件版本：1.4.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## lxcs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >脸型测试-测脸型配发型小助手（永久会员）
^https?:\/\/app.faxingwu.com\/index.php\/(face\/(user\/checkVip|login\/getUserInfo2)|v1\/mt\/appfacebookdesign).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/lxcs.js

#[mitm] 
# hostname = app.faxingwu.com

## manhuadaquan.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：漫画大全-高清漫画阅读
# 软件版本：6.0.2
# 下载地址：美区App Store
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## manhuadaquan.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >
http:\/\/server.007manhua.com\/daquan2_rest.action\?(type=(mine5&userId|restore)|manhua) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/manhuadaquan.js



#[mitm]
# hostname = 

## mankezhan.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：漫客宅,会员漫画小说解锁
# 软件版本：++
# 下载地址：
# 脚本作者：
# 更新时间：2025年8月23日 12:21
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mankezhan.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 漫客宅,会员解锁
^https?:\/\/(member|comic|base).mkzcdn.com\/(user|chapter\/buy\/add|vote\/add|gift\/play\/add|advert\/app\/(read|user|task)).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mankezhan.js
^https?:\/\/(comic|story).mkzcdn.com\/chapter\/(content\/v1|read\/check).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/mkztk.js
#[mitm] 
# hostname = member.mkzcdn.com,comic.mkzcdn.com,base.mkzcdn.com

## manxiong.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：漫熊（美区商店下载）
# 软件版本：6.5
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## manxiong.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >漫熊（美区商店下载）
^https?:\/\/cb.uvogin.xyz\/(auth\/getUserByUserid|comic\/getComicDetialByid) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/manxiong.js

#[mitm] 
# hostname = cb.uvogin.xyz

## mbfm.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：面包fm 解锁vip，付费讲书
# 软件版本：3.3.7
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月17日 22:39
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mbfm.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 面包fm 解锁vip，付费讲书
^https?:\/\/wx.voxpie.com\/api\/(fmapp_bookinfo|fmapp_user).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mbfm.js

#[mitm] 
# hostname = wx.voxpie.com

## mbpt.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：面包拼图（永久会员）
# 软件版本：1.0.9
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mbpt.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >面包拼图（永久会员）
^https?:\/\/bread28resapi.wscreativity.com\/v2\/(purchase\/vip\/verification|user\/profile) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mbpt.js

#[mitm] 
# hostname = bread28resapi.wscreativity.com

## mdtv.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：美队tv（永久会员+去广告）18+
# 软件版本：++
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mdtv.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >美队tv（永久会员+去广告）18+
^https?:\/\/api.+.com\/api\/video\/(user/info|banner\?type|config\?field) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mdtv.js

#[mitm] 
# hostname = api.736136.com

## meiwu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：魅.舞直播++ 解锁付费房
# 软件版本：
# 下载地址：
# 脚本作者：**
# 更新时间：2024-
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## meiwu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >魅.舞直播++
^https?:\/\/.+(member\/detail|user_video_async|lottery-ns) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/meiwu.js

#[mitm] 
# hostname = qq.baiduan*.com,*wensan*com,ws.wensangq.com,45.117.11.*

## memo.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：便签小组件-备忘录笔记本手机widget vip 
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2024年.4.28
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## memo.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 便签小组件-备忘录笔记本手机widget vip 
^https?:\/\/www.40sishi.com\/memo\/user\/profile url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/memo.js

#[mitm] 
# hostname = www.40sishi.com

## mgsp.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：vip
# 软件版本：6.5.30
# 下载地址：
# 脚本作者：
# 更新时间：2024-8.16
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## mgsp.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 咪咕视频Vip会员
^https?:\/\/(play|dis).*miguvideo.com\/(play|dis)(url|play)\/.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mgsp.js
^https?:\/\/play.miguvideo.com\/playurl\/v1\/play\/playurl\?2Kvivid=true?.*=true$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/mgspck.js
# > 开屏广告
^https://.*miguvideo\.com/request/sdk url reject-200
^https://common-sc\.miguvideo\.com/task/v7/task-list/cmvideo/visitor url reject-200

#[mitm] 
# hostname = *.miguvideo.com

## mgspck.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：
# 软件版本：6.5.30
# 下载地址：
# 脚本作者：**
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## mgspck.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 咪咕视频Vip会员
^https?:\/\/(play|dis).*miguvideo.com\/(play|dis)(url|play)\/.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mgsp.js
^https?:\/\/play.miguvideo.com\/playurl\/v1\/play\/playurl\?2Kvivid=true?.*=true$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/mgspck.js
# > 开屏广告
^https://.*miguvideo\.com/request/sdk url reject-200
^https://.*miguvideo\.com/task/v7/task-list/cmvideo/visitor url reject-200

#[mitm] 
# hostname = *.miguvideo.com

## mgtk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：
# 软件版本：++
# 下载地址：
# 脚本作者:
# 更新时间：2025+
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## mgtk.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## mgtv.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：Vip+会员画质+去广告,页面优化
# 软件版本：++
# 下载地址：苹果商店下载
# 脚本作者:
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## mgtv.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 芒果TV,Vip+会员画质+去广告,页面优化
^https?:\/\/.*mgtv.com.*(GetUserInfo|module\/list\?_support|app\/bdboot|pullReleaseInfo|floor\/detail|video\/source|entry\?_support).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mgtv.js
^https?:\/\/mobile-stream\.api\.mgtv\.com\/v1\/video\/source\? url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/mgtk.js
^https?://mobile.da.mgtv.com\/m\/page url reject

#[mitm] 
# hostname = *mgtv.com,hb-boom.api.mgtv.com,*.api.mgtv.com,mobile-stream.api.mgtv.com,hb-boom.api.mgtv.com,mobile-stream.api.mgtv.com,nuc.api.mgtv.com,111.31.104.108, 111.31.104.*, 111.31.104.96, *.api.mgtv.com, iphone.v0.mgtv.com, api.data.mgtv.com, *.*.mgtv.*,

## mhlz.js 从第1行到 [rewrite_local] 前一行：
# /*
# 脚本功能：魔幻粒子 - 游戏货币修改
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mhlz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/ss\.landintheair\.com\/storage\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mhlz.js

#[mitm]
# hostname = ss.landintheair.com

## mhy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：漫画园
# 软件版本：1.7.2
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mhy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >漫画园
^http:\/\/aimanhua.meidanmh.com\/chunai\/web\/(member\/memberInfo\/getEntity|cartoon\/cartoonChapter\/needVip) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mhy.js


#[mitm]
# hostname = 

## migu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：咪咕音乐
# 软件版本：7.41.5
# 下载地址：
# 脚本作者：
# 更新时间：2024年3月18日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## migu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 咪咕音乐vip、音质、下载
^https?:\/\/(u|c|app).(musicapp|c.nf).migu.cn.*(user\/(queryUserInfo.do|member\/center))|column\/(startup-pic-with-ad|marketing\/advertising).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/jhxs.js
^https?:\/\/app.(c|pd).nf.migu.cn\/.*\/(listen-url|music\/batchQueryMusicPolicy|download-url).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/migu.js
^https://app\.c\.nf\.migu\.cn/member/api/marketing/text url reject
^https://app\.c.nf\.migu\.cn/payment/watch-ad url reject-200
#[mitm] 
# hostname = *.migu.cn

## miguyinyue.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：咪咕音乐
# 软件版本：7.44.8
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## miguyinyue.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 咪咕音乐vip、音质、下载
^https?:\/\/(u|c|app).(musicapp|c.nf).migu.cn.*(user\/(queryUserInfo.do|member\/center))|(column\/(startup-pic-with-ad|marketing\/advertising)|\/resource\/skin\/list\/).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/miguyinyue.js
^https?:\/\/app.(c|pd).nf.migu.cn\/.*\/(listen-url|music\/batchQueryMusicPolicy|download-url).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/miguyinyue.js
^https://app\.c\.nf\.migu\.cn/member/api/marketing/text url reject
^https://app\.c.nf\.migu\.cn/payment/watch-ad url reject-200
#[mitm] 
# hostname = *.migu.cn

## mimu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：你推陪你背单词练口语
# 软件版本：2.2.0
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mimu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 你推陪你背单词练口语
^https:\/\/api\.mimuapp\.com\/v2\/fm\/userInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mimu.js

#[mitm]
# hostname = api.mimuapp.com

## mjgsad.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：解锁会员视频、漫画、小说、有声书、图片、部分广告
# 软件版本：h5&轻量版
# 下载地址：
# 脚本作者：@ios151
# 更新时间：2024+
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## mjgsad.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >18+面具公社🚗
^https?:\/\/.+\.com\/(playerinfo|api\/get_advert|api\/home\/popup) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mjgsad.js
^https?:\/\/.+\.com\/request_game\/v3\/game\/list url reject-dict
^https?:\/\/.+\.com\/api\/(comic|video|short_movie_info|novel|gallery|audio) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/mjgshd.js


#[mitm]
# hostname = gnjk.*.com, mfjk.*.com,buyaoda.*.com,gnjk*.com

## mjgshd.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：会员视频、小说、漫画
# 软件版本：h5&轻量版
# 下载地址：
# 脚本作者：@ios151
# 更新时间：2024+
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## mjgshd.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >18+面具公社
^https:\/\/.+\.com\/(playerinfo|api\/get_advert|api\/home\/popup) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mjgsad.js
^https:\/\/.+\.com\/request_game\/v3\/game\/list url reject-dict
^https?:\/\/.+\.com\/api\/(comic|video|short_movie_info|novel) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/mjgshd.js


#[mitm]
# hostname = gnjk.*.com, mfjk.*.com,gnjk*.com

## mkztk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：漫客宅,会员漫画小说解锁
# 软件版本：++
# 下载地址：
# 脚本作者：
# 更新时间：2025年9月4日 09:08
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mkztk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 漫客宅,会员解锁
^https?:\/\/(member|comic|base).mkzcdn.com\/(user|chapter\/buy\/add|vote\/add|gift\/play\/add|advert\/app\/(read|user|task)).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mankezhan.js
^https?:\/\/(comic|story).mkzcdn.com\/chapter\/(content\/v1|read\/check).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/mkztk.js
#[mitm] 
# hostname = member.mkzcdn.com,comic.mkzcdn.com,base.mkzcdn.com

## mlkp.js 从第1行到 [rewrite_local] 前一行：
# 
# /*
#  *
#  *
# 脚本功能：麻溜快跑 无限金币+钻石
# 软件版本：微信小程序
# 下载地址：
# 脚本作者：
# 更新时间：2026
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mlkp.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 麻溜快跑 无限金币+钻石
^https?:\/\/udl.lesejie.com\/api\/game\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mlkp.js

#[mitm]
# hostname = udl.lesejie.com

## motu.js 从第1行到 [rewrite_local] 前一行：
# 
# 
# /*
#  *
#  *
# 脚本功能：摩途-摩托车骑行导航路书轨迹地图
# 软件版本：4.1.0
# 下载地址：
# 脚本作者：
# 更新时间：2026年01月25日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## motu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 摩途-摩托车骑行导航路书轨迹地图
^https:\/\/motu\.motumap\.com\/v3\/user\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/motu.js

#[mitm] 
# hostname = motu.motumap.com

## mozhai.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：墨摘-金句摘抄阅读 永久会员
# 软件版本：1.2.7
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mozhai.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >墨摘-金句摘抄阅读 永久会员
^https?:\/\/api.juzikong.com\/mobi\/mooz\/am\/me\/meInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mozhai.js
^https?:\/\/api.juzikong.com\/mobi\/mooz\/am\/(explore\/getTagCateConfig|music\/getBgmList) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/mozhaitk.js

#[mitm]
# hostname = api.juzikong.com

## mozhaitk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：墨摘-金句摘抄阅读 永久会员
# 软件版本：1.2.7
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mozhaitk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >墨摘-金句摘抄阅读 永久会员
^https?:\/\/api.juzikong.com\/mobi\/mooz\/am\/me\/meInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mozhai.js
^https?:\/\/api.juzikong.com\/mobi\/mooz\/am\/(explore\/getTagCateConfig|music\/getBgmList) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/mozhaitk.js

#[mitm]
# hostname = api.juzikong.com

## mp3zh.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：mp3转换助手-视频提取音频编辑制作手机铃声
# 软件版本：1.7.6
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mp3zh.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >mp3转换助手-视频提取音频编辑制作手机铃声 VIP
^https?:\/\/audio.xinmengma.com\/api\/user url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mp3zh.js

#[mitm]
# hostname = audio.xinmengma.com

## mpy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：萌配音-实时游戏语音包变声器软件（永久会员）
# 软件版本：9.7
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mpy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >萌配音-实时游戏语音包变声器软件（永久会员）
^https?:\/\/lc.i7play.com\/1.1\/classes\/_User\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mpy.js

#[mitm] 
# hostname = lc.i7play.com

## mrdtbz.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：每日动态壁纸-动态高清手机壁纸
# 软件版本：2.3.8
# 下载地址：+
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mrdtbz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >每日动态壁纸-动态高清手机壁纸
^https?:\/\/standard.rhinoxlab.com\/standard\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mrdtbz.js
#[mitm] 
# hostname = standard.rhinoxlab.com

## mrsyxj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：每日水印相机-时间地点打卡、工程记录、团队打卡（永久会员）
# 软件版本：3.0.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mrsyxj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >每日水印相机-时间地点打卡、工程记录、团队打卡（永久会员）
^https?:\/\/appss.rhinoxlab.com\/\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mrsyxj.js

#[mitm] 
# hostname = appss.rhinoxlab.com

## msdj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：漫森短剧
# 软件版本：1.0.10
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## msdj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >漫森短剧
^https?:\/\/man.api.mansenwenhua77.com\/app\/(my-home\/user\/info|login\/apple) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/msdj.js


#[mitm]
# hostname = man.api.mansenwenhua77.com

## mtddgs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：每天读点故事-原生、奇崛、动人（解锁音频节目）
# 软件版本：6.8.4
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mtddgs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >每天读点故事（解锁音频节目）
^https?:\/\/apipc01.gushi.cn\/(v9\/articles|audio|collection\/audios|v3\/app\/config\/check|users\/vipinfos|v6\/chapters) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mtddgs.js
^https?:\/\/apipc01.gushi.cn\/v5\/advertisements\? url reject-dict
#[mitm] 
# hostname = apipc01.gushi.cn

## muogushipin.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：🔞蘑菇视频
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## muogushipin.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 🔞蘑菇视频
^https?:\/\/.+\/api\/(ads|user\/info) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/muogushipin.js
^https?:\/\/.+\/api\/vod\/vip\/ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/muogushipin.js

#[mitm] 
# hostname = 118.31.70.61,wapi.koudailc.net

## mybsq.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：魔音变声器-聊天变声软件（永久会员）
# 软件版本：2.0.8
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mybsq.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >魔音变声器-聊天变声软件（永久会员）
^https?:\/\/appss.rhinoxlab.com\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mybsq.js

#[mitm] 
# hostname = appss.rhinoxlab.com

## mydj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：蜜柚短剧-触手可及的短剧世界
# 软件版本：1.0.7
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mydj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 蜜柚短剧
^https?:\/\/api.duoyolo.cn\/play\/app\/user\/v1\/userInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mydj.js
# >去广告
^https?://mobads.baidu.com url reject-dict
^https?://bgg.baidu.com url reject-dict
^https?://sf3-fe-tos.pglstatp-toutiao.com url reject-dict
^https?://cmyd-10-88.getui.com url reject-dict
^https?://open.e.kuaishou.com url reject-dict
^https?://api-access.pangolin-sdk-toutiao.com url reject-dict
^https?://toblog.ctobsnssdk.com url reject-dict
^https?://log-api.pangolin-sdk-toutiao.com url reject-dict
^https?://api-access.pangolin-sdk-toutiao1.com url reject-dict
^https?://log-api.pangolin-sdk-toutiao-b.com url reject-dict
^https?://api-access.pangolin-sdk-toutiao1.com url reject-dict
^https?://log-api.pangolin-sdk-toutiao.com url reject-dict
^https?://api-access.pangolin-sdk-toutiao1.com url reject-dict
^https?://log-api.pangolin-sdk-toutiao-b.com url reject-dict
^https?://api-access.pangolin-sdk-toutiao-b.com url reject-dict
^https?://b-gy.getui.net url reject-dict
^https?://gy.gepush.com url reject-dict
^https?://mssdk.volces.com url reject-dict
^https?://sdk.open.talk.getui.net url reject-dict
^https?://gromore.pangolin-sdk-toutiao.com url reject-dict
^https?://sdk.open.talk.gepush.com url reject-dict
^https?://adservice.sigmob.cn url reject-dict
^https?://sdk.open.talk.getui.com url reject-dict
^https?://yun.tuitiger.com url reject-dict
^https?://mobads-pre-config.cdn.bcebos.com url reject-dict
^https?://mobads-logs.baidu.com url reject-dict
^https?:\/\/api.duoyolo.cn\/play\/channel\/v1\/adevertiseConfig url reject
#[mitm]
# hostname = api.duoyolo.cn,ioskgeyd.lmkaiger.sanwubeixin.cn,www.shutiaomh.com,mobads.baidu.com,bgg.baidu.com,sf3-fe-tos.pglstatp-toutiao.com,cmyd-10-88.getui.com,open.e.kuaishou.com,api-access.pangolin-sdk-toutiao.com,toblog.ctobsnssdk.com,log-api.pangolin-sdk-toutiao.com,api-access.pangolin-sdk-toutiao1.com,log-api.pangolin-sdk-toutiao-b.com,api-access.pangolin-sdk-toutiao-b.com,b-gy.getui.net,gy.gepush.com,mssdk.volces.com,sdk.open.talk.getui.net,gromore.pangolin-sdk-toutiao.com,sdk.open.talk.gepush.com,adservice.sigmob.cn,sdk.open.talk.getui.com,yun.tuitiger.com,mobads-pre-config.cdn.bcebos.com,mobads-logs.baidu.com

## myt.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：（漫芽糖指绘1.26.11） + （漫芽糖简笔画1.13.5）（永久会员）
# 软件版本：++
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## myt.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >（漫芽糖指绘1.26.11） + （漫芽糖简笔画1.13.5）（永久会员）
^https?:\/\/(sketch|paint).manyatang.cn.+\/func\/login url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/myt.js

#[mitm] 
# hostname = sketch.manyatang.cn,paint.manyatang.cn

## mzxj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：美妆相机 - 上新多款睫毛（永久会员）
# 软件版本：6.6.6
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## mzxj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >美妆相机 （永久会员）
^https?:\/\/api.(revenuecat|account.meitu).com\/(v1\/subscribers|v1\/receipts|users\/show_current.json) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/mzxj.js

#[mitm] 
# hostname = api.revenuecat.com,api.account.meitu.com

## namibox.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：纳米盒-中小学课本同步辅导提分必备
# 软件版本：13.4
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## namibox.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# >纳米盒-中小学课本同步辅导提分必备
^https:\/\/namibox\.com url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/namibox.js

#[mitm] 
# hostname = namibox.com

## nanguadianying.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：南瓜电影🎬
# 软件版本：9.5.2
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## nanguadianying.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 南瓜电影🎬
^https?:\/\/open-secret-api-v2.vcinema.cn\/v.+\/(user\/v2|get_overdue_device_menu_v2) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/nanguadianying.js

#[mitm]
# hostname = open-secret-api-v2.vcinema.cn,47.93.95.61

## nc.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：捏词-自担陪你背四六级考研
# 软件版本：1.2.0
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## nc.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 捏词-自担陪你背四六级考研
^https:\/\/ainieci\.com\/test\/user\/getUserInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/nc.js

#[mitm]
# hostname = ainieci.com

## niceday.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：niceday-冥想,助眠故事,减轻压力,白噪音,心情日记
# 软件版本：3.9.16
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## niceday.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >niceday-冥想,助眠故事,减轻压力,白噪音,心情日记
^https?:\/\/niceday.youloft.com\/api\/appuser\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/niceday.js

#[mitm] 
# hostname = niceday.youloft.com

## njk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：牛健康-ai营养师减肥私教-体重身材管理好搭档
# 软件版本：4.11.0
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## njk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https:\/\/gw\.qnniu\.com\/api\/servlets\?endpoint=\/member_centers url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/njk.js

#[mitm]
# hostname = gw.qnniu.com

## nmxjc.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：柠檬小剧场 Unlock VIP剧
# 软件版本：1.3.3
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## nmxjc.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >柠檬小剧场 Unlock VIP剧
^https?:\/\/(videohub.xdplt|mv-ps.xdplt).com\/api\/v1\/(short\/play|user\/info|payment\/apple\/pay\/verify\/info) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/nmxjc.js


#[mitm]
# hostname = mv-ps.xdplt.com,videohub.xdplt.com

## nndj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：牛牛短剧-短剧官方-海量热门短剧视频大全随心看
# 软件版本：1.5.8
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## nndj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 牛牛短剧-短剧官方-海量热门短剧视频大全随心看
^https?:\/\/new.tianjinzhitongdaohe.com\/api\/v1\/app\/user\/visitorInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/nndj.js

#[mitm]
# hostname = new.tianjinzhitongdaohe.com

## nnjk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：拿捏驾考-新生代绝绝子驾考（解锁会员）
# 软件版本：1.1.7
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## nnjk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >拿捏驾考（解锁会员）
^https?:\/\/nanie.flyingeffect.com\/api\/me\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/nnjk.js

#[mitm] 
# hostname = nanie.flyingeffect.com

## nxfl.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## nxfl.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## nyai.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：暖悦-ai大姨妈记录月经期助手
# 软件版本：1.7.1
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## nyai.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 暖悦-ai大姨妈记录月经期助手
^https?:\/\/gs.gateway.gameley.cn\/np-womens-user\/user\/info\/edit url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/nyai.js

#[mitm]
# hostname = gs.gateway.gameley.cn

## panda.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：百度网盘、在线画质清晰度、体验解锁下载速度
# 软件版本：13.10.5
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## panda.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 百度网盘、在线画质清晰度、体验解锁下载速度
^https?:\/\/pan\.baidu\.com.+(rest\/.+\/membership\/user|api\/user\/getinfo)\? url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/panda.js
;^https?:\/\/.+(pcs\.baidu\.com|baidu\.com)\/rest\/2\.0\/pcs\/file\?.*\bfid= url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/panda.js

#[mitm] 
# hostname = pan.baidu.com, *.pcs.baidu.com, *.baidu.com

## pdf.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：pdf转换助手-智能pdf处理编辑软件（永久会员）
# 软件版本：++
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## pdf.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >pdf转换助手-智能pdf处理编辑软件（永久会员）
^https?:\/\/pdfnew-api.shoujicad.com.+\/api\/user url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/pdf.js

#[mitm] 
# hostname = 39.107.159.85

## peiyinshi.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：配音师-文字转语音助手
# 软件版本：4.1.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## peiyinshi.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >配音师-文字转语音助手 Unlock VIP+配音课
^http:\/\/(dubbing.csweimei.cn\/course\/GetCourseInfo?|music.dreamyin.cn\/Svip\/SVIP_Existence.aspx) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/peiyinshi.js


#[mitm]
# hostname = 

## picma.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：picma - ai照片修复上色，找回你我当年回忆
# 软件版本：2.8.8
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## picma.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >picma - ai照片修复上色，找回你我当年回忆
^https?:\/\/api.magictiger.ai\/api\/v1\/uservip\/getMemberInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/picma.js

#[mitm] 
# hostname = api.magictiger.ai

## piggame.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：微信小程序 猪了个猪 金币修改9999
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年818
# 电报频道：https://t.me/GieGie777
# 问题反馈：<<需要通关新手关卡第一关，然后重进小程序即可>>
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## piggame.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >微信小程序 猪了个猪 金币修改9999
^https?:\/\/api.flysheeep.com\/api\/expro\/get_pig_game_info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/piggame.js

#[mitm]
# hostname = api.flysheeep.com

## pika.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：咔皮action-复古-dv（永久会员）
# 软件版本：3.0.6
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## pika.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >咔皮action-复古-dv（永久会员）
^https?:\/\/kapi-action.shyuhuankj.com\/user url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/pika.js

#[mitm] 
# hostname = kapi-action.shyuhuankj.com

## pixart.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：pixart ai photo editor（永久会员）
# 软件版本：1.1.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## pixart.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >pixart ai photo editor（永久会员）
^https?:\/\/api.qonversion.io\/v1\/user\/init url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/pixart.js

#[mitm] 
# hostname = api.qonversion.io

## pozhu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：破竹SVIP+财富圈+部分付费课程
# 软件版本：2.3.6
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月28日 18:39
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## pozhu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 破竹SVIP+财富圈+部分付费直播
^https?:\/\/www\.pozhu\.com\/(v1|v2)\/(package\/playinfo\?|notes\/index|notes\/notes|notes\/audio|notes\/video|package\/subart|package\/artlist2|video\/show|college\/chapter2|member\/userinfo|home\/startimg|deep\/course|notes\/secret_lives).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/pozhu.js

#[mitm] 
# hostname = www.pozhu.com

## ppybqb.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：皮皮鸭表情包（永久会员）
# 软件版本：1.1.7
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ppybqb.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >皮皮鸭表情包（永久会员）
^https?:\/\/114.116.104.132\/app\/main\/account\/index url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ppybqb.js

#[mitm] 
# hostname = 114.116.104.132

## ptds.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# "p图大神 – ai换脸变装秀"
# Appstore版本 : 2.0.7
# "拼图照片-图片拼接修图神器，图片排版合成裁剪编辑大师"
# Appstore版本 : 1.0.3
# 下载地址：++
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ptds.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >p图大神 – ai换脸变装秀 & 拼图照片-图片拼接修图神器，图片排版合成裁剪编辑大师
^https?:\/\/appss.linhongshi.com\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ptds.js
#[mitm] 
# hostname = appss.linhongshi.com

## py.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：配音-变音变声器配音软件（永久会员）
# 软件版本：1.6
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## py.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >配音-变音变声器配音软件（永久会员）
^https?:\/\/xihongshi.dicallapp.com\/SDGetMeCount.ashx url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/py.js

#[mitm] 
# hostname = xihongshi.dicallapp.com

## pyjzp.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：片羽集照片日记软件
# 软件版本：1.6.31
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## pyjzp.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 片羽集照片日记软件
^https:\/\/lentoapp\.com:8091\/getUserMemberShipInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/pyjzp.js

#[mitm]
# hostname = lentoapp.com:8091

## pzst.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：拍照搜题｜小自律 vip 
# 软件版本：1.0.6
# 下载地址：
# 脚本作者：
# 更新时间：2024.4.5
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## pzst.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 拍照搜题 vip 
^https?:\/\/appss.rhinoxlab.com\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/pzst.js
# > 开屏广告
^https://ad\.rhinox\.cn/client/sdk/getData url reject
#[mitm] 
# hostname = ad.rhinox.cn,appss.rhinoxlab.com

## pzsw.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：拍照识物-生活拍照小百科 vip 
# 软件版本：1.1.5
# 下载地址：
# 脚本作者：
# 更新时间：2024.4.10
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## pzsw.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 拍照识物 vip 
^https?:\/\/standard.rhinoxlab.com\/standard\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/pzsw.js
# > 开屏广告
^https://ad\.rhinox\.cn/client/sdk/getData url reject
#[mitm] 
# hostname = ad.rhinox.cn,standard.rhinoxlab.com

## qbxs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：全本小说&淘小说 解锁vip，
# 软件版本：2.3.1,2.8.2
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月18日 06:55
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qbxs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 全本小说&淘小说 解锁vip，
^https?:\/\/t(f|y)book.taoyuewenhua.net\/(auth\/account\?adid|auth\/task_account_ios.do\?|wx_register.do\?|auth\/try_wx_bind\?).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qbxs.js
^https://thirdwx\.qlogo\.cn/mmopen/.*$ url 302 https://zdimg.lifeweek.com.cn/app/20230410/16810960185662892.jpg
#[mitm] 
# hostname = tfbook.taoyuewenhua.net,tybook.taoyuewenhua.net

## qbyy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：千变语言（永久会员）
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qbyy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >千变语言（永久会员）
^https?:\/\/www.helloshiyu.com\/voice\/user\/profile url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qbyy.js

#[mitm] 
# hostname = www.helloshiyu.com

## qddsvip.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：起点读书 解锁会员免费听书板块
# 软件版本：5.9.364
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qddsvip.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >起点读书 解锁会员免费听书板块
^https?:\/\/.+.qidian.com\/argus\/api\/v[0-9]\/(adv\/getadvlistbatch|user\/getaccountpage|subscription\/getvipprice|client\/getsplashscreen) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qddsvip.js
^https?:\/\/.+.qidian.com\/argus\/api\/v[0-9]\/(audio\/getTtsChapterAudio|bookcontent\/getvipcontent|audio\/getaudiochapter\?) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/qddsvip.js

#[mitm] 
# hostname = *.qidian.com,*.if.qidian.com

## qieting.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## qieting.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## qifu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：起伏-睡眠、冥想与白噪音
# 软件版本：4.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qifu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >起伏-睡眠、冥想与白噪音
^https?:\/\/api.risingfalling.com\/api\/(vip\/detail|memberInfo\/myInfo) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qifu.js


#[mitm]
# hostname = api.risingfalling.com

## qingtu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：轻图（永久会员）
# 软件版本：5.0.20
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qingtu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >轻图（永久会员）
^https?:\/\/(purchase-qingtu-api.b612kaji|api-qingtu.kajicam).com\/(v1\/purchase\/subscription\/subscriber\/status|xht\/api\/user\/info) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qingtu.js

#[mitm] 
# hostname = purchase-qingtu-api.b612kaji.com,api-qingtu.kajicam.com

## qiyou.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：奇游手游加速器
# 软件版本：3.5.4
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qiyou.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >奇游手游加速器
^https?:\/\/api.qiyou.cn\/api\/common_bll\/.+\/(member\/login_status|members).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qiyou.js
^https?://api\.qiyou\.cn/api/common_bll/v1/client_advertisements url reject
^https?://api\.qiyou\.cn/api/common_bll/adv url reject-dict
#[mitm]
# hostname = api.qiyou.cn,apifast.qiyou.cn

## qjbwl.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：轻记-备忘记录笔记、日记便签和清单待办（永久会员）
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qjbwl.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 轻记-备忘记录笔记、日记便签和清单待办（永久会员）
^https?:\/\/app.yinxiang.com\/third\/memo\/restful\/v1\/membership url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qjbwl.js

#[mitm] 
# hostname = app.yinxiang.com

## qjsm.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：全局扫描-图片转文字&全能扫描仪&拍照识别花草（永久会员）
# 软件版本：1.0.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qjsm.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >全局扫描-图片转文字&全能扫描仪&拍照识别花草（永久会员）
^https?:\/\/sm.szsszykj.com\/api\/user\/vip url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qjsm.js

#[mitm] 
# hostname = sm.szsszykj.com

## qmjyzc.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：全民解压找茬 - 关卡辅助（系统通知显示操作提示 + 延长游戏时间）
# 软件版本：微信小程序
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qmjyzc.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/res5.haotgame.com\/cu03\/static\/OpenDoors\/Res\/data\/levels\/\d+\.json url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qmjyzc.js

#[mitm]
# hostname = res5.haotgame.com

## qmxfs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：课程+会员解锁
# 软件版本：++
# 下载地址：苹果商店下载
# 脚本作者:
# 更新时间：2024年2月4日 10:44
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## qmxfs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 全民幸福舍&课程+会员解锁
^https?:\/\/api\.qmxfs\.com\/v003\/(user\/queryAccountInfo|user\/userInfoNum|content).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qmxfs.js
^https?:\/\/api\.qmxfs\.com\/v003\/subject\/subjectDetail\.json url script-request-body https://raw.githubusercontent.com/WeiGiegie/666/main/qmxfs1.js

#[mitm] 
# hostname = api.qmxfs.com

## qmxfs1.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## qmxfs1.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## qnsmw.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：全能扫描王 vip 
# 软件版本：3.2.7
# 下载地址：
# 脚本作者：
# 更新时间：2024.4.6
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qnsmw.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 全能扫描王 vip 
^https?:\/\/appss.rhinoxlab.com\/\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qnsmw.js
# > 开屏广告
^https://ad\.rhinox\.cn/client/sdk/getData url reject
#[mitm] 
# hostname = ad.rhinox.cn,appss.rhinoxlab.com,appss.rhinoxlab.com

## qpy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：趣配音
# 软件版本：7.36.5
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qpy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 趣配音
^https:\/\/apis40\.qupeiyin\.com\/(course\/related_course|album\/course_list|member\/index|course\/detail_new) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qpy.js

#[mitm]
# hostname = apis40.qupeiyin.com

## qqcx.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：全球潮汐-全球天气预报及离线潮汐
# 软件版本：5.2.0
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qqcx.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 全球潮汐-全球天气预报及离线潮汐
^https:\/\/g\.catches\.com\/v1\/user\?method\=userInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qqcx.js

#[mitm]
# hostname = g.catches.com

## qqtk.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## qqtk.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## qqyd.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：qq阅读会员！
# 软件版本：8.0.81
# 下载地址：
# 脚本作者：**
# 更新时间：2024-2.28
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## qqyd.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > qq阅读会员！
^https?:\/\/(bookcommon|commontgw)\.reader\.qq\.com\/(querymediainfo|.*nativepage\/personal\?lastTime|common\/adV|.*tts(Speaker|Buy)List) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qqyd.js
^https?:\/\/(newminerva-tgw|commontgw|bookcommon|unitelogreport|cartoon)\.reader\.qq\.com.*(audio\/auth\?|ChapBatAuthWithPD\?bookId).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/qqtk.js
#[mitm] 
# hostname = *.reader.qq.com

## qsdj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：去刷短剧
# 软件版本：1.0.3
# 下载地址：
# 脚本作者：
# 更新时间：2026年9月6日
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qsdj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 去刷短剧 vip
^https?:\/\/qs.szqibu.cn\/api\/user\/getUserinfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qsdj.js

#[mitm]
# hostname = qs.szqibu.cn

## qtfm.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：蜻蜓FM 
# 软件版本：10.6.8
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qtfm.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 蜻蜓FM
^https?:\/\/(user|app|entry).qtfm.cn\/(m-bff|api|u2\/api)\/(v1|v5)\/(channel_verify|personal\/\?carrier|user).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qtfm.js
^https?:\/\/app\.qtfm\.cn\/m-bff\/v1\/audiostreams url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/qtfm.js
^https://ad\.qtfm\.cn/api/ url reject
^https://ip\.qtfm\.cn/ip url reject
^https://adlaunch\.qtfm\.cn/launch url reject
^https://woqt2\.qtfm\.cn/v2/userConfig url reject

#[mitm]
# hostname = app.qtfm.cn,user.qtfm.cn,recpage-c.qtfm.cn,entry.qtfm.cn,ad.qtfm.cn,ip.qtfm.cn,adlaunch.qtfm.cn,woqt2.qtfm.cn

## qtfmtk.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## qtfmtk.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## qwrmdj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：全网热门短剧-热门短剧无限看免费电视剧
# 软件版本：1.2
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qwrmdj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >
^https?:\/\/iosqwrmdj.sanwubeixin.cn\/ct\/user\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qwrmdj.js

#[mitm]
# hostname = iosqwrmdj.sanwubeixin.cn

## qys.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：轻音社
# 软件版本：1.1.4
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年3月10日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qys.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 轻音社vip
^https?:\/\/fuciyuanbang.ciyuans.com\/fuciyuanphp\/community\/social url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qys.js

#[mitm] 
# hostname = fuciyuanbang.ciyuans.com

## qysj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：千亦视界 解锁永久vip(重新登录账号即可)
# 软件版本：++
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月10日 12:22
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## qysj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 千亦视界 解锁永久vip
^https?:\/\/app.thwlqy.com\/(login\/login\/sign|login\/login\/veifys).html.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/qysj.js

#[mitm] 
# hostname = app.thwlqy.com

## rbjm.js 从第1行到 [rewrite_local] 前一行：
# 
# /*
#  *
#  *
# 脚本功能：日本🈲️漫 金币解锁🔓
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## rbjm.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >日本🈲️漫 金币解锁🔓
^https?:\/\/javcomics.(store|top)\/(buychapter|chapter\/\d+) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/rbjm.js
^https?:\/\/javcomics.store\/ucenter url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/rbjman.js
#[mitm]
# hostname = javcomics.store

## rbjman.js 从第1行到 [rewrite_local] 前一行：
# 
# /*
#  *
#  *
# 脚本功能：日本🈲️漫 金币解锁🔓
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## rbjman.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >日本🈲️漫 金币解锁🔓
^https?:\/\/javcomics.(store|top)\/(buychapter|ucenter|chapter\/\d+) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/rbjm.js
^https?:\/\/javcomics.store\/ucenter url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/rbjman.js
#[mitm]
# hostname = javcomics.store

## ryqpy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：日语趣配音-零基础学日语
# 软件版本：1.7.5
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ryqpy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 日语趣配音-零基础学日语
^https?:\/\/japaneselar.qupeiyin.com\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ryqpy.js

#[mitm]
# hostname = japaneselar.qupeiyin.com

## rysyxj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：任意水印相机（永久会员）
# 软件版本：1.6.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## rysyxj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >任意水印相机（永久会员）
^https?:\/\/ff.mofangzhice.com\/api\/client\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/rysyxj.js

#[mitm] 
# hostname = ff.mofangzhice.com

## rzxj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：日杂相机&Fomz复古相机 Pro
# 软件版本：日杂相机 1.5.2，Fomz 1.3.3
# 下载地址：
# 脚本作者：**
# 更新时间：2024-2.14
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## rzxj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 日杂相机, Fomz复古相机 Pro
^https?:\/\/(rzres86|fz68res)api.imendon.com\/v2\/(user\/profile|purchase\/vip\/verification).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/rzxj.js

#[mitm] 
# hostname = *api.imendon.com

## sdds.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：十点读书会员,去广告+
# 软件版本：6.11.1
# 下载地址：
# 脚本作者：**
# 更新时间：2024-3.14
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## sdds.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 十点读书会员
^https?:\/\/api.read10.cn\/kt_server\/(audio_book|mine\/index|splash\/start_advertising).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/sdds.js
^https?:\/\/api.read10.cn\/kt_server\/(audio_book\/get_audio_book_catalogue|player\/detail) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/sddstk.js
#[mitm] 
# hostname = api.read10.cn

## sddstk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：十点读书会员,去广告+
# 软件版本：6.11.1
# 下载地址：
# 脚本作者：**
# 更新时间：2024-3.14
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## sddstk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 十点读书会员
^https?:\/\/api.read10.cn\/kt_server\/(audio_book|mine\/index|splash\/start_advertising).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/sdds.js
^https?:\/\/api.read10.cn\/kt_server\/(audio_book\/get_audio_book_catalogue|player\/detail) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/sddstk.js
#[mitm] 
# hostname = api.read10.cn

## sdsy.js 从第1行到 [rewrite_local] 前一行：
# 
# /*
#  *
#  *
# 脚本功能：闪电水印-视频下载提取解析短视频
# 软件版本：4.2.0
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## sdsy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 闪电水印-视频下载提取解析短视频
^https?:\/\/watershandiana.yunxiaoguo.cn\/user\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/sdsy.js

#[mitm]
# hostname = watershandiana.yunxiaoguo.cn

## sesedongman.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：瑟瑟动漫,解锁=视频,无需登录！
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## sesedongman.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 瑟瑟动漫,解锁=视频,无需登录！
^https?:\/\/91acg.(cyou|work|monster|net|website)\/video/\d+.*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/sesedongman.js

#[mitm]
# hostname = 91acg.website,91acg.*

## sgtk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：三更视频 会员+漫画小说解锁
# 软件版本：+++
# 下载地址：
# 脚本作者：
# 更新时间：2024年,6.20
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## sgtk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# >18+ 三更视频 会员+漫画小说解锁
^https?:\/\/api.*.shop\/api\/(search|mov\/(detail|progress|index)|novel\/(chapter|play)|comic\/chapter|clip\/index|comic\/play|user\/index) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/sgtk.js

#[mitm] 
# hostname = api.*.shop

## shenshi.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：绅士-摄影者的天堂
# 软件版本：3.2
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## shenshi.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >绅士-摄影者的天堂
^https?:\/\/gentlemanapp.lingdu2019.cn\/ios_api\/(my_home\/my_detail|portrait\/goods_detail|publish\/yuepai_detail) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/shenshi.js

#[mitm]
# hostname = gentlemanapp*

## shiri.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：识日-背日语-查日语单词-大家的日语
# 软件版本：2.1.5
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## shiri.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 识日-背日语-查日语单词-大家的日语
^http:\/\/shiri\.jingqueyun\.com url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/shiri.js

#[mitm]
# hostname = shiri.jingqueyun.com

## shutiaomanhua.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：薯条漫画-热门耽美bl漫画
# 软件版本：1.3.2
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## shutiaomanhua.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >薯条漫画-热门耽美bl漫画 Unlock VIP
^https?:\/\/www.shutiaoapp.com\/api\/\/getSysUserInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/shutiaomanhua.js
# >去弹窗
^https://www\.shutiaoapp\.com/api//adOpen/getAd url reject

#[mitm]
# hostname = www.shutiaoapp.com

## siyu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：丝域 - model
# 软件版本：1.3
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## siyu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >丝域 - model
^https?:\/\/apimodel.ctwh77.com\/api\/(user4app\/myinfo|item\/detail) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/siyu.js
^https://apidance\.ctwh77\.com/api/appsplash/detail url reject

#[mitm]
# hostname = apimodel.ctwh77.com

## sjjn.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：时间胶囊-写给亲爱未来的日记本
# 软件版本：3.7
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## sjjn.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 时间胶囊-写给亲爱未来的日记本
^https?:\/\/api\.timecaps\.app\/v1\/(snapshots\?since|subscriptions) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/sjjn.js

#[mitm]
# hostname = api.timecaps.app

## sjsm.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：试卷扫描-拍照清除笔迹，还原空白试卷，错题标记重组（永久会员）
# 软件版本：1.1.0 
# 下载地址： 
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## sjsm.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >试卷扫描-拍照清除笔迹，还原空白试卷，错题标记重组（永久会员）
^https?:\/\/appss.baomingding.com\/\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/sjsm.js

#[mitm] 
# hostname = appss.baomingding.com

## sleepcycle.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：sleep cycle: 睡眠监测梦话鼾声记录及智能闹钟
# 软件版本：6.24.25
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## sleepcycle.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >sleep cycle: 睡眠监测梦话鼾声记录及智能闹钟（永久会员）
^https?:\/\/ch.sleepcycle.com\/api\/v1\/subscription/get$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/sleepcycle.js

#[mitm] 
# hostname = ch.sleepcycle.com

## slzd.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：三联中读  (选择设备登录，会员到期后再重新登录即可)
# 软件版本：10.7.3
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## slzd.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 三联中读 知识会员，数字刊会员
^https?:\/\/apis.lifeweek.com.cn\/reg\/vistorRegister.do url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/slzd.js


#[mitm]
# hostname = apis.lifeweek.com.cn,apis.lifeweek.com.cn:443

## smb.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：扫描宝（永久会员）
# 软件版本：6.69
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## smb.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >扫描宝（永久会员）
^https?:\/\/app.yinxiang.com\/third\/scanner\/scanner\/app\/userInfo\/get url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/smb.js

#[mitm] 
# hostname = app.yinxiang.com

## smtw2.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：神庙逃亡2(微信小程序)
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## smtw2.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/temple-release.dskystudio.com\/api\/login url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/smtw2.js

#[mitm]
# hostname = temple-release.dskystudio.com

## smzs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：睡眠助手-专注白噪音冥想助眠（永久会员）
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## smzs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >睡眠助手-专注白噪音冥想助眠（永久会员）
^https?:\/\/www.helloshiyu.com\/sleep\/user\/profile url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/smzs.js

#[mitm] 
# hostname = www.helloshiyu.com

## snbz.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：少女壁纸 - 超高清主题动态壁纸大全（永久会员）
# 软件版本：2.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## snbz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >少女壁纸 - 超高清主题动态壁纸大全（永久会员）
^https?:\/\/www.helloshiyu.com\/mood\/superadmin\/user/profile url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/snbz.js

#[mitm] 
# hostname = www.helloshiyu.com

## sndd.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：少年得到 解锁课程
# 软件版本：++
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月11日 01:22
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## sndd.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 少年得到 解锁课程vip
^https?:\/\/igetcool-gateway.igetcool.com\/(api\/product\/course\/option\/course\/audios|app-api-product|app-api-user-server).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/sndd.js

#[mitm] 
# hostname = *igetcool.com

## stepsapp.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：计步器 StepsApp（恢复购买）
# 软件版本：7.0.20
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## stepsapp.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >计步器 StepsApp（恢复购买）
^https?:\/\/api.revenuecat.com\/v1\/(receipts|subscribers).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/stepsapp.js

#[mitm] 
# hostname = api.revenuecat.com

## sugar.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：糖心视频VIP+付费视频解锁
# 软件版本：loon✅，quantumulyx✅，surge✅
# 下载地址：
# 脚本作者：
# 更新时间：2026年0208
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## sugar.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/([a-z0-9-]+\.)*tx[a-z0-9-]*\.com\/h5\/(user\/info|system\/info|movie\/(detail|block|search)) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/sugar.js

#[mitm]
# hostname = tx.*.com,tx.*.cn,txh057.com,txh*.com,txh066.com,tth.txh*.com

## symfs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：声音魔法师
# 软件版本：1.1.6
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## symfs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >声音魔法师
^https?:\/\/amgaic.soundkey.net\/iosvoicemagic\/urZXO8RY\/ifZXO8RY url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/symfs.js

#[mitm] 
# hostname = amgaic.soundkey.net

## taijumi.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：泰剧迷-泰剧tv大全爱泰剧部落
# 软件版本：1.0.7
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## taijumi.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >泰剧迷-泰剧tv (去广告)
^https?:\/\/api.dingcy.top\/HJ_Video\/HJ_Config.php url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/taijumi.js



#[mitm]
# hostname = api.dingcy.top

## tgzj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：糖果追剧-全网海量热门短剧小说
# 软件版本：1.0.6
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## tgzj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 糖果追剧-全网海量热门短剧小说
^https?:\/\/candydjapi.winner1319.top\/tg\/v8\/account\/command\/userInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/tgzj.js

#[mitm]
# hostname = candydjapi.winner1319.top

## thlyzs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：通话录音助手 vip
# 软件版本：
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## thlyzs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >通话录音助手 vip
^https?:\/\/call-recorder.xinmengmakeji.com\/api\/user$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/thlyzs.js

#[mitm] 
# hostname = call-recorder.xinmengmakeji.com

## ticking.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：ticking
# 软件版本：1.18.27
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ticking.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > ticking
^https:\/\/todo\.i99yun\.com\/v2\/account\/get_user_by_token url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ticking.js

#[mitm]
# hostname = todo.i99yun.com

## tlp.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：塔罗牌-星座运势爱情测测 vip 
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## tlp.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 塔罗牌-星座运势爱情测测 vip 
^https?:\/\/www.40sishi.com\/tarot\/user\/profile url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/tlp.js

#[mitm] 
# hostname = www.40sishi.com

## tlsx.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：途灵随行—情侣定位家庭位置共享（永久会员）
# 软件版本：++
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## tlsx.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >途灵随行—情侣定位家庭位置共享（永久会员）
^https?:\/\/locnew-api.shoujicad.com.+\/api\/user$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/tlsx.js

#[mitm] 
# hostname = locnew-api.shoujicad.com

## tmmyxpk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：汤姆猫英雄跑酷(微信小程序) - 仅修改货币
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## tmmyxpk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/h5-tthd-server.jinkejoy.com\/user\/login url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/tmmyxpk.js
#[mitm]
# hostname = h5-tthd-server.jinkejoy.com

## toptoons.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：TopToons顶通韩漫网 金币解锁🔓
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## toptoons.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >TopToons顶通韩漫网 金币付费购买章节即可，无需登录！
^https?:\/\/toptoons.(online|click)\/(buychapter|comic\/\d+) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/toptoons.js

#[mitm]
# hostname = toptoons.click,toptoons.online

## touxiang.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：头像制作-情侣头像制作大全（永久会员）
# 软件版本：1.2
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## touxiang.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >头像制作-情侣头像制作大全（永久会员）
^https?:\/\/laoniu.dicallapp.com\/HMGetMeCount.ashx url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/touxiang.js

#[mitm] 
# hostname = laoniu.dicallapp.com

## toxx.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：toxx-可爱治愈的心情日记手帐本（永久会员）
# 软件版本：1.0.9
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## toxx.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >toxx-可爱治愈的心情日记手帐本（永久会员）
^https?:\/\/toxxres8api.wscreativity.com\/v2\/purchase\/vip\/verification url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/toxx.js

#[mitm] 
# hostname = toxxres8api.wscreativity.com

## tpcl.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：图片处理-照片处理ai修复工具（永久会员）
# 软件版本：1.3.8
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## tpcl.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >图片处理-照片处理ai修复工具（永久会员）
^https?:\/\/xpc.autvideo.com\/api\/user\/my url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/tpcl.js

#[mitm] 
# hostname = xpc.autvideo.com

## tpkt.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：一卟抠图-ai抠图换背景证件照制作消除笔抠图p图扣图软件（永久会员）
# 软件版本：1.4.2
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## tpkt.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >一卟抠图-ai抠图换背景证件照制作消除笔抠图p图扣图软件（永久会员）
^https?:\/\/ff.mofangzhice.com\/api\/client\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/tpkt.js

#[mitm] 
# hostname = ff.mofangzhice.com

## tqjc.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：塔奇剧场
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## tqjc.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >塔奇剧场
^https?:\/\/quickapi.xiegangsir.com\/api\/app\/.+\/user\/viptime\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/tqjc.js

#[mitm]
# hostname = quickapi.xiegangsir.com

## tswz.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：特殊文字-特殊字体转换app（永久会员）
# 软件版本：2.1.2
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## tswz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >特殊文字-特殊字体转换app（永久会员）
^https?:\/\/fancytext.ycase.cn\/app\/(user_info|package_type) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/tswz.js

#[mitm] 
# hostname = fancytext.ycase.cn

## ttdj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：天天短剧-热门短剧追不停
# 软件版本：1.1.4.6
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ttdj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >天天短剧
^https?:\/\/api.maibiq.top\/center-(admin|biz)\/(app-api\/v1\/users\/me|app-api\/v1\/skitsLists\/check) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ttdj.js
^https?://api\.maibiq\.top/center-system/app-api/v1/banner/list url reject-dict

#[mitm]
# hostname = api.maibiq.top

## ttjz.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：团团记账-自动记账ai聊天记账软件
# 软件版本：2.6.1
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ttjz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 团团记账-自动记账ai聊天记账软件
^https?:\/\/gs.gateway.gameley.cn\/np-book-user\/user\/refresh\/token url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ttjz.js

#[mitm]
# hostname = gs.gateway.gameley.cn

## tuu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：兔u-引领广播剧潮牌
# 软件版本：2.1.7
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## tuu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >兔u-引领广播剧潮牌 Unlock VIP SVIP
^https?:\/\/api.zuidie.net\/v1\/user\/info\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/tuu.js
# >开屏?
#[mitm]
# hostname = api.zuidie.net

## twxz.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：探味闲置
# 软件版本：1.2.8
# 下载地址：
# 脚本作者：@TmeNu11
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## twxz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 探味闲置
^https:\/\/admin\.tanweixianzhi\.com\/prod\/api\/centre\/userInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/twxz.js
^https:\/\/admin\.tanweixianzhi\.com\/prod\/api\/video\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/twxz.js

#[mitm]
# hostname = admin.tanweixianzhi.com

## tx.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：糖心视频VIP+付费视频解锁
# 软件版本：loon✅，quantumulyx✅，surge✅
# 下载地址：
# 脚本作者：
# 更新时间：2026年0208
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## tx.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/([a-z0-9-]+\.)*tx[a-z0-9-]*\.com\/h5\/(user\/info|system\/info|movie\/(detail|block|search)) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/tx.js

#[mitm]
# hostname = tx.*.com,tx.*.cn,txh057.com,txh*.com,txh066.com,tth.txh*.com

## tyxj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：甜盐相机（永久会员）
# 软件版本：8.2.11
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## tyxj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >甜盐相机（永久会员）
^https?:\/\/purchase-tianyan-api.tianyancam.com\/v1\/purchase\/subscription\/subscriber\/status url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/tyxj.js
^http://api-ad\.kajicam.com/ad/pop/rule/v2 url reject
#[mitm] 
# hostname = purchase-qingtu-api.b612kaji.com,api-qingtu.kajicam.com,purchase-tianyan-api.tianyancam.com,api-ad.kajicam.com

## vsco.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：vsco: photo & video editor（恢复购买）
# 软件版本：375.0.0
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## vsco.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >vsco: photo & video editor（恢复购买）
^https?:\/\/api.revenuecat.com\/v1/receipts url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/vsco.js

#[mitm] 
# hostname = api.revenuecat.com

## wangyiyun.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：网易云音乐 
# 软件版本：9.3.60
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wangyiyun.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 网易云音乐
# 播放器会员皮肤
^https:\/\/interface3?\.music\.163\.com\/eapi\/playermode\/ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/wangyiyun.js
# 搜索结果会员歌曲
^https:\/\/interface3?\.music\.163\.com\/eapi\/search\/complex\/(page|rec\/song\/get) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/wangyiyun.js
# 播放器会员歌曲
^https:\/\/interface3?\.music\.163\.com\/eapi\/v3\/song\/detail url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/wangyiyun.js
^https:\/\/interface3?\.music\.163\.com\/eapi\/song\/(chorus|enhance\/|play\/|type\/detail\/get) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/wangyiyun.js
^https:\/\/interface3?\.music\.163\.com\/eapi\/(v1\/artist\/top\/song|v3\/discovery\/recommend\/songs) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/wangyiyun.js
# 侧边栏会员等级
^https:\/\/interface3?\.music\.163\.com\/eapi\/vipnewcenter\/app\/resource\/newaccountpage url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/wangyiyun.js
# 首页歌单会员歌曲
^https?:\/\/interface3?\.music\.163\.com\/eapi\/(homepage\/|v6\/)?playlist\/ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/wangyiyun.js
# 会员认证
^https?:\/\/interface3?\.music\.163\.com\/eapi\/vipauth\/app\/auth\/(soundquality\/)?query url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/wangyiyun.js
# 去除部分广告
^https?://interface.*\.music\.163\.com/eapi/ad/get url reject
^https?://interface.*\.music\.163\.com/eapi/ad/config/get url reject
^https?://interface.*\.music\.163\.com/eapi/ad/iyunIds url reject
^https?://interface.*\.music\.163\.com/eapi/ad/prefetch/select url reject
^https?://interface.*\.music\.163\.com/eapi/ad/loading/current url reject
#[mitm] 
# hostname = *music.163.com,interface*.music.163.com,

## weinidushi.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：为你读诗pro
# 软件版本：1.5.6
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## weinidushi.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 为你读诗pro
^^https?:\/\/app.thepoemforyou.com\/api\/v1\/(getProgramList|getUserInfo) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/weinidushi.js

#[mitm]
# hostname = app.thepoemforyou.com

## wfcy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：悟饭触悦
# 软件版本：1.0.4
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年3月24日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wfcy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 悟饭触悦 vip
^https?:\/\/iosv2.cjapi.*\/user\/ios_store_pay_edition\/members_login\/get_member_info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/wfcy.js

#[mitm] 
# hostname = iosv2.cjapi.5fun.com,iosv2.cjapi.wufan88.com

## wfgljz.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：王富贵垃圾站（无限马内💰）
# 软件版本：
# 下载地址：微信小程序(如果金币没变删除小程序重新进一遍)
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wfgljz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >王富贵垃圾站（无限马内💰）
^https?:\/\/www.icgame.top.*\/getGameUserData url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/wfgljz.js

#[mitm] 
# hostname = www.icgame.top

## wnbfq.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：万能播放器-全格式视频手机解码 vip 
# 软件版本：1.3.3
# 下载地址：
# 脚本作者：
# 更新时间：2024年 3.29
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wnbfq.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 万能播放器 vip 
^https?:\/\/www.40sishi.com\/seaplayer\/user\/profile url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/wnbfq.js

#[mitm] 
# hostname = www.40sishi.com

## wnbsq.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：万能变声器 vip 
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wnbsq.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 万能变声器 vip 
^https?:\/\/www.40sishi.com\/voice\/user\/profile url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/wnbsq.js

#[mitm] 
# hostname = www.40sishi.com

## woting.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：我听 解锁vip电子书,讲书
# 软件版本：++
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月10日 22:39
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## woting.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 我听 解锁vip电子书,讲书
^https?:\/\/api.wotingfm.com\/wt-api\/(wtRichVoiceIntro|wtRichPdf|wtRichAudio\/getAudio|users\/homePage|wtRichAudioItem).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/woting.js
^https://api\.wotingfm\.com/wt-api/ad/list-by-ad-type url reject
#[mitm] 
# hostname = api.wotingfm.com

## wscf.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：网上厨房-家常菜谱美食烘焙教程
# 软件版本：16.8.99
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wscf.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# >网上厨房-家常菜谱美食烘焙教程
^https:\/\/api\.ecook\.xiaochushuo\.com url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/wscf.js

#[mitm] 
# hostname = api.ecook.xiaochushuo.com

## wshb.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：微商海报-海报制作&搞定海报设计（永久会员）
# 软件版本：1.3.8
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wshb.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >微商海报-海报制作&搞定海报设计（永久会员）
^https?:\/\/api-poster.ycase.cn\/(web\/display\?subject|api\/user\/info|api\/User\/home) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/wshb.js

#[mitm] 
# hostname = api-poster.ycase.cn

## wudaosheng.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：舞蹈生 
# 软件版本：1.3
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wudaosheng.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >舞蹈生
^https?:\/\/apidance.ctwh77.com\/api\/(newitem\/detail|user4app\/myinfo) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/wudaosheng.js
^https://apidance\.ctwh77\.com/api/appsplash/detail url reject

#[mitm]
# hostname = apidance.ctwh77.com

## wuzei.js 从第1行到 [rewrite_local] 前一行：
# 
# /*
#  *
#  *
# 脚本功能：污贼直播（前神户）会员+去广告
# 软件版本：0.8.1++
# 下载地址：
# 脚本作者：
# 更新时间：2024年,
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wuzei.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >18+ 污贼直播（前神户）会员+去广告
^https?:\/\/.+Api\/(Account\/LoginV2|App\/Splash|App\/Update|App\/PlatData|App\/homeData|Live\/Play) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/wuzei.js
^https?:\/\/.+\/Api\/Account\/LoginV2 url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/wuzeick.js

#[mitm] 
# hostname = www.auth.open.52kuajing002.51kuajing.net,*.aliyuncs.com,videocloud.cn-hangzhou.log.aliyuncs.com,www.auth.open.52kuajing002.51kuajing.net

## wuzei1.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：污贼直播（前神户）会员+去广告
# 软件版本：++
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wuzei1.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >18+ 污贼直播（前神户）会员+去广告
^https?:\/\/.+Api\/(Account\/Login|App\/Splash|App\/Update).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/wuzei1.js

#[mitm] 
# hostname = www.auth.open.52kuajing002.51kuajing.net

## wuzeick.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## wuzeick.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## wxds.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：微信读书 解锁付费vip
# 软件版本：8.0.0
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月29日 22:39
# 电报频道：
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wxds.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 微信读书vip(需重新用微信登陆，无法刷新->关闭脚本进入) 
^https?:\/\/i.weread.qq.com\/(login|pay\/balance|user\/profile).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/wxds.js
^https://i\.weread\.qq\.com/updateConfig url reject
#[mitm] 
# hostname = i.weread.qq.com

## wxhcr.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：火柴人神射手（金币、钻石）
# 软件版本：
# 下载地址：微信小程序(如果金币没变删除小程序重新进一遍)
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wxhcr.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >火柴人神射手（金币、钻石）
^https?:\/\/game.dcgstd.cn\/api\/v1\/account\/init\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/wxhcr.js

#[mitm] 
# hostname = game.dcgstd.cn

## wztk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：污贼直播（前神户）会员+去广告
# 软件版本：0.8.0++
# 下载地址：
# 脚本作者：
# 更新时间：2024年,6.20
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wztk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >18+ 污贼直播（前神户）会员+去广告
^https?:\/\/.+Api\/(Account\/Login|App\/Splash|App\/Update|App\/PlatData|App\/homeData|Live\/Play).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/wuzei.js
^https?:\/\/www\.auth\.open\.52kuajing002\.51kuajing\.net\/Api\/Account\/Login.*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/wztk.js

#[mitm] 
# hostname = www.auth.open.52kuajing002.51kuajing.net,*.aliyuncs.com,videocloud.cn-hangzhou.log.aliyuncs.com

## wzzyyzs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：
# 软件版本：
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## wzzyyzs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >文字转语音助手 vip
^https?:\/\/text-to-speech\.xinmengmakeji\.com\/api\/user url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/wzzyyzs.js

#[mitm] 
# hostname = text-to-speech.xinmengmakeji.com

## xboy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：xboy ,会员+付费。18+
# 软件版本：++
# 下载地址：
# 脚本作者：
# 更新时间：2024年3月3日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xboy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > Xboy,会员+付费
^https?:\/\/(.*x-boy|.*xo2xo).live/api\/(movie\/DetailInfo|Account\/IndexDetail|movie\/watch|VersionIosNew).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xboy.js

#[mitm] 
# hostname = *x-boy.*,*.xo2xo.live

## xhczb.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：桃花村直播 搭配“SenPlayer播放器” 弹窗播放
# 软件版本：
# 下载地址：邀请码:T2595
# 脚本作者：**
# 更新时间：2024
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## xhczb.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 桃花村直播 搭配“SenPlayer播放器” 弹窗播放
^http:\/\/lb\.h12345\.net\/zb\/.*\.json url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xhczb.js

#[mitm] 
# hostname = lb.h12345.net

## xhsdj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：西红柿短剧-全网热门短剧抢先看
# 软件版本：1.3.0.2
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xhsdj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >西红柿短剧
^https?:\/\/api.jycds.cn\/(app-api\/v1\/skitsLists\/frontCheck\?fileName|login\/detail) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xhsdj.js


#[mitm]
# hostname = api.jycds.cn

## xhts.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：小荷听书 解锁vip，付费讲书
# 软件版本：++
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月18日 06:55
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xhts.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 小荷听书 解锁vip，付费
^https?:\/\/xiaohe.sjs.com.cn\/App\/\/(Chapter\/index|User\/index|Money\/vip).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xhts.js

#[mitm] 
# hostname = xiaohe.sjs.com.cn

## xiaohongshu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：小红书 无水印视频/图片下载 + 开启作者关闭的下载权限
# 软件版本：9.1.8
# 下载地址：
# 脚本作者：@RuCu6
# 更新时间：2025年1月
# 电报频道：https://t.me/GieGie777
# 使用声明：此脚本仅供学习与交流，请勿用于商业用途！
# *******************************

## xiaohongshu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 小红书 无水印视频/图片下载 + 强制开启下载权限
^https?:\/\/(edith|rec|www).xiaohongshu.com\/api\/sns.+\/(videofeed|video\/save|imagefeed|live_photo\/save) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xiaohongshu.js

#[mitm]
# hostname = rec.xiaohongshu.com, edith.xiaohongshu.com, www.xiaohongshu.com

## xiaoying.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：小影
# 软件版本：9.14.6
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月22日 12:33
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xiaoying.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 小影vip
^https?:\/\/api-chn.rthdo.com\/api\/rest\/u\/vipVerifyReceipt.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xiaoying.js

#[mitm] 
# hostname = api-chn.rthdo.com

## ximalaya.js 从第1行到 [rewrite_local] 前一行：
# 
# /*
#  *
#  *
# 脚本功能：喜马拉雅+会员调试
# 软件版本：9.4.50
# 脚本作者：
# 更新时间：+20260609
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ximalaya.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 喜马拉雅+会员解锁+大师课+音质/音效/下载/播放器皮肤
^https?:\/\/.+((ximalaya)|(xmcdn)).+(mobile-user\/v2\/homePage|product\/detail\/v1|v1\/album\/track\/ts|mobile-playpage\/track\/v4\/baseInfo\/ts|mobile-playpage\/playpage\/tabs\/v2|mobile-playpage\/playpage\/track\/qualityAndEffect|mobile\/download\/v2\/track|mobile-user-grade\/decoratorV2\/decorationDetails\/page|mobile-album\/album\/plant\/grass) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ximalaya.js
#  > 去广告
^https?://passport\.ximalaya\.com/friendship-mobile/v1/findFriendsBanner/show/ url reject
^https?://xdcs-collector\.ximalaya\.com/api/v1/realtime url reject
^https?://adse\.wsa\.ximalaya\.com url reject
^https?://adse\.ximalaya\.com url reject
^https?://ad\.ximalaya\.com url reject
^https?://ulogs\.umeng\.com\/unify_logs url reject
^https?://.+ximalaya\.com/x-web-activity/signIn/getHomePageSignInInfo/ url reject-200
^https?://.+ximalaya\.com/product/promotion/v1/album/price url reject-200
#底部弹出会员购买页面
^https?://mobile\.ximalaya\.com/business-sale-promotion-guide-mobile-web/popup/info/ url reject
#[mitm]
# hostname = mobwsa.ximalaya.com,mobile.ximalaya.com, apisg.himalaya.com,36.150.215.*,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*, 211.152.137.*,47.100.227.85,61.164.145.12, 106.41.204.126,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,117.34.49.212,36.103.197.65,198.18.1*.*,198.18.2*.*,101.91.13*,36.42.77.*,118.180.43.252,49.119.120.*,58.144.235.61,58.251.62*,221.204.4*.*,112.84.131.*,112.80.180.72,112.98.170.228,112.99.146.108,116.136.188.184,116.162.203.111,116.177.225.247,123.138.8.*,183.204.35.*,183.201.114.*,101.91.135.*,101.91.133.*,101.91.134.*,adse.ximalaya.com,61.170.88.*,101.91.134.*,42.56.64.*,*.xmcdn.com,*.ximalaya.com,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,112.80.180.72,112.98.170.228, 112.99.146.108,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,101.89.53.*,36.131.221.*,111.42.194.*,adse.ximalaya.com,36.131.221.*,112.84.131.*,111.6.56.*,111.6.56.228,*.xmcdn.com,120.232.165.228,43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.132.80.77,43.132.83.175,43.132.84.11,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,114.80.99.90,114.80.99.70,114.80.99.71,114.80.99.89,114.80.99.91,114.80.99.88,114.80.99.87,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.136.87,203.205.136.100,203.205.136.102,203.205.136.159,203.205.137.27,203.205.137.87,203.205.137.241,203.205.250.111,203.205.250.113,211.152.137.25,ulogs.umeng.com,passport.ximalaya.com,m.ximalaya.com,116.153.*.*,39.156.8.*,183.204.64.*,61.241.178.*,112.64.213.*,43.174.54.*,27.152.187.*,117.161.*.*,183.204.64.*,182.242.50.*,101.91.134.*,123.12.235.*,101.91.135.*,85.211.193.*,111.12.187.*,117.161.169.*,59.37.143.*,42.202.164.*,219.144.82.*,152.67.253.*,42.200.166.*,163.177.58.*,120.240.179.*,36.170.27.*,74.226.216.*

## ximalayack.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## ximalayack.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## ximalayajisuban.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：喜马拉雅极速版
# 软件版本：3.0.34
# 下载地址：
# 脚本作者：
# 更新时间：2026年0130
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ximalayajisuban.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >喜马拉雅极速版
^https?:\/\/mobile.ximalaya.com\/fmobile-user\/homePage\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ximalayajisuban.js
^https?:\/\/.+ximalaya.com\/(fmobile-track\/fmobile\/track\/playpage|mobile\/track\/pay)\/ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/ximalayajisuban.js
^https?://adse\.+ximalaya\.com/ url reject-dict

#[mitm]
# hostname = 43.175.170.*,adse*ximalaya.com,*.ximalaya.com,*.ximalaya.*,183.204.13.*,183.204.35.*,43.141.49*,101.91.133.96,120.222.152.124,183.201.114.101,120.221.238.133,111.63.149.40,111.13.143.140,117.161.38.167,120.221.238.76,111.6.56.228,111.63.149.170,120.220.18.85,111.63.149.21,117.161.38.167,183.201.114.*,101.91.135.*,101.91.133.*,101.91.134.*,adse.ximalaya.com,61.170.88.*,101.91.134.*,42.56.64.*,*.xmcdn.com,*.ximalaya.com,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89, 36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,112.80.180.72,112.98.170.228,112.99.146.108,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,101.89.53.*,36.131.221.*,111.42.194.*,adse.ximalaya.com,36.131.221.*,112.84.131.*,111.6.56.*,111.6.56.228,*.xmcdn.com,120.232.165.228,43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,114*0.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.132.80.77,43.132.83.175,43.132.84.11,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,114.80.99.90,114.80.99.70,114.80.99.71,114.80.99.89,114.80.99.91,114.80.99.88,114.80.99.87,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.136.87,203.205.136.100,203.205.136.102,203.205.136.159,203.205.137.27,203.205.137.87,203.205.137.241,203.205.250.111,203.205.250.113,211.152.137.25,ulogs.umeng.com,passport.ximalaya.com,43.174.54.*,116.153.*.*,39.156.8.*,183.204.64.*,61.241.178.*,112.64.213.*,43.174.54.*

## ximalayaparsingaudio.js 从第1行到 [rewrite_local] 前一行：
# 
# /*
#  *
#  *
# 脚本功能：喜马拉雅音频地址解析
# 软件版本：通用
# 脚本作者：
# 更新时间：2025年10月10日
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ximalayaparsingaudio.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 喜马拉雅音频地址解析
^https?:\/\/(?:mobile|mobwsa)\.ximalaya\.com\/thirdparty-share\/share\/content url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ximalayaparsingaudio.js
# > 去广告,
^https?://passport\.ximalaya\.com/friendship-mobile/v1/findFriendsBanner/show/ url reject
^https?://xdcs-collector\.ximalaya\.com/api/v1/realtime url reject
^https?://.+\.ximalaya\.com/linkeye.+/checkIP\?ip.+passport.ximalaya.com url reject
^https?://adse\.wsa\.ximalaya\.com url reject
^https?://openapi\.mysteel\.com\/v5\/getAdv\.htm url reject
^https?://ulogs\.umeng\.com\/unify_logs url reject
^https?://adse\.ximalaya\.com\/ url reject
^https?://gslbali\.ximalaya\.com url reject
^https?://www\.taobao\.com\/help\/getip\.php url reject
^https?://.+location\.ximalaya\.com url reject
^https?://.+dbehavior\.ximalaya\.com url reject
^https?://ad\.ximalaya\.com url reject
^https?://.+audid-api\.taobao.com url reject
^https?://passport\.ximalaya\.com\/user-http-app\/v1\/token\/refresh url reject
^https?://adse\.ximalaya\.com/incentive/ting/durationConfig url reject-dict
^https?://mobile\.ximalaya\.com/mobile-playpage/view/ url reject-dict

#[mitm]
# hostname = mobwsa.ximalaya.com,mobile.ximalaya.com, apisg.himalaya.com,36.150.215.*,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135, 42.81.4.198, 42.81.26.128, 42.81.120.58, 43.152.24.12, 43.152.24.18, 43.152.25.127, 43.152.29.38, 43.175.16.34, 43.175.22.25, 43.175.44.15, 49.7.69.197, 49.51.224.95, 101.33.11.32, 101.33.11.106, 101.33.20.34, 101.33.29.110, 103.105.60.99, 140.249.84.135, 140.249.85.189, 150.109.90.80, 150.109.91.35, 150.138.47.94, 150.138.136.145, 203.205.13*.*, 203.205.250.*, 211.152.137.*, 47.100.227.85, 61.164.145.12, 106.41.204.126, 118.25.119.177, 223.111.231.198, 120.22*.2*.*, 43.132.8*.*, 101.33.27.*, 43.141.11.*, 117.34.49.212, 36.103.197.65, 198.18.1*.*,198.18.2*.*, 101.91.13*, 36.42.77.*, 118.180.43.252, 49.119.120.*, 58.144.235.61, 58.251.62*, 221.204.4*.*, 112.84.131.*, 112.80.180.72, 112.98.170.228, 112.99.146.108, 116.136.188.184, 116.162.203.111, 116.177.225.247,123.138.8.*,183.204.35.*,183.201.114.*,101.91.135.*,101.91.133.*,101.91.134.*,adse.ximalaya.com,61.170.88.*,101.91.134.*,42.56.64.*,*.xmcdn.com,*.ximalaya.com,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171, 23.236.99.89, 36.99.200.135, 42.81.4.198, 42.81.26.128, 42.81.120.58, 43.152.24.12, 43.152.24.18, 43.152.25.127, 43.152.29.38, 43.175.16.34, 43.175.22.25, 43.175.44.15, 49.7.69.197, 49.51.224.95, 101.33.11.32, 101.33.11.106, 101.33.20.34, 101.33.29.110, 103.105.60.99, 140.249.84.135, 140.249.85.189, 150.109.90.80, 150.109.91.35, 150.138.47.94, 150.138.136.145, 203.205.13*.*, 203.205.250.*, 211.152.137.*, 47.100.227.85, 61.164.145.12, 106.41.204.126, 112.80.180.72, 112.98.170.228, 112.99.146.108, 118.25.119.177, 223.111.231.198, 120.22*.2*.*, 43.132.8*.*, 101.33.27.*, 43.141.11.*,101.89.53.*,36.131.221.*,111.42.194.*,adse.ximalaya.com,36.131.221.*,112.84.131.*,111.6.56.*,111.6.56.228,*.xmcdn.com,120.232.165.228,43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,114*0.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171, 23.236.99.89, 36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.132.80.77,43.132.83.175,43.132.84.11,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,114.80.99.90,114.80.99.70,114.80.99.71, 114.80.99.89,114.80.99.91,114.80.99.88, 114.80.99.87,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.136.87,203.205.136.100,203.205.136.102,203.205.136.159,203.205.137.27,203.205.137.87,203.205.137.241,203.205.250.111,203.205.250.113,211.152.137.25,ulogs.umeng.com,passport.ximalaya.com,m.ximalaya.com

## xindong.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：心动短剧-全网热门短剧海量小视频app
# 软件版本：1.4.17
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xindong.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >心动短剧-全网热门短剧海量小视频app
^https?:\/\/dj.vjread.com\/m2\/(video\/episodesV2\?episodeId|user\/register) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xindong.js
#[mitm] 

# hostname = dj.vjread.com

## xingdongjuchang.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：星动剧场
# 软件版本：4.4
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xingdongjuchang.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 星动剧场
^https?:\/\/akappapi.vipduanju.cn\/v4akapp\/(user\/index|video\/(chapter|detail)) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xingdongjuchang.js

#[mitm]
# hostname = akappapi.vipduanju.cn

## xinguang.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：心光-ai-生活记录伴侣
# 软件版本：4.0.31
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xinguang.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 心光-ai-生活记录伴侣
^https:\/\/moolight\.online\/prod-api\/getClientInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xinguang.js

#[mitm]
# hostname = moolight.online

## xintiao.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：有心跳-舞蹈-跳舞-学舞神器
# 软件版本：1.75.5
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xintiao.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 有心跳-舞蹈-跳舞-学舞神器
^https:\/\/api\.train2win\.cn\/v1\/users\/myInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xintiao.js

#[mitm]
# hostname = api.train2win.cn

## xinyutingshu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：新语听书
# 软件版本：4.2.36
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xinyutingshu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >新语听书 Unlock 会员课程
^https?:\/\/(api|i).xinyulib.com.*s url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xinyutingshu.js


#[mitm]
# hostname = api.xinyulib.com,i.xinyulib.com.cn

## xiongman.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：漫熊（美区商店下载）
# 软件版本：6.5
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xiongman.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >漫熊（美区商店下载）
^https?:\/\/cb.uvogin.xyz\/(auth\/getUserByUserid|comic\/getComicDetialByid) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/manxiong.js

#[mitm] 
# hostname = cb.uvogin.xyz

## xjdj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：香蕉短剧-视频热门在线观看社区神器 （恢复购买）
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xjdj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >
^http:\/\/8.149.129.124:3000\/api\/account\/v1 url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xjdj.js

#[mitm]
# hostname = 

## xjsm.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：星际使命(微信小程序)
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xjsm.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/star.jvplay.cn\/v2\/storage url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xjsm.js

#[mitm]
# hostname = star.jvplay.cn

## xjsp.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：香蕉视频
# 软件版本：
# 下载地址：邀请码：IQ9FLO 
# 脚本作者：**
# 更新时间：2024-
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## xjsp.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 香蕉视频++
^https?:\/\/.+\/(getGlobalData|ucp\/index).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xjsp.js
^https?:\/\/.+\/vod\/* url script-request-header https://raw.githubusercontent.com/ppmm5211/haisi/main/xjsp.js

#[mitm] 
# hostname = *.cqict.cn,cwt98ehs.cqict.cn

## xkwy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：星空外语-ai英语口语练习-雅思口语真题-ai外教一对一陪练
# 软件版本：1.8.0
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xkwy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 星空外语-ai英语口语练习-雅思口语真题-ai外教一对一陪练
^https:\/\/api\.skyai\.tech\/user\/profile url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xkwy.js

#[mitm]
# hostname = api.skyai.tech

## xmbd.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：消灭病毒 (微信小程序) 修改金币、钻石、
# 软件版本：
# 脚本作者：
# 更新时间：2026-03-31
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xmbd.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/wx-bingdu\.lanfeitech\.com\/api\/archive\/get url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xmbd.js

#[mitm]
# hostname = wx-bingdu.lanfeitech.com

## xmck.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：喜马拉雅,会员.大师课,音效&音质,去广告,界面优化+++
# 软件版本：9.2.42
# 下载地址：
# 脚本作者：**
# 更新时间：2024-7.21
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 注意事项：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## xmck.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 喜马拉雅,会员.大师课,音效&音质,去广告,界面优化+++
^https?:\/\/.+((ximalaya)|(xmcdn)).+(mobile-user\/v2|mobile-web|queryCategoryPageData|discovery-feed\/v4|vip\/v1\/recommand|product\/detail\/v1|mobile\/v1\/album|playpage|album\/price\/ts|mobile\/user\/member).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xmly.js
^https?:\/\/.+((ximalaya)|(xmcdn)).+\/[a-z]{6}\-[a-z]{8}\/track\/(v[0-9])\/[a-zA-Z]+\/ts.*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/xmck.js
# > 去广告,
^https?:\/\/openapi\.mysteel\.com\/v5\/getAdv\.htm url reject
^https?:\/\/ulogs\.umeng\.com\/unify_logs url reject
^https?:\/\/adse\.ximalaya\.com\/ url reject
^https?:\/\/gslbali\.ximalaya\.com url reject
^https?:\/\/www\.taobao\.com\/help\/getip\.php url reject
^https?:\/\/.+location\.ximalaya\.com url reject
^https?:\/\/.+dbehavior\.ximalaya\.com url reject
^https?:\/\/ad\.ximalaya\.com url reject
^https?:\/\/.+audid-api\.taobao.com url reject
^https?:\/\/passport\.ximalaya\.com\/user-http-app\/v1\/token\/refresh url reject

#[mitm] 
# hostname =  43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,114*0.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171, 23.236.99.89, 36.99.200.135, 42.81.4.198, 42.81.26.128, 42.81.120.58, 43.132.80.77, 43.132.83.175, 43.132.84.11, 43.152.24.12, 43.152.24.18, 43.152.25.127, 43.152.29.38, 43.175.16.34, 43.175.22.25, 43.175.44.15, 49.7.69.197, 49.51.224.95, 101.33.11.32, 101.33.11.106, 101.33.20.34, 101.33.29.110, 103.105.60.99, 114.80.99.90, 114.80.99.70, 114.80.99.71, 114.80.99.89, 114.80.99.91, 114.80.99.88, 114.80.99.87, 140.249.84.135, 140.249.85.189, 150.109.90.80, 150.109.91.35, 150.138.47.94, 150.138.136.145, 203.205.136.87, 203.205.136.100, 203.205.136.102, 203.205.136.159, 203.205.137.27, 203.205.137.87, 203.205.137.241, 203.205.250.111, 203.205.250.113, 211.152.137.25,ulogs.umeng.com,passport.ximalaya.com

## xmck1.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## xmck1.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## xmdj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：小木短剧 - 解锁付费视频
# 软件版本：1.0.6
# 下载地址：
# 脚本作者：
# 更新时间：2026年9月7日
# 电报频道：https://t.me/GieGie777
# 问题反馈：https://t.me/GieGie777
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xmdj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 小木短剧 - 解锁付费视频
^https?:\/\/nj\.xiaomumedia\.com\/api\/dz-shop\/v1\/playlet\/episode\/index url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xmdj.js

#[mitm]
# hostname = nj.xiaomumedia.com

## xmet.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：
# 软件版本：5.0.0
# 下载地址：
# 脚本作者：**
# 更新时间：2024-8.04
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## xmet.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 喜马拉雅儿童版会员
^https?:\/\/((.+ximalaya.+)|(.+xmcdn.+))(album\/v2\/trackRecord\/queryTrackRecordsByAlbumIdAndUid|queryByParentId|mobile\/user).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xmet.js
^https?:\/\/((.+ximalaya.+)|(.+xmcdn.+))(queryTrackRecordsByAlbumIdAndUid|album\/trackRecord\/querySampleTrack|album\/trackRecord\/palyTrack).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/xmck1.js
# > 开屏去广告
^https?://xxm\.ximalaya\.com/mobile/album/ad/asaActivate url reject-200

#[mitm] 
# hostname = xxm.ximalaya.com,*.ximalaya.*,180.153.255.123,61.172.194.187

## xmk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：熊猫客-全球自然探索指南
# 软件版本：1.9.11
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xmk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >熊猫客-全球自然探索指南
^https:\/\/shopapi\.pandatrek\.cn\/tour\/remote\/storeUserMembership url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xmk.js

#[mitm]
# hostname = shopapi.pandatrek.cn

## xmly.js 从第1行到 [rewrite_local] 前一行：
# 
# /*
#  *
#  *
# 脚本功能：喜马拉雅+会员调试
# 软件版本：9.4.50
# 脚本作者：
# 更新时间：+20260213
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xmly.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 喜马拉雅+会员解锁+大师课+音质/音效/下载/播放器皮肤
^https?:\/\/.+((ximalaya)|(xmcdn)).+(mobile-user\/v2\/homePage|product\/detail\/v1|v1\/album\/track\/ts|mobile-playpage\/track\/v4\/baseInfo\/ts|mobile-playpage\/playpage\/tabs\/v2|mobile-playpage\/playpage\/track\/qualityAndEffect|mobile\/download\/v2\/track|mobile-user-grade\/decoratorV2\/decorationDetails\/page|mobile-album\/album\/plant\/grass) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xmly.js
#  > 去广告
^https?://passport\.ximalaya\.com/friendship-mobile/v1/findFriendsBanner/show/ url reject
^https?://xdcs-collector\.ximalaya\.com/api/v1/realtime url reject
^https?://adse\.wsa\.ximalaya\.com url reject
^https?://adse\.ximalaya\.com url reject
^https?://ad\.ximalaya\.com url reject
^https?://ulogs\.umeng\.com\/unify_logs url reject
^https?://.+ximalaya\.com/x-web-activity/signIn/getHomePageSignInInfo/ url reject-200
^https?://.+ximalaya\.com/product/promotion/v1/album/price url reject-200
#底部弹出会员购买页面
^https?://mobile\.ximalaya\.com/business-sale-promotion-guide-mobile-web/popup/info/ url reject
#[mitm]
# hostname = mobwsa.ximalaya.com,mobile.ximalaya.com, apisg.himalaya.com,36.150.215.*,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*, 211.152.137.*,47.100.227.85,61.164.145.12, 106.41.204.126,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,117.34.49.212,36.103.197.65,198.18.1*.*,198.18.2*.*,101.91.13*,36.42.77.*,118.180.43.252,49.119.120.*,58.144.235.61,58.251.62*,221.204.4*.*,112.84.131.*,112.80.180.72,112.98.170.228,112.99.146.108,116.136.188.184,116.162.203.111,116.177.225.247,123.138.8.*,183.204.35.*,183.201.114.*,101.91.135.*,101.91.133.*,101.91.134.*,adse.ximalaya.com,61.170.88.*,101.91.134.*,42.56.64.*,*.xmcdn.com,*.ximalaya.com,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,112.80.180.72,112.98.170.228, 112.99.146.108,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,101.89.53.*,36.131.221.*,111.42.194.*,adse.ximalaya.com,36.131.221.*,112.84.131.*,111.6.56.*,111.6.56.228,*.xmcdn.com,120.232.165.228,43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,114*0.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.132.80.77,43.132.83.175,43.132.84.11,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,114.80.99.90,114.80.99.70,114.80.99.71,114.80.99.89,114.80.99.91,114.80.99.88,114.80.99.87,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.136.87,203.205.136.100,203.205.136.102,203.205.136.159,203.205.137.27,203.205.137.87,203.205.137.241,203.205.250.111,203.205.250.113,211.152.137.25,ulogs.umeng.com,passport.ximalaya.com,m.ximalaya.com,116.153.*.*,39.156.8.*,183.204.64.*,61.241.178.*,112.64.213.*,43.174.54.*,27.152.187.*,117.161.*.*

## xmlycs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：喜马拉雅+会员调试
# 软件版本：9.4.41
# 脚本作者：
# 更新时间：+20260122
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xmlycs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 喜马拉雅音频地址解析+会员解锁
^https?:\/\/.+((ximalaya)|(xmcdn)).+(mobile-user\/v2\/homePage|product\/detail\/v1|v1\/album\/track\/ts|mobile-playpage\/track\/v4\/baseInfo\/ts|mobile-playpage\/playpage\/tabs\/v2|mobile-playpage\/playpage\/track\/qualityAndEffect|mobile\/download\/v2\/track|mobile-user-grade\/decoratorV2\/decorationDetails\/page) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xmlycs.js
# > 去广告
^https?://passport\.ximalaya\.com/friendship-mobile/v1/findFriendsBanner/show/ url reject
^https?://xdcs-collector\.ximalaya\.com/api/v1/realtime url reject
^https?://adse\.wsa\.ximalaya\.com url reject
^https?://adse\.ximalaya\.com url reject
^https?://ad\.ximalaya\.com url reject
^https?://ulogs\.umeng\.com\/unify_logs url reject
^https?://.+ximalaya\.com/x-web-activity/signIn/getHomePageSignInInfo/ url reject-200
^https?://.+ximalaya\.com/product/promotion/v1/album/price url reject-200

#[mitm]
# hostname = mobwsa.ximalaya.com,mobile.ximalaya.com, apisg.himalaya.com,36.150.215.*,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*, 211.152.137.*,47.100.227.85,61.164.145.12, 106.41.204.126,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,117.34.49.212,36.103.197.65,198.18.1*.*,198.18.2*.*,101.91.13*,36.42.77.*,118.180.43.252,49.119.120.*,58.144.235.61,58.251.62*,221.204.4*.*,112.84.131.*,112.80.180.72,112.98.170.228,112.99.146.108,116.136.188.184,116.162.203.111,116.177.225.247,123.138.8.*,183.204.35.*,183.201.114.*,101.91.135.*,101.91.133.*,101.91.134.*,adse.ximalaya.com,61.170.88.*,101.91.134.*,42.56.64.*,*.xmcdn.com,*.ximalaya.com,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,112.80.180.72,112.98.170.228, 112.99.146.108,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,101.89.53.*,36.131.221.*,111.42.194.*,adse.ximalaya.com,36.131.221.*,112.84.131.*,111.6.56.*,111.6.56.228,*.xmcdn.com,120.232.165.228,43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,114*0.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.132.80.77,43.132.83.175,43.132.84.11,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,114.80.99.90,114.80.99.70,114.80.99.71,114.80.99.89,114.80.99.91,114.80.99.88,114.80.99.87,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.136.87,203.205.136.100,203.205.136.102,203.205.136.159,203.205.137.27,203.205.137.87,203.205.137.241,203.205.250.111,203.205.250.113,211.152.137.25,ulogs.umeng.com,passport.ximalaya.com,m.ximalaya.com,116.153.*.*,39.156.8.*,183.204.64.*,61.241.178.*,112.64.213.*

## xmlywechatapp.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：@喜马拉雅小程序
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xmlywechatapp.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >喜马拉雅极速版
^https?:\/\/.+ximalaya.com\/(revision\/user\/basic|.+track\/queryMPTrackPage) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xmlywechatapp.js
^https?:\/\/.+ximalaya.com\/(mobile-playpage\/track\/v3\/baseInfo|.+queryAlbumTrackRecordsByPage) url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/xmlywechatapp.js
^https?://adse\.wsa\.ximalaya\.com/ url reject-dict

#[mitm]
# hostname = 36.150.215.*,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135, 42.81.4.198, 42.81.26.128, 42.81.120.58, 43.152.24.12, 43.152.24.18, 43.152.25.127, 43.152.29.38, 43.175.16.34, 43.175.22.25, 43.175.44.15, 49.7.69.197, 49.51.224.95, 101.33.11.32, 101.33.11.106, 101.33.20.34, 101.33.29.110, 103.105.60.99, 140.249.84.135, 140.249.85.189, 150.109.90.80, 150.109.91.35, 150.138.47.94, 150.138.136.145, 203.205.13*.*, 203.205.250.*, 211.152.137.*, 47.100.227.85, 61.164.145.12, 106.41.204.126, 118.25.119.177, 223.111.231.198, 120.22*.2*.*, 43.132.8*.*, 101.33.27.*, 43.141.11.*, 117.34.49.212, 36.103.197.65, 198.18.1*.*,198.18.2*.*, 101.91.13*, 36.42.77.*, 118.180.43.252, 49.119.120.*, 58.144.235.61, 58.251.62*, 221.204.4*.*, 112.84.131.*, 112.80.180.72, 112.98.170.228, 112.99.146.108, 116.136.188.184, 116.162.203.111, 116.177.225.247,123.138.8.*,183.204.35.*,183.201.114.*,101.91.135.*,101.91.133.*,101.91.134.*,adse.ximalaya.com,61.170.88.*,101.91.134.*,42.56.64.*,*.xmcdn.com,*.ximalaya.com,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171, 23.236.99.89, 36.99.200.135, 42.81.4.198, 42.81.26.128, 42.81.120.58, 43.152.24.12, 43.152.24.18, 43.152.25.127, 43.152.29.38, 43.175.16.34, 43.175.22.25, 43.175.44.15, 49.7.69.197, 49.51.224.95, 101.33.11.32, 101.33.11.106, 101.33.20.34, 101.33.29.110, 103.105.60.99, 140.249.84.135, 140.249.85.189, 150.109.90.80, 150.109.91.35, 150.138.47.94, 150.138.136.145, 203.205.13*.*, 203.205.250.*, 211.152.137.*, 47.100.227.85, 61.164.145.12, 106.41.204.126, 112.80.180.72, 112.98.170.228, 112.99.146.108, 118.25.119.177, 223.111.231.198, 120.22*.2*.*, 43.132.8*.*, 101.33.27.*, 43.141.11.*,101.89.53.*,36.131.221.*,111.42.194.*,adse.ximalaya.com,36.131.221.*,112.84.131.*,111.6.56.*,111.6.56.228,*.xmcdn.com,120.232.165.228,43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,114*0.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171, 23.236.99.89, 36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.132.80.77,43.132.83.175,43.132.84.11,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,114.80.99.90,114.80.99.70,114.80.99.71, 114.80.99.89,114.80.99.91,114.80.99.88, 114.80.99.87,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.136.87,203.205.136.100,203.205.136.102,203.205.136.159,203.205.137.27,203.205.137.87,203.205.137.241,203.205.250.111,203.205.250.113,211.152.137.25,ulogs.umeng.com,passport.ximalaya.com,m.ximalaya.com

## xmndxs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：熊猫脑洞小说
# 软件版本：2.14
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xmndxs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >熊猫脑洞小说 VIP
^https?:\/\/xiaoshuo.xmxsapp.com\/api\/v3\/user\/my-center url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xmndxs.js

#[mitm]
# hostname = xiaoshuo.xmxsapp.com

## xmsm.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：课程+会员解锁
# 软件版本：++
# 下载地址：苹果商店下载
# 脚本作者:
# 更新时间：2024年2月5日 10:44
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## xmsm.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]

# > 熊猫助眠&课程+会员解锁
^https?:\/\/sleep.navolove.com\/api\/(refresh_user_info\?|get_ad_list).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xmsm.js


#[mitm] 
# hostname = sleep.navolove.com

## xnd.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：小南斗（永久会员）+
# 软件版本：1.23.4
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xnd.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >小南斗（永久会员）
^https?:\/\/papi.xiaonandou.com.cn url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xnd.js
^https?://papi\.xiaonandou\.com.cn/moria/assist/assistVipCheck url reject-dict

#[mitm] 
# hostname = papi.xiaonandou.com.cn

## xqjh.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：心晴计划-日常计划-project50自律健身打卡-习惯养成
# 软件版本：1.4.6
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xqjh.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https:\/\/relaxed\.navolove\.com\/api\/refreshUserInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xqjh.js

#[mitm]
# hostname = relaxed.navolove.com

## xqlhdt.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：小区楼号地图-快递外卖极速导航
# 软件版本：1.0.10
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xqlhdt.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 小区楼号地图-快递外卖极速导航
^https:\/\/mp\.zhuzixi\.cn\/api\/users\/getDeviceTrial url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xqlhdt.js

#[mitm]
# hostname = mp.zhuzixi.cn

## xunlei.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：迅雷
# 软件版本：2.2.21
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xunlei.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >迅雷
^https?:\/\/.+.xunlei.com\/(drive\/v1\/files|xluser.core.login\/v3\/getuserinfo) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xunlei.js
^http://adapi\.izuiyou\.com url reject
^https?://api-shoulei-ssl\.xunlei\.com/flowhub/v1/slots:batchGet url reject
#[mitm] 
# hostname = *xunlei.com,api-shoulei-ssl.xunlei.com

## xwws.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：小歪微商
# 软件版本：1.0.9
# 下载地址：
# 脚本作者：
# 更新时间：2024年3月15日 20:39 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xwws.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 小歪微商vip
^https?:\/\/xw.jietuguanjia.com\/api\/app\/(resetEquipment|userInfo|parseVideo).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xwws.js

#[mitm] 
# hostname = xw.jietuguanjia.com

## xxf.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：小旋风收音机
# 软件版本：4.5
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年10月27日
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xxf.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 小旋风恢复购买
^https?:\/\/49.232.234.212:80\/user\/fixUserPayData url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xxf.js

#[mitm] 
# hostname = 49.232.234.212

## xy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：现映-短剧独家好剧抢先看
# 软件版本：3.0.1
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https:\/\/xianying-api.fansxiaoshuo.cn\/(usercenter\/client|video\/client\/short_play) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xy.js

#[mitm]
# hostname = xianying-api.fansxiaoshuo.cn

## xyxj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：新颜相机-ai照片创意工坊 仅解锁会员模版，其他功能无法使用
# 软件版本：1.2.9
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xyxj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 新颜相机-ai照片创意工坊
^https?:\/\/yixiu.tuanduixiangce.com\/api\/v1\/userinfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xyxj.js
^https://yixiu\.tuanduixiangce\.com/api/v1/ad/config url reject
#[mitm] 
# hostname = yixiu.tuanduixiangce.com

## xyzw.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：咸鱼之王 - 通用配置修改（敌伤害=1,血量=1）
# 软件版本：微信小程序 (需要通过新手教程=通关10层)
# 下载地址：
# 脚本作者：
# 更新时间：2026年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xyzw.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >咸鱼之王 - 通用配置修改（敌伤害=1,血量=1）
^https?:\/\/xxz-xyzw-res\.hortorgames\.com\/remote\/config\/import\/10 url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xyzw.js

#[mitm]
# hostname = xxz-xyzw-res.hortorgames.com

## xyzw1.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：咸鱼之王 - 通用配置修改（敌伤害=1,血量=1）
# 软件版本：微信小程序 (需要通过新手教程=通关10层)
# 下载地址：
# 脚本作者：
# 更新时间：2026年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xyzw1.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >咸鱼之王 - 通用配置修改（敌伤害=1,血量=1）
^https?:\/\/xxz-xyzw-res\.hortorgames\.com\/remote\/config\/import url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xyzw1.js

#[mitm]
# hostname = xxz-xyzw-res.hortorgames.com

## xzj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：小组件-桌面灵动岛主题壁纸小插件 vip 
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## xzj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 小组件-桌面灵动岛主题壁纸小插件 vip 
^https?:\/\/www.40sishi.com\/artwidget\/user\/profile url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/xzj.js

#[mitm] 
# hostname = www.40sishi.com

## ybcr.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：一波超人(微信小程序)
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ybcr.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
^https?:\/\/api.xgamevip.com\/api\/v1\/getUserData url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ybcr.js

#[mitm]
# hostname = api.xgamevip.com

## yctds.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：一草亭读书 Vip
# 软件版本：1.5.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年3月26日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## yctds.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 一草亭读书 Vip
^https?:\/\/m.yicaoting.com\/api\/(center\/Center\/new_center_user|book\/book\/audio).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yctds.js

#[mitm] 
# hostname = m.yicaoting.com

## yczjz.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：一寸证件照
# 软件版本：1.1.3
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## yczjz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 一寸证件照
^https?:\/\/idp2api.netpock.com\/api\/user_info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yczjz.js

#[mitm]
# hostname = idp2api.netpock.com

## yeyan.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：18+夜艳 会员视频、小说、图片、去广告
# 软件版本：h5&轻量版
# 下载地址：
# 脚本作者：
# 更新时间：2024+
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## yeyan.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >18+夜艳
^https?:\/\/\w+.yeyan\w+.xyz\/api\/(user\/personal|community\/edit|adv\/) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yeyan.js
^https?:\/\/\w+.yeyan\w+.xyz\/api\/community\/xq_community url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/yytk.js
^https?:\/\/\w+.yeyan\w+.xyz\/api/comment/gg url reject-dict

#[mitm]
# hostname = *yeyan*

## ygmh.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：原耽漫画
# 软件版本：1.6.2
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ygmh.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >原耽漫画
^http:\/\/tianxin.xiangroutech.top\/xiaozhu\/web\/(member\/memberInfo\/getEntity|cartoon\/cartoonChapter\/needVip) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ygmh.js


#[mitm]
# hostname = 

## yhjs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：硬汗健身-专业健身计划app覆盖器械居家多场景
# 软件版本：1.49.1
# 下载地址：
# 脚本作者：https://t.me/GieGie777
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## yhjs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >硬汗健身-专业健身计划app覆盖器械居家多场景
#[rewrite_local]
^https?:\/\/api\.dailyworkout\.cn\/fitness\/user\/common\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yhjs.js

#[mitm]
# hostname = api.dailyworkout.cn,121.41.11.0

## yikaobang.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：医考帮+护考帮（解锁课程）
# 软件版本：3.5.5.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## yikaobang.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >医考帮（解锁能解锁的课程）
^https?:\/\/api.yikaobang.com.cn\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yikaobang.js

#[mitm] 
# hostname = api.yikaobang.com.cn

## ylts.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：
# 软件版本：
# 下载地址：
# 脚本作者：**
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## ylts.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 月亮听书 解锁付费听书购买
^https?:\/\/www.yueliangfm.com\/v5\/api\/getUserData url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ylts.js
^https?:\/\/www.yueliangfm.com\/.+\/api\/(chapter\?bookId|getChapterUrl).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/ylts1.js
^https?:\/\/www.yueliangfm.com\/v5\/api\/getCurrentChapter.*$ url script-request-body https://raw.githubusercontent.com/WeiGiegie/666/main/ylts2.js
# > 去广告
^https?://case-cdn\.oceanplayable\.com url reject
^https?://ios\.bugly\.qq\.com url reject
^https?://ulogs\.um.*\.com url reject
^https?://token\.safebrowsing\.apple url reject
^https?://toblog\.ctobsnssdk\.com url reject

#[mitm] 
# hostname = www.yueliangfm.com,ios.bugly.qq.com,ulogs.umeng.com,ulogs.umengcloud.com,*toutiao.com,toblog.ctobsnssdk.com,token.safebrowsing.apple,toblog.ctobsnssdk.com

## ylts1.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## ylts1.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## ylts2.js 从第1行到 [rewrite_local] 前一行：
## [跳过] 未检测到 [rewrite_local]

## ylts2.js 中的 [rewrite_local] 到 hostname:
## [跳过] 未检测到 rewrite_local 或 hostname

## ymdj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：薏米短剧
# 软件版本：4.1.2
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ymdj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 薏米短剧
^https?:\/\/newyimi-api.zhangyue.com\/(usercenter\/client/mine_tab|video\/client\/short_play\/episode_list) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ymdj.js

#[mitm]
# hostname = newyimi-api.zhangyue.com

## youdao.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：有道翻译官 无限体验同传翻译体验时间
# 软件版本：4.3.15
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## youdao.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 有道翻译官-107种语言翻译
^https?:\/\/.+youdao.com\/(user\/info|vip\/user\/extra\/status) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/youdao.js
^https?:\/\/.+youdao.com\/user\/info url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/youdaouserlogin.js


#[mitm] 
# hostname = dict-ai-subtitle.youdao.com,dict.youdao.com

## youdaoall.js 从第1行到 [rewrite_local] 前一行：
# 
# /*
#  *
#  *
# 脚本功能：lectmate-有道留学听课宝
# 软件版本：2.8.0
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## youdaoall.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > lectmate-有道留学听课宝
^https?:\/\/intervip.youdao.com\/api\/verify\/ios-receipt url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/youdaoall.js

#[mitm]
# hostname = intervip.youdao.com

## youdaouserlogin.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：有道翻译官 
# 软件版本：4.3.15
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## youdaouserlogin.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 有道翻译官-107种语言翻译
^https?:\/\/dict-ai-subtitle.youdao.com\/(user\/info|vip\/user\/extra\/status) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/youdao.js
^https?:\/\/dict-ai-subtitle.youdao.com\/user\/info url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/youdaouserlogin.js


#[mitm] 
# hostname = dict-ai-subtitle.youdao.com

## youshu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：有书 ,有书精读+付费课程
# 软件版本：7.1.00
# 下载地址：
# 脚本作者：
# 更新时间：2024年3月4日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## youshu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 有书 ,有书精读+付费课程+有声书(电子书暂时无解)
^https?:\/\/(gongdu|sc).youshu.cc\/(api\/v2\/user\/page_info|m\/page\/v2|m\/user\/more_list|m\/open_screen\/rec_list|app\/gongdu\/pack_list|mp\/item\/subpage_pack_list|mp\/item\/getItemInfo|m\/media\/play_list|m\/audio_book_page\/info|m\/plan_page\/info) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/youshu.js
^https?:\/\/gongdu.youshu.cc\/m\/media\/media_info\? url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/ystk.js
#[mitm] 
# hostname = gongdu.youshu.cc,sc.youshu.cc

## yqbz.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：元气壁纸-海量超清壁纸下载
# 软件版本：1.1.9
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## yqbz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 元气壁纸-海量超清壁纸下载
^https:\/\/wallpaper\.soutushenqi\.com\/api url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yqbz.js

#[mitm]
# hostname = wallpaper.soutushenqi.com

## ysgs.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：视频雅俗共赏-朋友圈形象打造热门车型（永久会员）
# 软件版本：1.0.6
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ysgs.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >视频雅俗共赏-朋友圈形象打造热门车型（永久会员）
^https?:\/\/apps-api.lianaishouce.cn\/user\/(public\/loginByAnonymousDeviceId|getUserInfo) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/ysgs.js

#[mitm] 
# hostname = apps-api.lianaishouce.cn

## ystk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：有书 ,有书精读+付费课程
# 软件版本：7.1.00
# 下载地址：
# 脚本作者：
# 更新时间：2024年3月4日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## ystk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 有书 ,有书精读+付费课程+有声书(电子书暂时无解)
^https?:\/\/(gongdu|sc).youshu.cc\/(api\/v2\/user\/page_info|m\/page\/v2|m\/user\/more_list|m\/open_screen\/rec_list|app\/gongdu\/pack_list|mp\/item\/subpage_pack_list|mp\/item\/getItemInfo|m\/media\/play_list|m\/audio_book_page\/info|m\/plan_page\/info) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/youshu.js
^https?:\/\/gongdu.youshu.cc\/m\/media\/media_info\? url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/ystk.js
#[mitm] 
# hostname = gongdu.youshu.cc,sc.youshu.cc

## yuyinzhuanhuanwenzi.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：语音转换文字-录音转文字-语音备忘录音软件 解锁VIP
# 软件版本：1.4.0
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## yuyinzhuanhuanwenzi.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 语音转换文字-录音转文字-语音备忘录音软件
^http:\/\/yyzwzsbios.softinto.com.+\/api\/(recharge|personal\/index|user\/visit) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yuyinzhuanhuanwenzi.js


#[mitm] 
# hostname = 

## yxsj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：印象时间（永久会员）
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## yxsj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >印象时间（永久会员）
^https?:\/\/app.yinxiang.com\/third\/tasks\/user\/info url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yxsj.js

#[mitm] 
# hostname = app.yinxiang.com

## yyds.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：英语读书
# 软件版本：2.2.3
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## yyds.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 英语读书 vip
^https?:\/\/duoting.tatatimes.com\/tataeraapi\/api.s\?h=QueryVipUser url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yyds.js

#[mitm] 
# hostname = duoting.tatatimes.com

## yyfy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：语音翻译-实时翻译同声传译翻译软件（永久会员）
# 软件版本：1.2.9
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## yyfy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >语音翻译-实时翻译同声传译翻译软件（永久会员）
^https?:\/\/fy.yunmiao.tech\/api\/user\/index url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yyfy.js

#[mitm] 
# hostname = fy.yunmiao.tech

## yyjz.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：元元记账-ai极速语音记账
# 软件版本：1.2.8
# 下载地址：
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## yyjz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 元元记账-ai极速语音记账
^https?:\/\/gs.gateway.gameley.cn\/ai-book-user\/user\/refresh\/token url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yyjz.js

#[mitm]
# hostname = gs.gateway.gameley.cn

## yytk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：18+夜艳 会员视频、小说、图片、去广告
# 软件版本：h5&轻量版
# 下载地址：
# 脚本作者：
# 更新时间：2024+
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## yytk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >18+夜艳
^https?:\/\/web\w+.yeyan\w+.xyz\/api\/(user\/personal|community\/edit|adv\/) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yeyan.js
^https?:\/\/web\w+.yeyan\w+.xyz\/api\/community\/xq_community url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/yytk.js
^https?:\/\/web\w+.yeyan\w+.xyz\/api/comment/gg url reject-dict

#[mitm]
# hostname = *yeyan*

## yyxxt.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：粤语学习通-学说广东话粤语学习翻译词典
# 软件版本：8.0.1
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## yyxxt.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 粤语学习通-学说广东话粤语学习翻译词典
^http:\/\/api\.528529\.com\/apple_product url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yyxxt.js

#[mitm]
# hostname = api.528529.com

## yyzjz.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：易颜证件照-智能美颜证件照制作 （永久会员）
# 软件版本：1.1.0
# 脚本功能: 风云学习宝-中小学试卷题库精选试卷宝ai写作(永久会员)
# 软件版本：1.3
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## yyzjz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 易颜证件照-智能美颜证件照制作 （永久会员）
# > 风云学习宝-中小学试卷题库精选 试卷宝ai写作 （永久会员）
^https?:\/\/fufei.cfcpe.com.cn\/api\/client\/info.*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yyzjz.js

#[mitm] 
# hostname = fufei.cfcpe.com.cn

## yyzwz.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：语音转文字-录音转文字软件（永久会员）
# 软件版本：1.1.5
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## yyzwz.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >语音转文字-录音转文字软件（永久会员）
^https?:\/\/ly.autvideo.com\/api\/user\/index url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/yyzwz.js

#[mitm] 
# hostname = ly.autvideo.com

## zhengqi.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：正气-戒色助手
# 软件版本：7.1.33
# 下载地址：
# 脚本作者：@liul0ng
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：@liul0ng
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zhengqi.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 正气-戒色助手
^https:\/\/(api|apio)\.zhengqi100\.com url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zhengqi.js

#[mitm]
# hostname = api.zhengqi100.com,apio.zhengqi100.com

## zhihu.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：知乎vip++
# 软件版本：++最新
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zhihu.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 知乎vip 盐选内容，
^https?:\/\/((api|app|www).zhihu.com\/|.*resolv\?host)(unlimited\/go\/my_card|people\/self|bazaar\/vip_tab|sku\/reversion_sku|.*zhihu).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zhihu.js
^https?:\/\/((www)|(api)).zhihu.com\/(api\/v3\/books|market\/paid_magazine|market\/paid_column|market\/album|remix\/resource\/paid_magazine|appview\/v2\/answer).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/zhihutk.js

^https?://api\.zhihu\.com/commercial_api/launch_v2 url reject
^https?://api\.zhihu\.com/commercial_api\/real_time_launch_v2 url reject-dict
^https?://datahub\.zhihu\.com/collector/lastn-realtime url reject
^https?://www\.zhihu\.com/appview/v3/zhmore url reject-200
#^https?://www\.zhihu\.com/xen/market/hybridMetaDetail/ url reject
^https?:\/\/datahub\.zhihu\.com\/collector\/lastn-realtime url reject-dict
^https?:\/\/api\.zhihu\.com\/commercial_api\/launch_v2 url reject-dict
^https?://www\.zhihu\.com/appview/v3/zhmore url reject-200
^https?://www\.zhihu.com/api/v4/members/homepage_card url reject

#[mitm] 
# hostname = api.zhihu.com,app.zhihu.com.www.zhihu.com,datahub.zhihu.com,datahub.zhihu.com,duga.zhihu.com,mqtt.zhihu.com,apm.zhihu.com,*.zhimg.com,61.162.174.*,123.125.244.*,119.188.123.*,101.72.233.*,120.92.107.*,116.114.96.*,211.93.211.*,211.204.58.*,1.24.81.*,103.41.167.*,125.39.43.*,219.159.26.*,42.231.136.*,115.56.76.*,124.163.196.*,211.204.209.*,42.231.136.*,211.204.166.*,218.29.50.*,101.72.254.*,60.28.220.*,116.136.12.*,27.221.72.*,119.29.29.*,118.89.204.198,103.41.167.237,2402:4e00:1200:ed00:0:9089:6dac:96b6,www.zhihu.com,api.zhihu.com,zhuanlan.zhihu.com,appcloud2.zhihu.com,m-cloud.zhihu.com,103.41.167.236,103.41.167.234,103.41.167.235,103.41.167.226,116.114.96.41,120.222.238.146,58.222.30.*

## zhihutk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：知乎vip++
# 软件版本：++最新
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zhihutk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 知乎vip 盐选内容，
^https?:\/\/((api|app|www).zhihu.com\/|.*resolv\?host)(unlimited\/go\/my_card|people\/self|bazaar\/vip_tab|sku\/reversion_sku|.*zhihu).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zhihu.js
^https?:\/\/((www)|(api)).zhihu.com\/(api\/v3\/books|market\/paid_magazine|market\/paid_column|market\/album|remix\/resource\/paid_magazine|appview\/v2\/answer).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/zhihutk.js

^https?://api\.zhihu\.com/commercial_api/launch_v2 url reject
^https?://api\.zhihu\.com/commercial_api\/real_time_launch_v2 url reject-dict
^https?://datahub\.zhihu\.com/collector/lastn-realtime url reject
^https?://www\.zhihu\.com/appview/v3/zhmore url reject-200
#^https?://www\.zhihu\.com/xen/market/hybridMetaDetail/ url reject
^https?:\/\/datahub\.zhihu\.com\/collector\/lastn-realtime url reject-dict
^https?:\/\/api\.zhihu\.com\/commercial_api\/launch_v2 url reject-dict
^https?://www\.zhihu\.com/appview/v3/zhmore url reject-200
^https?://www\.zhihu.com/api/v4/members/homepage_card url reject

#[mitm] 
# hostname = api.zhihu.com,app.zhihu.com.www.zhihu.com,datahub.zhihu.com,datahub.zhihu.com,duga.zhihu.com,mqtt.zhihu.com,apm.zhihu.com,*.zhimg.com,61.162.174.*,123.125.244.*,119.188.123.*,101.72.233.*,120.92.107.*,116.114.96.*,211.93.211.*,211.204.58.*,1.24.81.*,103.41.167.*,125.39.43.*,219.159.26.*,42.231.136.*,115.56.76.*,124.163.196.*,211.204.209.*,42.231.136.*,211.204.166.*,218.29.50.*,101.72.254.*,60.28.220.*,116.136.12.*,27.221.72.*,119.29.29.*,118.89.204.198,103.41.167.237,2402:4e00:1200:ed00:0:9089:6dac:96b6,www.zhihu.com,api.zhihu.com,zhuanlan.zhihu.com,appcloud2.zhihu.com,m-cloud.zhihu.com,103.41.167.236,103.41.167.234,103.41.167.235,103.41.167.226,116.114.96.41,120.222.238.146,111.6.56.*,103.41.167.*

## zhtk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：知乎vip++
# 软件版本：++最新+++
# 下载地址：苹果商店下载+
# 脚本作者：
# 更新时间：2025
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zhtk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 知乎vip 盐选内容，
^https?:\/\/((api|app|www).zhihu.com\/|.*resolv\?host)(unlimited\/go\/my_card|people\/self|bazaar\/vip_tab|sku\/reversion_sku|.*zhihu).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zhvip.js
^https?:\/\/((www)|(api)).zhihu.com\/(api\/v3\/books|market\/paid_magazine|market\/paid_column|market\/album|remix\/resource\/paid_magazine|appview\/v2\/answer).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/zhtk.js

^https?://api\.zhihu\.com/commercial_api/launch_v2 url reject
^https?://api\.zhihu\.com/commercial_api\/real_time_launch_v2 url reject-dict
^https?://datahub\.zhihu\.com/collector/lastn-realtime url reject
^https?://www\.zhihu\.com/appview/v3/zhmore url reject-200
^https?://www\.zhihu\.com/xen/market/hybridMetaDetail/ url reject
^https?:\/\/datahub\.zhihu\.com\/collector\/lastn-realtime url reject-dict
^https?:\/\/api\.zhihu\.com\/commercial_api\/launch_v2 url reject-dict
^https?://www\.zhihu\.com/appview/v3/zhmore url reject-200
^https?://www\.zhihu.com/api/v4/members/homepage_card url reject

#[mitm] 
# hostname = api.zhihu.com,app.zhihu.com.www.zhihu.com,datahub.zhihu.com,datahub.zhihu.com,duga.zhihu.com,mqtt.zhihu.com,apm.zhihu.com,*.zhimg.com,61.162.174.*,123.125.244.*,119.188.123.*,101.72.233.*,120.92.107.*,116.114.96.*,211.93.211.*,211.204.58.*,1.24.81.*,103.41.167.*,125.39.43.*,219.159.26.*,42.231.136.*,115.56.76.*,124.163.196.*,211.204.209.*,42.231.136.*,211.204.166.*,218.29.50.*,101.72.254.*,60.28.220.*,116.136.12.*,27.221.72.*,119.29.29.*,118.89.204.198,103.41.167.237,2402:4e00:1200:ed00:0:9089:6dac:96b6,www.zhihu.com,api.zhihu.com,zhuanlan.zhihu.com,appcloud2.zhihu.com,m-cloud.zhihu.com,103.41.167.236,103.41.167.234,103.41.167.235,103.41.167.226,116.114.96.41

## zhuanyexiangji.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：专业相机-专业音视频录制工具
# 软件版本：1.0.5
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zhuanyexiangji.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >专业相机-专业音视频录制工具 Unlock VIP
^http:\/\/ym.dreamyin.cn\/\/ym_user/IsVip\? url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zhuanyexiangji.js


#[mitm]
# hostname = 

## zhvip.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：知乎vip++
# 软件版本：++最新
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年3月9 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zhvip.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 知乎vip 盐选内容，
^https?:\/\/((api|app|www).zhihu.com\/|.*resolv\?host)(unlimited\/go\/my_card|people\/self|bazaar\/vip_tab|sku\/reversion_sku|.*zhihu).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zhvip.js
^https?:\/\/((www)|(api)).zhihu.com\/(api\/v3\/books|market\/paid_magazine|market\/paid_column|market\/album|remix\/resource\/paid_magazine|appview\/v2\/answer).*$ url script-request-header https://raw.githubusercontent.com/WeiGiegie/666/main/zhtk.js

^https?://api\.zhihu\.com/commercial_api/launch_v2 url reject
^https?://api\.zhihu\.com/commercial_api\/real_time_launch_v2 url reject-dict
^https?://datahub\.zhihu\.com/collector/lastn-realtime url reject
^https?://www\.zhihu\.com/appview/v3/zhmore url reject-200
#^https?://www\.zhihu\.com/xen/market/hybridMetaDetail/ url reject
^https?:\/\/datahub\.zhihu\.com\/collector\/lastn-realtime url reject-dict
^https?:\/\/api\.zhihu\.com\/commercial_api\/launch_v2 url reject-dict
^https?://www\.zhihu\.com/appview/v3/zhmore url reject-200
^https?://www\.zhihu.com/api/v4/members/homepage_card url reject

#[mitm] 
# hostname = api.zhihu.com,app.zhihu.com.www.zhihu.com,datahub.zhihu.com,datahub.zhihu.com,duga.zhihu.com,mqtt.zhihu.com,apm.zhihu.com,*.zhimg.com,61.162.174.*,123.125.244.*,119.188.123.*,101.72.233.*,120.92.107.*,116.114.96.*,211.93.211.*,211.204.58.*,1.24.81.*,103.41.167.*,125.39.43.*,219.159.26.*,42.231.136.*,115.56.76.*,124.163.196.*,211.204.209.*,42.231.136.*,211.204.166.*,218.29.50.*,101.72.254.*,60.28.220.*,116.136.12.*,27.221.72.*,119.29.29.*,118.89.204.198,103.41.167.237,2402:4e00:1200:ed00:0:9089:6dac:96b6,www.zhihu.com,api.zhihu.com,zhuanlan.zhihu.com,appcloud2.zhihu.com,m-cloud.zhihu.com,103.41.167.236,103.41.167.234,103.41.167.235,103.41.167.226,116.114.96.41,120.222.238.146

## zjrl.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：追剧日历-你的追剧好帮手
# 软件版本：2.0.4
# 下载地址：
# 脚本作者：
# 更新时间：2025年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zjrl.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >追剧日历-你的追剧好帮手
^https?:\/\/api.rc-backup.com\/v1\/subscribers\/identify url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zjrl.js

#[mitm]
# hostname = api.rc-backup.com

## zjzxg.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：证件照相馆-美颜证件照&一寸证件照相机（永久会员）
# 软件版本：1.4.2
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zjzxg.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >证件照相馆-美颜证件照&一寸证件照相机（永久会员）
^https?:\/\/appss.rhinoxlab.com\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zjzxg.js

#[mitm] 
# hostname = appss.rhinoxlab.com

## zmfy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：字幕翻译-实时视频字幕识别视频翻译软件（永久会员）
# 软件版本：1.1.8
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zmfy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >字幕翻译-实时视频字幕识别视频翻译软件（永久会员）
^https?:\/\/zimu.yunmiaomiao.cn\/api\/user\/index url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zmfy.js

#[mitm] 
# hostname = zimu.yunmiaomiao.cn

## zmxzj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：美化小组件-桌面倒计时小组件（永久会员）
# 软件版本：1.1.4
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zmxzj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >美化小组件-桌面倒计时小组件（永久会员）
^https?:\/\/timewp.szsszykj.com\/api\/getiosvip url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zmxzj.js

#[mitm] 
# hostname = timewp.szsszykj.com

## zptcq.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：自拍题词器 vip  
# 软件版本：2.1.1
# 下载地址：
# 脚本作者：
# 更新时间：2024.4.14
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zptcq.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 自拍题词器 vip 
^https?:\/\/appss.rhinox.cn\/\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zptcq.js
# > 开屏广告
^https://ad\.rhinox\.cn/client/sdk/getData url reject
#[mitm] 
# hostname = ad.rhinox.cn,standard.rhinoxlab.com.appss.rhinoxlab.com,appss.rhinox.cn

## zstkx.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：知识脱口秀
# 软件版本：1.5.2
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zstkx.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >知识脱口秀
^https:\/\/talkshow.wfkids.net\/wfcm-api\/(vip\/queryVipInfo|talkshow\/chapter|course\/courseInfo) url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zstkx.js


#[mitm]
# hostname = talkshow.wfkids.net

## zukong.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：足控社
# 软件版本：
# 下载地址：
# 脚本作者：
# 更新时间：2026年0303
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zukong.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >足控社
^https?:\/\/.+\/appapi\/(detail|buyPicture|getUserInfo|goldBuyVideo)\/ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zukong.js


#[mitm]
# hostname = 1zher0st.504824.xyz,s1jyc4asdr14nsa.504824.xyz,suning.679134.xyz,b8u1lvlb.504867.xyz,umcuy46v9yr3gn4.504843.xyz,*.504*.xyz,5sfc4hgy.504832.xyz,core.*.xyz,v1a.*.xyz,file.*.xyz,data.*.xyz,stream.*.xyz,svc.core*.xyz,test.*.xyz,doc.lum*.xyz,apiv1.*.xyz,

## zwds.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：早晚读书vip+课程、有声书、电子书
# 软件版本：1.8.4
# 下载地址：
# 脚本作者：
# 更新时间：2024年3月20日 
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zwds.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 早晚读书vip+付费课程、有声书、vip电子书,付费书
^https?:\/\/api.zaowandushu.com\/(app\/initApp|v\/customer\/info|ebook\/detail|resource\/(content\/play\/view|ebook\/play\/chapter)).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zwds.js

#[mitm] 
# hostname = api.zaowandushu.com

## zxsy.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：解锁vip电子书,付费电子书
# 软件版本：++
# 下载地址：苹果商店下载
# 脚本作者：
# 更新时间：2024年2月9日 01:09
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！作者TG频道 : https://t.me/GieGie777
# *******************************

## zxsy.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# > 中信书院 解锁vip电子书,付费电子书
^https?:\/\/napi.yunpub.cn\/api\/(product/getBookContent|user/login\/getUserInfo|cms\/index\/getMyPage).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zxsy.js

#[mitm] 
# hostname = napi.yunpub.cn

## zyfj.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：中医方剂（解锁会员）
# 软件版本：2.3
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zyfj.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >中医方剂（解锁会员）
^https?:\/\/medicine-books.meappstore.top\/medicineModule\/api\/users\/iosUserLogin url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zyfj.js

#[mitm] 
# hostname = medicine-books.meappstore.top

## zymk.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：知音漫客 解锁会员+付费
# 软件版本：4.1.6
# 下载地址：
# 脚本作者：
# 更新时间：2024+
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：⚠️此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！⚠️⚠️⚠️
# *******************************

## zymk.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >知音漫客 解锁会员+付费
^https?:\/\/apigate.kaimanhua.com\/zymk.+(paychapters|getuserinfo).*$ url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zymk.js
^https?://api-cdn\.kaimanhua\.com/advertiseapi/app/advertise/getappadvertise url reject


#[mitm]
# hostname = apigate.kaimanhua.com,api-cdn.kaimanhua.com

## zypg.js 从第1行到 [rewrite_local] 前一行：
# /*
#  *
#  *
# 脚本功能：作业批改-家长辅导作业工具（永久会员）
# 软件版本：2.1.1
# 下载地址：
# 脚本作者：
# 更新时间：2024年
# 电报频道：https://t.me/GieGie777
# 问题反馈：
# 使用声明：此脚本仅供学习与交流，请在下载使用24小时内删除！请勿在中国大陆转载与贩卖！
# *******************************

## zypg.js 中的 [rewrite_local] 到 hostname:
#[rewrite_local]
# >作业批改-家长辅导作业工具（永久会员）
https?:\/\/appss.rhinoxlab.com\/app\/account\/getAccountInfo url script-response-body https://raw.githubusercontent.com/WeiGiegie/666/main/zypg.js

#[mitm] 
# hostname = appss.rhinoxlab.com


hostname = ,*.bilibili.*,*.biliapi.*,111.31.33.*,111.6.167.*,app.bilibili.com,api.bilibili.com,api.live.bilibili.com,grpc.biliapi.net,*.cloudfront.net,dag29jmgma1g.site,*site,d1no17eycutrsi.cloudfront.net,lldajbfyobffkbg288-api.com,lasdasdgapi.cc,las23gsapi.cc,*.cqict.cn,cwt98ehs.cqict.cn,*.cyapi.cn,*.dushu365.com,gw2.dushu365.com,gw1.dushu365.com,*.kkmh.com,*kkmh.com,*.kuwo.cn,abt-kuwo.tencentmusic.com,ad.tencentmusic.com:443,*.migu.cn,*.miguvideo.com,*.qidian.com,*.if.qidian.com,*.reader.qq.com,*.reader.qq.com,newminerva-tgw.reader.qq.com,detailadr.reader.qq.com,commontgw.reader.qq.com,*.tencentcs.com,settings.itranscreen.com,api.itranscreen.com:8080,*api.imendon.com,*hundun.cn,*igetcool.com,*mgtv.com,hb-boom.api.mgtv.com,*.api.mgtv.com,mobile-stream.api.mgtv.com,hb-boom.api.mgtv.com,mobile-stream.api.mgtv.com,nuc.api.mgtv.com,111.31.104.108,111.31.104.*,111.31.104.96,*.api.mgtv.com,iphone.v0.mgtv.com,api.data.mgtv.com,*.*.mgtv.*,,*music.163.com,interface*.music.163.com,,*x-boy.*,*.xo2xo.live,*xdplt.com,*xunlei.com,api-shoulei-ssl.xunlei.com,*yeyan*,114.116.104.132,118.31.70.61,wapi.koudailc.net,139fm.bond,139fm.fun,139fm.cfd,139fm.click,139fm.cyou,18pcs.bond,18pcs.fun,18pcs.cfd,1zher0st.504824.xyz,s1jyc4asdr14nsa.504824.xyz,suning.679134.xyz,b8u1lvlb.504867.xyz,umcuy46v9yr3gn4.504843.xyz,*.504*.xyz,5sfc4hgy.504832.xyz,core.*.xyz,v1a.*.xyz,file.*.xyz,data.*.xyz,stream.*.xyz,svc.core*.xyz,test.*.xyz,doc.lum*.xyz,apiv1.*.xyz,,36.150.215.*,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,117.34.49.212,36.103.197.65,198.18.1*.*,198.18.2*.*,101.91.13*,36.42.77.*,118.180.43.252,49.119.120.*,58.144.235.61,58.251.62*,221.204.4*.*,112.84.131.*,112.80.180.72,112.98.170.228,112.99.146.108,116.136.188.184,116.162.203.111,116.177.225.247,123.138.8.*,183.204.35.*,183.201.114.*,101.91.135.*,101.91.133.*,101.91.134.*,adse.ximalaya.com,61.170.88.*,101.91.134.*,42.56.64.*,*.xmcdn.com,*.ximalaya.com,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,112.80.180.72,112.98.170.228,112.99.146.108,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,101.89.53.*,36.131.221.*,111.42.194.*,adse.ximalaya.com,36.131.221.*,112.84.131.*,111.6.56.*,111.6.56.228,*.xmcdn.com,120.232.165.228,43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,114*0.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.132.80.77,43.132.83.175,43.132.84.11,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,114.80.99.90,114.80.99.70,114.80.99.71,114.80.99.89,114.80.99.91,114.80.99.88,114.80.99.87,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.136.87,203.205.136.100,203.205.136.102,203.205.136.159,203.205.137.27,203.205.137.87,203.205.137.241,203.205.250.111,203.205.250.113,211.152.137.25,ulogs.umeng.com,passport.ximalaya.com,m.ximalaya.com,39.107.159.85,43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,114*0.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.132.80.77,43.132.83.175,43.132.84.11,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,114.80.99.90,114.80.99.70,114.80.99.71,114.80.99.89,114.80.99.91,114.80.99.88,114.80.99.87,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.136.87,203.205.136.100,203.205.136.102,203.205.136.159,203.205.137.27,203.205.137.87,203.205.137.241,203.205.250.111,203.205.250.113,211.152.137.25,ulogs.umeng.com,passport.ximalaya.com,43.175.170.*,adse*ximalaya.com,*.ximalaya.com,*.ximalaya.*,183.204.13.*,183.204.35.*,43.141.49*,101.91.133.96,120.222.152.124,183.201.114.101,120.221.238.133,111.63.149.40,111.13.143.140,117.161.38.167,120.221.238.76,111.6.56.228,111.63.149.170,120.220.18.85,111.63.149.21,117.161.38.167,183.201.114.*,101.91.135.*,101.91.133.*,101.91.134.*,adse.ximalaya.com,61.170.88.*,101.91.134.*,42.56.64.*,*.xmcdn.com,*.ximalaya.com,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,112.80.180.72,112.98.170.228,112.99.146.108,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,101.89.53.*,36.131.221.*,111.42.194.*,adse.ximalaya.com,36.131.221.*,112.84.131.*,111.6.56.*,111.6.56.228,*.xmcdn.com,120.232.165.228,43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,114*0.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.132.80.77,43.132.83.175,43.132.84.11,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,114.80.99.90,114.80.99.70,114.80.99.71,114.80.99.89,114.80.99.91,114.80.99.88,114.80.99.87,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.136.87,203.205.136.100,203.205.136.102,203.205.136.159,203.205.137.27,203.205.137.87,203.205.137.241,203.205.250.111,203.205.250.113,211.152.137.25,ulogs.umeng.com,passport.ximalaya.com,43.174.54.*,116.153.*.*,39.156.8.*,183.204.64.*,61.241.178.*,112.64.213.*,43.174.54.*,49.232.234.212,91acg.website,91acg.*,91fengliu.club,aawapi-v3.trh999.com,accounts.felo.me,translator.felo.me,accounts.felo.me,translator.felo.me,52.78.154.178,ad.rhinox.cn,appss.rhinoxlab.com,ad.rhinox.cn,appss.rhinoxlab.com,appss.rhinox.cn,ad.rhinox.cn,appss.rhinoxlab.com,appss.rhinoxlab.com,ad.rhinox.cn,standard.rhinoxlab.com,ad.rhinox.cn,standard.rhinoxlab.com,appss.rhinoxlab.com,ad.rhinox.cn,standard.rhinoxlab.com.appss.rhinoxlab.com,appss.rhinox.cn,admin.tanweixianzhi.com,ainieci.com,akappapi.vipduanju.cn,alien.kampfiregames.cn,amgaic.soundkey.net,api-chn.jianbotaiapi.com,api-chn.rthdo.com,api-cn.hellotalk8.com,api-ipv6.acfunchina.com,api-ipv6.app.acfun.cn,api-poster.ycase.cn,api-sub.meitu.com,api.selfiecity.meitu.com,api.*.shop,api.528529.com,api.736136.com,api.9169kkxstzsjkdd222.app,api.ankiapp.com,api.banxueketang.com,api.dailyworkout.cn,121.41.11.0,api.dingcy.top,api.douhuawenxue.com,api.duoyolo.cn,ioskgeyd.lmkaiger.sanwubeixin.cn,www.shutiaomh.com,mobads.baidu.com,bgg.baidu.com,sf3-fe-tos.pglstatp-toutiao.com,cmyd-10-88.getui.com,open.e.kuaishou.com,api-access.pangolin-sdk-toutiao.com,toblog.ctobsnssdk.com,log-api.pangolin-sdk-toutiao.com,api-access.pangolin-sdk-toutiao1.com,log-api.pangolin-sdk-toutiao-b.com,api-access.pangolin-sdk-toutiao-b.com,b-gy.getui.net,gy.gepush.com,mssdk.volces.com,sdk.open.talk.getui.net,gromore.pangolin-sdk-toutiao.com,sdk.open.talk.gepush.com,adservice.sigmob.cn,sdk.open.talk.getui.com,yun.tuitiger.com,mobads-pre-config.cdn.bcebos.com,mobads-logs.baidu.com,api.ecook.xiaochushuo.com,api.fengkuangapp.com,api.flysheeep.com,api.gotokeep.com,kit.gotokeep.com,*.gotokeep.*,120.53.74.*,162.14.5.*,42.187.199.*,101.42.124.*,api.himalaya.com,*.himalaya.com,apisg.himalaya.com,www.himalaya.com,api.jingyuanw.top,api.juzikong.com,api.jycds.cn,api.kmovie.gifshow.com,id.kwaiying.com,mi.gdt.qq.com,api.lispod.cn,api.magictiger.ai,api.maibiq.top,api.manxinao.com,api.mimuapp.com,api.mycake.me,api.owhealth.com,api.qiyou.cn,apifast.qiyou.cn,api.qmxfs.com,api.qonversion.io,api.rc-backup.com,api.read10.cn,api.revenuecat.com,api.revenuecat.com,api.account.meitu.com,api.risingfalling.com,api.shaolinzen.com,api.skyai.tech,api.timecaps.app,api.train2win.cn,api.wokanjian.com.cn,api.wotingfm.com,api.xgamevip.com,api.xinyulib.com,i.xinyulib.com.cn,api.yikaobang.com.cn,api.zaowandushu.com,api.zhangdefenspace.com,api.zhengqi100.com,apio.zhengqi100.com,api.zhihu.com,app.zhihu.com.www.zhihu.com,datahub.zhihu.com,datahub.zhihu.com,duga.zhihu.com,mqtt.zhihu.com,apm.zhihu.com,*.zhimg.com,61.162.174.*,123.125.244.*,119.188.123.*,101.72.233.*,120.92.107.*,116.114.96.*,211.93.211.*,211.204.58.*,1.24.81.*,103.41.167.*,125.39.43.*,219.159.26.*,42.231.136.*,115.56.76.*,124.163.196.*,211.204.209.*,42.231.136.*,211.204.166.*,218.29.50.*,101.72.254.*,60.28.220.*,116.136.12.*,27.221.72.*,119.29.29.*,118.89.204.198,103.41.167.237,2402:4e00:1200:ed00:0:9089:6dac:96b6,www.zhihu.com,api.zhihu.com,zhuanlan.zhihu.com,appcloud2.zhihu.com,m-cloud.zhihu.com,103.41.167.236,103.41.167.234,103.41.167.235,103.41.167.226,116.114.96.41,api.zhihu.com,app.zhihu.com.www.zhihu.com,datahub.zhihu.com,datahub.zhihu.com,duga.zhihu.com,mqtt.zhihu.com,apm.zhihu.com,*.zhimg.com,61.162.174.*,123.125.244.*,119.188.123.*,101.72.233.*,120.92.107.*,116.114.96.*,211.93.211.*,211.204.58.*,1.24.81.*,103.41.167.*,125.39.43.*,219.159.26.*,42.231.136.*,115.56.76.*,124.163.196.*,211.204.209.*,42.231.136.*,211.204.166.*,218.29.50.*,101.72.254.*,60.28.220.*,116.136.12.*,27.221.72.*,119.29.29.*,118.89.204.198,103.41.167.237,2402:4e00:1200:ed00:0:9089:6dac:96b6,www.zhihu.com,api.zhihu.com,zhuanlan.zhihu.com,appcloud2.zhihu.com,m-cloud.zhihu.com,103.41.167.236,103.41.167.234,103.41.167.235,103.41.167.226,116.114.96.41,120.222.238.146,api.zhihu.com,app.zhihu.com.www.zhihu.com,datahub.zhihu.com,datahub.zhihu.com,duga.zhihu.com,mqtt.zhihu.com,apm.zhihu.com,*.zhimg.com,61.162.174.*,123.125.244.*,119.188.123.*,101.72.233.*,120.92.107.*,116.114.96.*,211.93.211.*,211.204.58.*,1.24.81.*,103.41.167.*,125.39.43.*,219.159.26.*,42.231.136.*,115.56.76.*,124.163.196.*,211.204.209.*,42.231.136.*,211.204.166.*,218.29.50.*,101.72.254.*,60.28.220.*,116.136.12.*,27.221.72.*,119.29.29.*,118.89.204.198,103.41.167.237,2402:4e00:1200:ed00:0:9089:6dac:96b6,www.zhihu.com,api.zhihu.com,zhuanlan.zhihu.com,appcloud2.zhihu.com,m-cloud.zhihu.com,103.41.167.236,103.41.167.234,103.41.167.235,103.41.167.226,116.114.96.41,120.222.238.146,111.6.56.*,103.41.167.*,api.zhihu.com,app.zhihu.com.www.zhihu.com,datahub.zhihu.com,datahub.zhihu.com,duga.zhihu.com,mqtt.zhihu.com,apm.zhihu.com,*.zhimg.com,61.162.174.*,123.125.244.*,119.188.123.*,101.72.233.*,120.92.107.*,116.114.96.*,211.93.211.*,211.204.58.*,1.24.81.*,103.41.167.*,125.39.43.*,219.159.26.*,42.231.136.*,115.56.76.*,124.163.196.*,211.204.209.*,42.231.136.*,211.204.166.*,218.29.50.*,101.72.254.*,60.28.220.*,116.136.12.*,27.221.72.*,119.29.29.*,118.89.204.198,103.41.167.237,2402:4e00:1200:ed00:0:9089:6dac:96b6,www.zhihu.com,api.zhihu.com,zhuanlan.zhihu.com,appcloud2.zhihu.com,m-cloud.zhihu.com,103.41.167.236,103.41.167.234,103.41.167.235,103.41.167.226,116.114.96.41,120.222.238.146,58.222.30.*,api.zuidie.net,api0.easeprime.com,apidance.ctwh77.com,apigate.kaimanhua.com,api-cdn.kaimanhua.com,apimodel.ctwh77.com,apipc01.gushi.cn,apis.lifeweek.com.cn,apis.lifeweek.com.cn:443,apis40.qupeiyin.com,apisg.himalaya.com,api.himalaya.com,app-i.dj-5.com,app.666syh.com,app.666syh.com,huibo.hcbnjy.com,app.api.bgwxc.com,app.dongfamao.com,app.dongfamao.com:443,app.faxingwu.com,app.gaohaozuowen.com,app.qtfm.cn,user.qtfm.cn,recpage-c.qtfm.cn,entry.qtfm.cn,ad.qtfm.cn,ip.qtfm.cn,adlaunch.qtfm.cn,woqt2.qtfm.cn,app.thepoemforyou.com,app.thwlqy.com,app.yinxiang.com,apps-api.lianaishouce.cn,apps-api.lingege.cn,appss.baomingding.com,appss.linhongshi.com,appss.rhinoxlab.com,appv3.filmix.com.cn,audio.xinmengma.com,bd-api.kuwo.cn,*.kuwo.*,ad.tencentmusic.com,*.l.qq.com,39.156.123.46:443,bd-api.kuwo.com,39.156.123.46,39.156.12*.*:443,h5app.kuwo.cn,bodianimgcdn.kuwo.cn,h5s.kuwo.cn,39.156.121.20:443,111.30.171.170:443,39.156.121.21:443,h5s.kuwo.cn,39.156.121.20:443,101.42.133.54:443,ab-bodian.kuwo.cn,39.156.121.65:443,39.156.121.*:443,bd-api.kuwo.cn:443,ab-bodian.kuwo.cn:443,39.156.121.60:80,xs.gdt.qq.com,bread28resapi.wscreativity.com,call-recorder.xinmengmakeji.com,camera.xinmengma.com,candydjapi.winner1319.top,cb.uvogin.xyz,ch.sleepcycle.com,cljssmb.5exg.cn,cn.invit.vip,www.lovewith.me,poster.leminet.cn,cocamapi.imendon.com,counting.movingshop.cn,csj.yy-5.com,dict-ai-subtitle.youdao.com,dict-ai-subtitle.youdao.com,dict.youdao.com,dj.vjread.com,dtpkwx-prod-web.gxpan.cn,duoting.tatatimes.com,explorer.tratao.com,fancytext.ycase.cn,ff.mofangzhice.com,firebaseremoteconfig.googleapis.com,fqapi.tiantiangf.cn,freeapp-api.zhangdu.com,121.40.246.38,121.41.30.120,front-gw.kuque.com,front-gw.luckpik.com,fuciyuanbang.ciyuans.com,fufei.cfcpe.com.cn,fy.yunmiao.tech,g.catches.com,g20.manmeng168.com,game.dcgstd.cn,gameapi.mma.dstor.cc,*.flare.com.51kuajing.net,gentlemanapp*,get-notes.luojilab.com,gnjk.*.com,mfjk.*.com,buyaoda.*.com,gnjk*.com,gnjk.*.com,mfjk.*.com,gnjk*.com,gongdu.youshu.cc,sc.youshu.cc,gs.gateway.gameley.cn,gw.qnniu.com,h5-tthd-server.jinkejoy.com,h5.jinliapp.top,h5s.kuwo.cn,anymatch.kuwo.cn,musicpay.kuwo.cn,mobilebasedata.kuwo.cn,vip1.kuwo.cn,rich.kuwo.cn,mobilead.kuwo.cn,*.kuwo.cn,hqfyg.5exg.cn,hxxx51.com,www.hxxxaa.com,hxxx52.com,app.hxxxaa.com,i.weread.qq.com,idp2api.netpock.com,in.m.jd.com,intervip.youdao.com,ioskgeyd.lmkaiger.sanwubeixin.cn,iosqwrmdj.sanwubeixin.cn,iosv2.cjapi.5fun.com,iosv2.cjapi.wufan88.com,japaneselar.qupeiyin.com,javcomics.store,javelin.mandrillvr.com,jiekou.antciti.cn,jlpt.jingqueyun.com,judouapp.com,kapi-action.shyuhuankj.com,kky.logtk.com,kkyun.com,laoniu.dicallapp.com,lb.h12345.net,lc.i7play.com,leancloud.emotionwp.com,lentoapp.com:8091,linecamera-api.snow.me,locnew-api.shoujicad.com,ly.autvideo.com,m.yicaoting.com,man.api.mansenwenhua77.com,mapi.kouyuxingqiu.com,md.liaoliaoy.com,www.yueliangfm.com,ios.bugly.qq.com,ulogs.umeng.com,ulogs.umengcloud.com,*toutiao.com,toblog.ctobsnssdk.com,token.safebrowsing.apple,toblog.ctobsnssdk.com,medi.dxzzywxk.top,medicine-books.meappstore.top,member.mkzcdn.com,comic.mkzcdn.com,base.mkzcdn.com,mobwsa.ximalaya.com,mobile.ximalaya.com,apisg.himalaya.com,36.150.215.*,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,117.34.49.212,36.103.197.65,198.18.1*.*,198.18.2*.*,101.91.13*,36.42.77.*,118.180.43.252,49.119.120.*,58.144.235.61,58.251.62*,221.204.4*.*,112.84.131.*,112.80.180.72,112.98.170.228,112.99.146.108,116.136.188.184,116.162.203.111,116.177.225.247,123.138.8.*,183.204.35.*,183.201.114.*,101.91.135.*,101.91.133.*,101.91.134.*,adse.ximalaya.com,61.170.88.*,101.91.134.*,42.56.64.*,*.xmcdn.com,*.ximalaya.com,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,112.80.180.72,112.98.170.228,112.99.146.108,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,101.89.53.*,36.131.221.*,111.42.194.*,adse.ximalaya.com,36.131.221.*,112.84.131.*,111.6.56.*,111.6.56.228,*.xmcdn.com,120.232.165.228,43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,114*0.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.132.80.77,43.132.83.175,43.132.84.11,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,114.80.99.90,114.80.99.70,114.80.99.71,114.80.99.89,114.80.99.91,114.80.99.88,114.80.99.87,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.136.87,203.205.136.100,203.205.136.102,203.205.136.159,203.205.137.27,203.205.137.87,203.205.137.241,203.205.250.111,203.205.250.113,211.152.137.25,ulogs.umeng.com,passport.ximalaya.com,m.ximalaya.com,mobwsa.ximalaya.com,mobile.ximalaya.com,apisg.himalaya.com,36.150.215.*,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,117.34.49.212,36.103.197.65,198.18.1*.*,198.18.2*.*,101.91.13*,36.42.77.*,118.180.43.252,49.119.120.*,58.144.235.61,58.251.62*,221.204.4*.*,112.84.131.*,112.80.180.72,112.98.170.228,112.99.146.108,116.136.188.184,116.162.203.111,116.177.225.247,123.138.8.*,183.204.35.*,183.201.114.*,101.91.135.*,101.91.133.*,101.91.134.*,adse.ximalaya.com,61.170.88.*,101.91.134.*,42.56.64.*,*.xmcdn.com,*.ximalaya.com,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,112.80.180.72,112.98.170.228,112.99.146.108,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,101.89.53.*,36.131.221.*,111.42.194.*,adse.ximalaya.com,36.131.221.*,112.84.131.*,111.6.56.*,111.6.56.228,*.xmcdn.com,120.232.165.228,43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,114*0.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.132.80.77,43.132.83.175,43.132.84.11,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,114.80.99.90,114.80.99.70,114.80.99.71,114.80.99.89,114.80.99.91,114.80.99.88,114.80.99.87,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.136.87,203.205.136.100,203.205.136.102,203.205.136.159,203.205.137.27,203.205.137.87,203.205.137.241,203.205.250.111,203.205.250.113,211.152.137.25,ulogs.umeng.com,passport.ximalaya.com,m.ximalaya.com,116.153.*.*,39.156.8.*,183.204.64.*,61.241.178.*,112.64.213.*,mobwsa.ximalaya.com,mobile.ximalaya.com,apisg.himalaya.com,36.150.215.*,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,117.34.49.212,36.103.197.65,198.18.1*.*,198.18.2*.*,101.91.13*,36.42.77.*,118.180.43.252,49.119.120.*,58.144.235.61,58.251.62*,221.204.4*.*,112.84.131.*,112.80.180.72,112.98.170.228,112.99.146.108,116.136.188.184,116.162.203.111,116.177.225.247,123.138.8.*,183.204.35.*,183.201.114.*,101.91.135.*,101.91.133.*,101.91.134.*,adse.ximalaya.com,61.170.88.*,101.91.134.*,42.56.64.*,*.xmcdn.com,*.ximalaya.com,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,112.80.180.72,112.98.170.228,112.99.146.108,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,101.89.53.*,36.131.221.*,111.42.194.*,adse.ximalaya.com,36.131.221.*,112.84.131.*,111.6.56.*,111.6.56.228,*.xmcdn.com,120.232.165.228,43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,114*0.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.132.80.77,43.132.83.175,43.132.84.11,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,114.80.99.90,114.80.99.70,114.80.99.71,114.80.99.89,114.80.99.91,114.80.99.88,114.80.99.87,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.136.87,203.205.136.100,203.205.136.102,203.205.136.159,203.205.137.27,203.205.137.87,203.205.137.241,203.205.250.111,203.205.250.113,211.152.137.25,ulogs.umeng.com,passport.ximalaya.com,m.ximalaya.com,116.153.*.*,39.156.8.*,183.204.64.*,61.241.178.*,112.64.213.*,43.174.54.*,27.152.187.*,117.161.*.*,mobwsa.ximalaya.com,mobile.ximalaya.com,apisg.himalaya.com,36.150.215.*,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,117.34.49.212,36.103.197.65,198.18.1*.*,198.18.2*.*,101.91.13*,36.42.77.*,118.180.43.252,49.119.120.*,58.144.235.61,58.251.62*,221.204.4*.*,112.84.131.*,112.80.180.72,112.98.170.228,112.99.146.108,116.136.188.184,116.162.203.111,116.177.225.247,123.138.8.*,183.204.35.*,183.201.114.*,101.91.135.*,101.91.133.*,101.91.134.*,adse.ximalaya.com,61.170.88.*,101.91.134.*,42.56.64.*,*.xmcdn.com,*.ximalaya.com,61.172.194.*,180.153.*.*,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.2*,61.162.174.*,119.188.123.*,59.83.227.*,114.80.161.29,1.62.62.64,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.13*.*,203.205.250.*,211.152.137.*,47.100.227.85,61.164.145.12,106.41.204.126,112.80.180.72,112.98.170.228,112.99.146.108,118.25.119.177,223.111.231.198,120.22*.2*.*,43.132.8*.*,101.33.27.*,43.141.11.*,101.89.53.*,36.131.221.*,111.42.194.*,adse.ximalaya.com,36.131.221.*,112.84.131.*,111.6.56.*,111.6.56.228,*.xmcdn.com,120.232.165.228,43.159.71.*,ulogs.umeng.com,www.taobao.com,43.132.81.*,101.33.27.*,61.172.1*.*,43.141.11.*,114.80.99.86,180.153.255.*,114.80.99.*,*.mysteel.*,61.172.194.196,180.153.*.*,*xima*,*xmcdn*,*.ximalaya.com,*.xmcdn.com,180.153.255.*,180.153.140.*,180.153.250.*,114.80.99.*,114.80.139.237,114.80.161.29,1.62.62.64,51*.com,1.194.255.171,23.236.99.89,36.99.200.135,42.81.4.198,42.81.26.128,42.81.120.58,43.132.80.77,43.132.83.175,43.132.84.11,43.152.24.12,43.152.24.18,43.152.25.127,43.152.29.38,43.175.16.34,43.175.22.25,43.175.44.15,49.7.69.197,49.51.224.95,101.33.11.32,101.33.11.106,101.33.20.34,101.33.29.110,103.105.60.99,114.80.99.90,114.80.99.70,114.80.99.71,114.80.99.89,114.80.99.91,114.80.99.88,114.80.99.87,140.249.84.135,140.249.85.189,150.109.90.80,150.109.91.35,150.138.47.94,150.138.136.145,203.205.136.87,203.205.136.100,203.205.136.102,203.205.136.159,203.205.137.27,203.205.137.87,203.205.137.241,203.205.250.111,203.205.250.113,211.152.137.25,ulogs.umeng.com,passport.ximalaya.com,m.ximalaya.com,116.153.*.*,39.156.8.*,183.204.64.*,61.241.178.*,112.64.213.*,43.174.54.*,27.152.187.*,117.161.*.*,183.204.64.*,182.242.50.*,101.91.134.*,123.12.235.*,101.91.135.*,85.211.193.*,111.12.187.*,117.161.169.*,59.37.143.*,42.202.164.*,219.144.82.*,152.67.253.*,42.200.166.*,163.177.58.*,120.240.179.*,36.170.27.*,74.226.216.*,moolight.online,motu.motumap.com,mp.zhuzixi.cn,mv-ps.xdplt.com,mv-ps.xdplt.com,videohub.xdplt.com,namibox.com,nanie.flyingeffect.com,napi.yunpub.cn,new.tianjinzhitongdaohe.com,newyimi-api.zhangyue.com,niceday.youloft.com,nj.xiaomumedia.com,open-secret-api-v2.vcinema.cn,47.93.95.61,outcut.szsszykj.com,oxtoon.cyou,oxtoon.work,oxtoon.monster,oxtoon.net,pan.baidu.com,*.pcs.baidu.com,*.baidu.com,papi.xiaonandou.com.cn,pc7dong.com,purchase-qingtu-api.b612kaji.com,api-qingtu.kajicam.com,purchase-qingtu-api.b612kaji.com,api-qingtu.kajicam.com,purchase-tianyan-api.tianyancam.com,api-ad.kajicam.com,purchase-qingtu-api.b612kaji.com,api-qingtu.kajicam.com,purchase-tianyan-api.tianyancam.com,purchase-foodiecn-api.yiruikecorp.com,purchase-qingtu-api.b612kaji.com,api-qingtu.kajicam.com,user-kaji-api.b612kaji.com,pvip.funshion.com,pm.funshion.com,*.funshion.com,qijk.fantor.cn,qijk.slrrj.com,ldy.syxsxl.com,qijk.accessco.cn,jsqiapi.hspherecn.com,jiekou.antciti.cn,qiip.fushizhengli.cn,ldy.*.cn,ldy.yyycb.com,ldy.yyycb.com,*yyycb*,117.161.96.7,api.tongyutongye.cn,qq.baiduan*.com,*wensan*com,ws.wensangq.com,45.117.11.*,qs.szqibu.cn,quickapi.xiegangsir.com,quiz.istarl.com,rec.xiaohongshu.com,edith.xiaohongshu.com,www.xiaohongshu.com,relaxed.navolove.com,res5.haotgame.com,sc.szsszykj.com,service.gpstool.com,service.hhdd.com,sgwitxgwit231sr.xunwxc.com,rdxijq.vo9ugl.xyz,bpitbu.ouxf9g.live,tjactb.xk2nsy.live,us6tp7.dkr9cv.live,qsukip.9jts57.live,wkqyx6.qdokqj.live,fqohbx.bkljtx.live,zihjyv.jajrc1.live,y7uyqg.hl8dbl.live,qrdure.youngnoble.cn,kpoe2e.x8w7t3.live,*jttv*,kpoe2e.x8w7t3.live,zihjyv.jajrc1.live,fceyeg.johjxe.com,*.vo9ugl.xyz,zqlhcn.kl9117.xyz,bsxqtd.xk8q7w.xyz,grvgdq.81fbg2.xyz,vcryyw.uc86fy.xyz,xospby.mldo9k.xyz,pmmqls.gregdoro.com,103.85.254.233,roa6fi.lyr6if.live,shapi.mting.info,shapi.lanrentingshu.com,gzapi.lanrentingshu.com,shapi.mting.info:443,shiri.jingqueyun.com,shopapi.pandatrek.cn,simplestorage.landintheair.com,sixth.xxcjpt.com,sketch.manyatang.cn,paint.manyatang.cn,sleep.navolove.com,sm.szsszykj.com,ss.landintheair.com,standard.rhinoxlab.com,star.jvplay.cn,svc.heartlylab.com,talkshow.wfkids.net,talkshow.wfkids.net,wfjava.wfkids.net,tasky-api.codefuture.top,temple-release.dskystudio.com,text-to-speech.xinmengmakeji.com,tfbook.taoyuewenhua.net,tybook.taoyuewenhua.net,theater-api.sylangyue.xyz,timewp.szsszykj.com,tmeadcomm.y.qq.com,bd-api.kuwo.cn,*.kuwo.*,ad.tencentmusic.com,*.l.qq.com,39.156.123.46:443,bd-api.kuwo.com,39.156.123.46,39.156.12*.*:443,h5app.kuwo.cn,bodianimgcdn.kuwo.cn,h5s.kuwo.cn,39.156.121.20:443,111.30.171.170:443,39.156.121.21:443,h5s.kuwo.cn,39.156.121.20:443,101.42.133.54:443,ab-bodian.kuwo.cn,39.156.121.65:443,39.156.121.*:443,bd-api.kuwo.cn:443,ab-bodian.kuwo.cn:443,39.156.121.60:80,xs.gdt.qq.com,todo.i99yun.com,toptoons.click,toptoons.online,toptoons.click,toptoons.online,toptoon.monster,boylove.cyou,boylove.*,toptoon.*,toxxres8api.wscreativity.com,tuoba.dida110.com,tx.*.com,tx.*.cn,txh057.com,txh*.com,txh066.com,tth.txh*.com,udl.lesejie.com,us-central1-project*,user.ihuaben.com,api.ihuaben.com,voiceapi.jimetec.com,voicechanger.szsszykj.com,w1c2.ooat88.com,tu.dfgfdg.win,103.44.236.*,*.artrg.cn,vkfqb.ojyrz.com,byapi.*.com,211.99.98.*,103.39.222.*,yubaoyu.oss-cn-shenzhen.aliyuncs.com,*api.h8h4h.com,42.157.129.25,siajksslwiso.anningsh.com,211.99.98.63,111.180.204.11,88.gbvdfe.com,vkfqb.ojyrz.com,211.99.98.*,vdop.fqoas.com,ica.ooat88.com,103.44.236.131:8001,211.154.21.150:8020,211.154.21.150,103.215.78.*,wallpaper.soutushenqi.com,watch.taotiangou.cn,water-fst.yifanxinmei.com,watershandiana.yunxiaoguo.cn,www.40sishi.com,www.asmr.red,www.auth.open.52kuajing002.51kuajing.net,www.auth.open.52kuajing002.51kuajing.net,*.aliyuncs.com,videocloud.cn-hangzhou.log.aliyuncs.com,www.auth.open.52kuajing002.51kuajing.net,*.aliyuncs.com,videocloud.cn-hangzhou.log.aliyuncs.com,www.auth.open.52kuajing002.51kuajing.net,www.duobeidanci.com,www.helloshiyu.com,www.himalaya.com,apisg.himalaya.com,183.232.157.73:13037,cdn.himalaya.com:443,*.himalaya.*,api.himalaya.com,www.icgame.top,www.listenleap.com,api.listenleap.com,www.mhzcookbook.xyz,www.pozhu.com,www.shutiaoapp.com,www.shutiaomh.com,mobads.baidu.com,bgg.baidu.com,sf3-fe-tos.pglstatp-toutiao.com,cmyd-10-88.getui.com,open.e.kuaishou.com,api-access.pangolin-sdk-toutiao.com,toblog.ctobsnssdk.com,log-api.pangolin-sdk-toutiao.com,api-access.pangolin-sdk-toutiao1.com,log-api.pangolin-sdk-toutiao-b.com,api-access.pangolin-sdk-toutiao-b.com,b-gy.getui.net,gy.gepush.com,mssdk.volces.com,sdk.open.talk.getui.net,gromore.pangolin-sdk-toutiao.com,sdk.open.talk.gepush.com,adservice.sigmob.cn,sdk.open.talk.getui.com,yun.tuitiger.com,mobads-pre-config.cdn.bcebos.com,mobads-logs.baidu.com,www.yearsmap.com:443,www.yearsmap.com,www.wordvector.com,www.yueliangfm.com,ios.bugly.qq.com,ulogs.umeng.com,ulogs.umengcloud.com,*toutiao.com,toblog.ctobsnssdk.com,token.safebrowsing.apple,toblog.ctobsnssdk.com,wx-bingdu.lanfeitech.com,wx.voxpie.com,xianying-api.fansxiaoshuo.cn,xiaohe.sjs.com.cn,xiaoshuo.xmxsapp.com,xieyi.cfcpe.com.cn,xihongshi.dicallapp.com,xpc.autvideo.com,xw.jietuguanjia.com,xxm.ximalaya.com,*.ximalaya.*,180.153.255.123,61.172.194.187,xxz-xyzw-res.hortorgames.com,yixiu.tuanduixiangce.com,yr-game-api.feigo.fun,zhcn.api.wordln.com,zimu.yunmiaomiao.cn,zqlhcn.kl9117.xyz,hkg2zx.stejnu.xyz,zvqldx.xv8gvn.xyz,kuhikr.1t3vwf.xyz,s4788x.taqndh.xyz,xkewutdekf873sr.chairsr.com,103.85.254.233,omfnmh.x2b3w6.xyz,zqlhcn.kl9117.xyz,hkg2zx.stejnu.xyz,zvqldx.xv8gvn.xyz,kuhikr.1t3vwf.xyz,s4788x.taqndh.xyz,xkewutdekf873sr.chairsr.com,103.85.254.233,omfnmh.x2b3w6.xyz,sgwitxgwit231sr.xunwxc.com,zqlhcn.kl9117.xyz,ssgwew231s.uzqew4qi.info,ykofye.qklmoq.xyz,pmmqls.gregdoro.com,103.85.254.233,omfnmh.x2b3w6.xyz,zqlhcn.kl9117.xyz,lieeys42jdi2kd.cx4c5mv7.info,fjboni.jn6588.xyz,rylzit.wqws5z.xyz,riwnkx.sw05uv.xyz,pmmqls.gregdoro.com,103.85.254.233,omfnmh.x2b3w6.xyz,zqlhcn.kl9117.xyz,lieeys42jdi2kd.cx4c5mv7.info,fjboni.jn6588.xyz,rylzit.wqws5z.xyz,riwnkx.sw05uv.xyz,pmmqls.gregdoro.com,103.85.254.233,omfnmh.x2b3w6.xyz,zqlhcn.kl9117.xyz,rylzit.wqws5z.xyz,riwnkx.sw05uv.xyz,bsxqtd.xk8q7w.xyz,grvgdq.81fbg2.xyz,pmmqls.gregdoro.com,103.85.254.233,omfnmh.x2b3w6.xyz,zqlhcn.kl9117.xyz,rylzit.wqws5z.xyz,riwnkx.sw05uv.xyz,bsxqtd.xk8q7w.xyz,grvgdq.81fbg2.xyz,pmmqls.gregdoro.com,103.85.254.233,omfnmh.x2b3w6.xyz,rdxijq.vo9ugl.xyz,bpitbu.ouxf9g.live,tjactb.xk2nsy.live,us6tp7.dkr9cv.live,qsukip.9jts57.live,wkqyx6.qdokqj.live,fqohbx.bkljtx.live,zihjyv.jajrc1.live,y7uyqg.hl8dbl.live,qrdure.youngnoble.cn,kpoe2e.x8w7t3.live,*jttv*,kpoe2e.x8w7t3.live,zihjyv.jajrc1.live,fceyeg.johjxe.com,*.vo9ugl.xyz,zqlhcn.kl9117.xyz,bsxqtd.xk8q7w.xyz,grvgdq.81fbg2.xyz,vcryyw.uc86fy.xyz,xospby.mldo9k.xyz,pmmqls.gregdoro.com,103.85.254.233,roa6fi.lyr6if.live,gp1wpm.xcg3zr.xyz,nfmq0v.lknlqz.xyz,nhppjkb.z5x6pzr.xyz,fwroeor.revxcvx.xyz,uetuys234ls.pbog5txn.app,fwroeor.revxcvx.xyz,hrp27mr.1ugq1uf.xyz,nhppjkb.z5x6pzr.xyz,zd7uwox.8ewxvhz.xyz,xkewutdekf873sr.chairsr.com,5tlsw6r.7n26ytu.xyz,uetuys234ls.pbog5txn.app,5jsd6q7.jnfkdtm.xyz,yw4pk9h.7fw2snb.xyz,m3pkeg2.v4l0p7y.xyz,ip6vu33.im0684h.xyz,xkewutdekf873sr.chairsr.com,bor4lch.8l6ldmz.xyz,uetuys234ls.pbog5txn.app,5jsd6q7.jnfkdtm.xyz,yw4pk9h.7fw2snb.xyz,m3pkeg2.v4l0p7y.xyz,ip6vu33.im0684h.xyz,xkewutdekf873sr.chairsr.com,bor4lch.8l6ldmz.xyz,uetuys234ls.pbog5txn.app,58z8gxu.pzmn7ku.xyz,bor4lch.8l6ldmz.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,58z8gxu.pzmn7ku.xyz,bor4lch.8l6ldmz.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,e6xun6l.wy6z0uq.xyz,ldyc3rs.0mlptkl.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,7x16kek.nwjnf0m.xyz,2h4q7lu.hjsn53p.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,0ncf2i3.wl8td88.xyz,4dhuyek.dusgrec.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,9sdec0e.mls08ph.xyz,5c8gqqf.to1ogvz.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,nt7474f.u8kgc14.xyz,85tug87.nkwyzcp.xyz,xkewutdekf873sr.chairsr.com,uetuys234ls.pbog5txn.app,85tug87.nkwyzcp.xyz,n05ggob.batlnhh.xyz,eg8oppq.19fc5lg.xyz,xkewutdekf873sr.chairsr.com
